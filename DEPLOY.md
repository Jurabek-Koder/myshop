...
cd frontend
npm install
# .env.production — sxema B bo‘lsa VITE_API_BASE_URL ni qo‘ying (namuna: .env.example)
npm run build
```
...
| Alomat | Sabab |
|--------|--------|
| Oq sahifa, markazda sariq **MyShop** | React yuklanmagan: `/assets/*.js` 404 (static rewrite yo‘q), eski kesh, yoki build xatosi. Brauzer **F12 → Network** da qizil `assets/` so‘rovlarini tekshiring. Alohida Render Static Site bo‘lsa **Rewrite `/*` → `/index.html`** qoidasi shart. Keyin `cd frontend && npm run build` va qayta deploy. |
| `forwardRef` / `vendor-….js` xatosi | **Eski build** + brauzer/CDN keshi. Yangi buildda `vendor-*.js` bo‘lagi bo‘lmaydi (faqat `index-….js`). Render’da **Clear build cache & deploy**, brauzerda **Ctrl+Shift+R** yoki kesh tozalash. Console’da hali `vendor-FtbPF2PJ.js` ko‘rinsa — yangi deploy hali tushmagan. |
...
- `frontend/.env.example` — production build uchun `VITE_*` namuna  
- `frontend/.env.capacitor` — mobil (Capacitor) build uchun, backend production URL bilan

---

## 10. GitHub + test deploy (Render yoki bitta domen PaaS)

1. **Repozitoriy** GitHub’ga yuklang (`.env` fayllar **tushirmang**; faqat `.env.example`). Loyiha ildizida `.gitignore` mavjud — `node_modules`, `dist`, `backend/data`, `backend/uploads`, `.env*` fayllar avtomatik chetlab o'tiladi.
...
4. **Auto-deploy:** Alohida GitHub Actions shart emas — Render'ning o'z GitHub integratsiyasi `main` branch'ga push qilinganda avtomatik build+deploy qiladi (Render Dashboard → Settings → Auto-Deploy). Agar CI/CD (masalan lint/test tekshiruvi) xohlasangiz, `.github/workflows/` papkasiga alohida workflow qo'shish mumkin — hozircha loyihada yo'q.

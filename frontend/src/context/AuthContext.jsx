import React, { createContext, useContext, useState, useCallback, useEffect, useRef } from 'react';

const AuthContext = createContext(null);
const base = (import.meta.env.VITE_API_BASE_URL || '').replace(/\/$/, '');
// VITE_API_BASE_URL bo‘sh bo‘lsa — nisbiy /api: Vite devda proxy har qanday host (masalan 192.168...) bilan ishlaydi.
// To‘g‘ridan-to‘g‘ri http://localhost:3000 faqat shu kompyuterda brauzer ochilganda ishlaydi; telefon/LAN — "Failed to fetch".
const API = base ? `${base}/api` : '/api';
const APP_SECRET_KEY = String(import.meta.env.VITE_APP_SECRET_KEY || '').trim();

function mapFetchFailure(err) {
  const msg = String(err?.message || err || '');
  if (err?.name === 'TypeError' || /failed to fetch|load failed|networkerror/i.test(msg)) {
    if (typeof window !== 'undefined' && /onrender\.com$/i.test(window.location.hostname) && !base) {
      return new Error(
        "Render: API ulanmagan. Static site → Environment da VITE_API_BASE_URL ni backend HTTPS manziliga qo'ying " +
          "(masalan https://myshop-api-xxxx.onrender.com, oxirida / bo'lmasin), so'ng Clear build cache & Deploy. " +
          "Backend Web Service da CORS_ORIGINS ga ushbu frontend manzilini qo'shing (masalan https://myshop-web-xxxx.onrender.com).",
      );
    }
    if (typeof window !== 'undefined' && base && /onrender\.com$/i.test(window.location.hostname)) {
      const health = `${base}/api/health`;
      return new Error(
        `Serverga ulanib bo'lmadi (API: ${base}). ` +
          `1) Yangi tabda ${health} oching — JSON "ok" ko'rinishi kerak. ` +
          `2) Backend Render → Environment: CORS_ORIGINS (yoki FRONTEND_URL) da aynan shu sahifa manzili bo'lsin: https://${window.location.host} ` +
          `(slash va bo'sh joy yo'q). Saqlang, backendni qayta deploy qiling. ` +
          `3) F12 → Network: /api/auth/login qaysi URL ga ketayotgani va CORS xabari.`,
      );
    }
    return new Error(
      "Serverga ulanib bo'lmadi. Backend ishga tushirilganini tekshiring (port 3000). " +
        "Telefon yoki tarmoq manzili orqali kirayotgan bo'lsangiz, frontend/.env da VITE_API_BASE_URL ni bo'sh qiling yoki # bilan o'chirib, so'rovlar /api proxy orqali yuborilishini ta'minlang.",
    );
  }
  return err instanceof Error ? err : new Error(String(err));
}

function parseResponseJson(text) {
  const raw = text == null ? '' : String(text).trim();
  if (!raw) return {};
  try {
    return JSON.parse(raw);
  } catch {
    return null;
  }
}

function strErr(v) {
  if (v == null) return '';
  if (typeof v === 'string') return v.trim();
  if (typeof v === 'object' && v !== null && typeof v.message === 'string') return v.message.trim();
  return String(v).trim();
}

/** Server JSON yoki status bo‘yicha foydalanuvchiga tushunarli xabar */
function extractHttpErrorMessage(res, data) {
  if (data && typeof data === 'object' && data !== null) {
    if (Array.isArray(data.details)) {
      const parts = data.details.map((d) => strErr(d?.message ?? d?.msg ?? d)).filter(Boolean);
      if (parts.length) return parts.join(' ');
    }
    const e = strErr(data.error);
    if (e) return e;
    const m = strErr(data.message);
    if (m) return m;

    if (Array.isArray(data.errors)) {
      const parts = data.errors.map((d) => strErr(d?.msg ?? d?.message)).filter(Boolean);
      if (parts.length) return parts.join(' ');
    }
  }

  const st = res?.status;
  if (st === 401) {
    return "Kechirasiz, bunday foydalanuvchi mavjud emas. Iltimos, ma'lumotlaringizni tekshiring va qaytadan urinib ko'ring.";
  }
  if (st === 429) return "Juda ko'p urinish. Bir ozdan keyin qayta urinib ko'ring.";
  if (st === 502 || st === 503 || st === 504) return "Server vaqtincha javob bermayapti. Backend (port 3000) ishlayotganini tekshiring.";
  if (st === 404) return "Kirish manzili topilmadi. Sayt /api sozlamalari va backend holatini tekshiring.";
  if (st === 409)
    return "Bu email yoki login allaqachon band. Boshqa manzil bilan urinib ko'ring yoki tizimga kiring.";
  if (st >= 500) return `Server xatosi (${st}). Keyinroq urinib ko'ring yoki administratorga murojaat qiling.`;
  if (st >= 400) return `So'rov bajarilmadi (${st}). Ma'lumotlarni tekshirib qayta urinib ko'ring.`;
  return "Kirish amalga oshmadi. Internet va serverni tekshirib, qayta urinib ko'ring.";
}

const USER_SNAPSHOT_KEY = 'myshopAuthUserJson';
/** Cookie + localStorage «eslab qol» rejimi (server HttpOnly refresh cookie) */
const REMEMBER_FLAG_KEY = 'myshop_remember';

/* F5 / qayta yuklanganda: API vaqtincha 5xx bersa, UI «chiqib ketgandek» bo‘lmasin — joriy foydalanuvchi shu yerdan tiklanadi */
function readUserSnapshot() {
  try {
    let raw = sessionStorage.getItem(USER_SNAPSHOT_KEY);
    if (!raw && typeof localStorage !== 'undefined' && localStorage.getItem(REMEMBER_FLAG_KEY) === '1') {
      raw = localStorage.getItem(USER_SNAPSHOT_KEY);
    }
    if (!raw) return null;
    const o = JSON.parse(raw);
    return o && typeof o === 'object' ? o : null;
  } catch {
    return null;
  }
}

function snapshotPayload(user) {
  return {
    id: user.id,
    email: user.email,
    login: user.login,
    full_name: user.full_name,
    role: user.role,
    role_id: user.role_id,
    seller_id: user.seller_id,
    staff_member_id: user.staff_member_id,
    role_profile_id: user.role_profile_id ?? null,
    role_profile_kind: user.role_profile_kind ?? null,
    phone: user.phone,
    allowed_pages: user.allowed_pages != null ? user.allowed_pages : [],
  };
}

/** `persistLocal`: brauzerni yopganda ham — localStorage (faqat foydalanuvchi «Eslab qol» tanlasa) */
function writeUserSnapshot(user, persistLocal = false) {
  if (!user) {
    try {
      sessionStorage.removeItem(USER_SNAPSHOT_KEY);
    } catch {}
    try {
      localStorage.removeItem(USER_SNAPSHOT_KEY);
    } catch {}
    return;
  }
  const j = JSON.stringify(snapshotPayload(user));
  try {
    sessionStorage.setItem(USER_SNAPSHOT_KEY, j);
  } catch {}
  try {
    if (persistLocal) localStorage.setItem(USER_SNAPSHOT_KEY, j);
    else localStorage.removeItem(USER_SNAPSHOT_KEY);
  } catch {}
}

/* sessionStorage: har bir oyna/tab o‘z sessiyasini saqlaydi; alohida oynalarda turli rollar (superuser, picker) bir-birini bosib ketmaydi */
function readToken(key) {
  try {
    const session = sessionStorage.getItem(key);
    if (session) return session;
    if (typeof localStorage !== 'undefined' && localStorage.getItem(REMEMBER_FLAG_KEY) === '1') {
      return localStorage.getItem(key);
    }
    return null;
  } catch {
    return null;
  }
}

function writeToken(key, value, persistLocal = false) {
  if (!value) return;
  try {
    sessionStorage.setItem(key, value);
  } catch {}
  try {
    if (persistLocal) localStorage.setItem(key, value);
    else localStorage.removeItem(key);
  } catch {}
}

function removeToken(key) {
  try { localStorage.removeItem(key); } catch {}
  try { sessionStorage.removeItem(key); } catch {}
}

export function AuthProvider({ children }) {
  const [user, setUser] = useState(null);
  const [accessToken, setAccessToken] = useState(() => readToken('accessToken'));
  const [authStatus, setAuthStatus] = useState(() => {
    const access = readToken('accessToken');
    const refresh = readToken('refreshToken');
    return access || refresh ? 'bootstrapping' : 'guest';
  });
  const [authMessage, setAuthMessage] = useState('');

  const refreshInFlightRef = useRef(null);
  const accessTokenRef = useRef(null);
  const loading = authStatus === 'bootstrapping';

  useEffect(() => {
    try { localStorage.removeItem('rememberedEmail'); } catch {}
    try { localStorage.removeItem('rememberedPassword'); } catch {}
  }, []);

  const clearSession = useCallback((nextStatus = 'guest', message = '') => {
    setUser(null);
    writeUserSnapshot(null);
    setAccessToken(null);
    accessTokenRef.current = null;
    setAuthStatus(nextStatus);
    setAuthMessage(message);
    removeToken('accessToken');
    removeToken('refreshToken');
    try {
      localStorage.removeItem(REMEMBER_FLAG_KEY);
    } catch {}
  }, []);

  const persistTokens = useCallback((data, { persistLocal = false } = {}) => {
    const access = data?.access ?? data?.accessToken;
    const refresh = data?.refresh ?? data?.refreshToken;
    if (access) {
      setAccessToken(access);
      accessTokenRef.current = access;
      writeToken('accessToken', access, persistLocal);
    }
    if (refresh) writeToken('refreshToken', refresh, persistLocal);
  }, []);

  const refreshAccessToken = useCallback(async () => {
    if (refreshInFlightRef.current) return refreshInFlightRef.current;

    refreshInFlightRef.current = (async () => {
      try {
        const storedRt = readToken('refreshToken');
        const body = storedRt ? { refresh_token: storedRt } : {};
        const res = await fetch(`${API}/auth/refresh`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(body),
          credentials: 'include',
        });

        if (res.ok) {
          const data = await res.json();
          const alsoLocal =
            (typeof localStorage !== 'undefined' && localStorage.getItem(REMEMBER_FLAG_KEY) === '1') ||
            !!localStorage.getItem('refreshToken');
          persistTokens(data, { persistLocal: alsoLocal });
          return { ok: true, access: data.access };
        }

        if (res.status === 401) {
          let deniedMsg = "Kechirasiz, bunday foydalanuvchi mavjud emas. Iltimos, ma'lumotlaringizni tekshiring va qaytadan urinib ko'ring.";
          try {
            const data = await res.json();
            if (data?.error) deniedMsg = String(data.error);
          } catch {
            /* ignore */
          }
          clearSession('expired', deniedMsg);
          return { ok: false, reason: 'expired' };
        }

        if (res.status === 400) {
          return { ok: false, reason: 'missing' };
        }

        if (res.status === 429 || res.status >= 500) {
          setAuthMessage('Server vaqtincha band. Qayta urinib ko\'ring.');
          return { ok: false, reason: 'retryable' };
        }

        return { ok: false, reason: 'failed' };
      } catch {
        setAuthMessage('Tarmoq xatosi. Internetni tekshirib qayta urinib ko\'ring.');
        return { ok: false, reason: 'retryable' };
      } finally {
        refreshInFlightRef.current = null;
      }
    })();

    return refreshInFlightRef.current;
  }, [clearSession, persistTokens]);

  const bootstrapSession = useCallback(async () => {
    let storedAccess = readToken('accessToken');
    let storedRefresh = readToken('refreshToken');

    if (!storedAccess && !storedRefresh) {
      const fromCookie = await refreshAccessToken();
      if (!fromCookie.ok) {
        if (fromCookie.reason === 'missing' || fromCookie.reason === 'expired') {
          if (fromCookie.reason === 'expired') return;
          setUser(null);
          writeUserSnapshot(null);
          setAccessToken(null);
          accessTokenRef.current = null;
          setAuthStatus('guest');
          setAuthMessage('Kirish qilinmagan. Kirish tugmasini bosing.');
          return;
        }
        setUser(null);
        setAuthStatus('guest');
        setAuthMessage('Sessiyani tekshirib bo\'lmadi. Qayta urinib ko\'ring.');
        return;
      }
      storedAccess = readToken('accessToken');
      storedRefresh = readToken('refreshToken');
    }

    setAuthStatus('bootstrapping');
    setAuthMessage('');
    const optimistic = readUserSnapshot();
    if (optimistic) setUser(optimistic);

    if (storedAccess) accessTokenRef.current = storedAccess;
    let token = storedAccess;

    if (!token && storedRefresh) {
      const refreshed = await refreshAccessToken();
      if (!refreshed.ok) {
        if (refreshed.reason === 'expired' || refreshed.reason === 'missing') return;
        if (readUserSnapshot()) {
          setUser(readUserSnapshot());
          setAuthStatus('authenticated');
          setAuthMessage('Server vaqtincha javob bermadi. Keyinroq urinib ko‘ring.');
          return;
        }
        setUser(null);
        setAuthStatus('guest');
        setAuthMessage('Sessiyani tekshirib bo\'lmadi. Qayta urinib ko\'ring.');
        return;
      }
      token = refreshed.access;
    }

    if (!token) {
      setUser(null);
      setAuthStatus('guest');
      setAuthMessage('Kirish qilinmagan. Kirish tugmasini bosing.');
      return;
    }

    const callMe = async (access) =>
      fetch(`${API}/auth/me`, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${access}`,
        },
        credentials: 'include',
      });

    const recoverFromServerGlitch = () => {
      const snap = readUserSnapshot();
      if (snap) {
        setUser(snap);
        setAuthStatus('authenticated');
        setAuthMessage('Server vaqtincha javob bermadi. Birozdan keyin sahifani yangilang.');
        return true;
      }
      return false;
    };

    const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

    try {
      let meRes;
      for (let attempt = 0; attempt < 3; attempt++) {
        meRes = await callMe(token);
        if (meRes.ok) break;
        if (meRes.status === 401) break;
        if (meRes.status === 403) break;
        if (meRes.status === 429 || meRes.status >= 500) {
          if (attempt < 2) await sleep(400 * (attempt + 1));
          continue;
        }
        break;
      }

      if (meRes.status === 401) {
        const refreshed = await refreshAccessToken();
        if (!refreshed.ok) {
          if (refreshed.reason === 'expired' || refreshed.reason === 'missing') return;
          if (recoverFromServerGlitch()) return;
          setUser(null);
          setAuthStatus('guest');
          setAuthMessage('Sessiyani tekshirib bo\'lmadi. Qayta urinib ko\'ring.');
          return;
        }
        meRes = await callMe(refreshed.access);
      }

      if (meRes.ok) {
        const data = await meRes.json();
        const u = data?.user || null;
        setUser(u);
        const persistSnap =
          typeof localStorage !== 'undefined' && localStorage.getItem(REMEMBER_FLAG_KEY) === '1';
        if (u) writeUserSnapshot(u, persistSnap);
        else writeUserSnapshot(null);
        setAuthStatus(u ? 'authenticated' : 'guest');
        setAuthMessage(u ? '' : 'Kirish qilinmagan. Kirish tugmasini bosing.');
        return;
      }

      if (meRes.status === 403) {
        setUser(null);
        writeUserSnapshot(null);
        setAuthStatus('forbidden');
        setAuthMessage('Ruxsat yo\'q.');
        return;
      }

      if (meRes.status === 429 || meRes.status >= 500) {
        if (recoverFromServerGlitch()) return;
        setUser(null);
        setAuthStatus('guest');
        setAuthMessage('Server vaqtincha band. Qayta urinib ko\'ring.');
        return;
      }

      clearSession('expired', 'Sessiya tugagan. Qayta kiring.');
    } catch {
      if (recoverFromServerGlitch()) return;
      setUser(null);
      setAuthStatus('guest');
      setAuthMessage('Tarmoq xatosi. Internetni tekshirib qayta urinib ko\'ring.');
    }
  }, [refreshAccessToken, clearSession]);

  useEffect(() => {
    bootstrapSession();
  }, [bootstrapSession]);

  const request = useCallback(async (path, options = {}) => {
    const headers = { ...options.headers };
    const isFormData = typeof FormData !== 'undefined' && options.body instanceof FormData;
    if (!isFormData && !headers['Content-Type']) {
      headers['Content-Type'] = 'application/json';
    }

    const token = accessTokenRef.current || accessToken || readToken('accessToken');
    if (token) headers.Authorization = `Bearer ${token}`;

    if (APP_SECRET_KEY && !headers['X-App-Secret-Key']) {
      headers['X-App-Secret-Key'] = APP_SECRET_KEY;
    }

    let res;
    try {
      res = await fetch(`${API}${path}`, { ...options, headers, credentials: 'include' });
    } catch (err) {
      setAuthMessage('Tarmoq xatosi. Internetni tekshirib qayta urinib ko\'ring.');
      throw err;
    }

    if (res.status !== 401) return res;

    const refreshed = await refreshAccessToken();
    if (!refreshed.ok && refreshed.reason === 'missing') {
      clearSession('expired', 'Sessiya tugagan. Qayta kiring.');
      return res;
    }
    if (!refreshed.ok) return res;

    const retryHeaders = { ...headers, Authorization: `Bearer ${refreshed.access}` };
    return fetch(`${API}${path}`, { ...options, headers: retryHeaders, credentials: 'include' });
  }, [accessToken, clearSession, refreshAccessToken]);

  const login = async (emailOrLogin, password, opts = {}) => {
    const persistLocal = Boolean(opts?.rememberDevice);
    let res;
    try {
      res = await fetch(`${API}/auth/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          identifier: String(emailOrLogin || '').trim(),
          email: String(emailOrLogin || '').trim(),
          login: String(emailOrLogin || '').trim(),
          password: String(password || '').trim(),
          remember_device: persistLocal,
        }),
        credentials: 'include',
      });
    } catch (e) {
      throw mapFetchFailure(e);
    }
    const text = await res.text();
    const data = parseResponseJson(text);
    if (data === null) {
      const low = String(text).slice(0, 120).toLowerCase();
      if (low.includes('<!doctype') || low.includes('<html')) {
        throw new Error(
          "Server HTML sahifa qaytardi — odatda API ishlamayapti yoki /api proxy sozlanmagan. Backend ni ishga tushiring.",
        );
      }
      if (!res.ok) throw new Error(extractHttpErrorMessage(res, {}));
      throw new Error('Server javobi noto\'g\'ri.');
    }
    if (!res.ok) {
      throw new Error(extractHttpErrorMessage(res, data));
    }

    const access = data.access ?? data.accessToken;
    if (!access) throw new Error('Server token qaytarmadi. Qayta urinib ko\'ring.');
    if (!data.user) throw new Error('Server foydalanuvchi ma\'lumotini qaytarmadi.');

    persistTokens(data, { persistLocal });
    setUser(data.user);
    writeUserSnapshot(data.user, persistLocal);
    try {
      if (persistLocal) localStorage.setItem(REMEMBER_FLAG_KEY, '1');
      else localStorage.removeItem(REMEMBER_FLAG_KEY);
    } catch {}
    setAuthStatus('authenticated');
    setAuthMessage('');
    return data;
  };

  const register = async (email, password, full_name, opts = {}) => {
    const persistLocal = Boolean(opts?.rememberDevice);
    let res;
    try {
      res = await fetch(`${API}/auth/register`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email,
          password,
          full_name,
          remember_device: persistLocal,
          referrer_id: opts?.referrerId ?? opts?.referrer_id ?? null,
        }),
        credentials: 'include',
      });
    } catch (e) {
      throw mapFetchFailure(e);
    }
    const text = await res.text();
    const data = parseResponseJson(text);
    if (data === null) {
      const low = String(text).slice(0, 120).toLowerCase();
      if (low.includes('<!doctype') || low.includes('<html')) {
        throw new Error("Server HTML qaytardi. Backend va /api sozlamalarini tekshiring.");
      }
      if (!res.ok) throw new Error(extractHttpErrorMessage(res, {}));
      throw new Error('Server javobi noto\'g\'ri.');
    }
    if (!res.ok) {
      throw new Error(extractHttpErrorMessage(res, data));
    }

    persistTokens(data, { persistLocal });
    setUser(data.user);
    writeUserSnapshot(data.user, persistLocal);
    try {
      if (persistLocal) localStorage.setItem(REMEMBER_FLAG_KEY, '1');
      else localStorage.removeItem(REMEMBER_FLAG_KEY);
    } catch {}
    setAuthStatus('authenticated');
    setAuthMessage('');
    return data;
  };

  const socialAuth = async (provider, payload, opts = {}) => {
    const persistLocal = Boolean(opts?.rememberDevice);
    let res;
    try {
      res = await fetch(`${API}/auth/oauth/${provider}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...payload, remember_device: persistLocal }),
        credentials: 'include',
      });
    } catch (e) {
      throw mapFetchFailure(e);
    }
    const text = await res.text();
    const data = parseResponseJson(text);
    if (data === null) {
      if (!res.ok) throw new Error(extractHttpErrorMessage(res, {}));
      throw new Error('Server javobi noto\'g\'ri.');
    }
    if (!res.ok) {
      throw new Error(extractHttpErrorMessage(res, data));
    }

    persistTokens(data, { persistLocal });
    setUser(data.user);
    writeUserSnapshot(data.user, persistLocal);
    try {
      if (persistLocal) localStorage.setItem(REMEMBER_FLAG_KEY, '1');
      else localStorage.removeItem(REMEMBER_FLAG_KEY);
    } catch {}
    setAuthStatus('authenticated');
    setAuthMessage('');
    return data;
  };

  const logout = () => {
    fetch(`${API}/auth/logout`, { method: 'POST', credentials: 'include' }).finally(() => {
      clearSession('guest', '');
    });
  };

  const requestPasswordReset = async (identifier) => {
    let res;
    try {
      res = await fetch(`${API}/auth/forgot-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ identifier: String(identifier || '').trim() }),
      });
    } catch (e) {
      throw mapFetchFailure(e);
    }
    const text = await res.text();
    const data = parseResponseJson(text);
    if (data === null) {
      if (!res.ok) throw new Error(extractHttpErrorMessage(res, {}));
      throw new Error("Server javobi noto'g'ri.");
    }
    if (!res.ok) {
      throw new Error(extractHttpErrorMessage(res, data));
    }
    return data;
  };

  const resetPasswordWithToken = async (identifier, token, password) => {
    let res;
    try {
      res = await fetch(`${API}/auth/reset-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          identifier: String(identifier || '').trim(),
          token: String(token || '').trim(),
          password: String(password || '').trim(),
        }),
      });
    } catch (e) {
      throw mapFetchFailure(e);
    }
    const text = await res.text();
    const data = parseResponseJson(text);
    if (data === null) {
      if (!res.ok) throw new Error(extractHttpErrorMessage(res, {}));
      throw new Error("Server javobi noto'g'ri.");
    }
    if (!res.ok) {
      throw new Error(extractHttpErrorMessage(res, data));
    }
    return data;
  };

  const updateProfile = useCallback(
    async (body) => {
      const res = await request('/auth/me', {
        method: 'PATCH',
        body: JSON.stringify(body),
      });
      const text = await res.text();
      const data = parseResponseJson(text);
      if (!res.ok) {
        throw new Error(extractHttpErrorMessage(res, data === null ? {} : data));
      }
      if (data?.user) {
        setUser(data.user);
        const pl =
          typeof localStorage !== 'undefined' &&
          (localStorage.getItem(REMEMBER_FLAG_KEY) === '1' || !!localStorage.getItem('refreshToken'));
        writeUserSnapshot(data.user, pl);
        return data.user;
      }
      await bootstrapSession();
      return null;
    },
    [request, bootstrapSession]
  );

  return (
    <AuthContext.Provider
      value={{
        user,
        loading,
        authStatus,
        authMessage,
        login,
        register,
        socialAuth,
        logout,
        requestPasswordReset,
        resetPasswordWithToken,
        request,
        updateProfile,
        retrySession: bootstrapSession,
      }}
    >
      {children}
    </AuthContext.Provider>
  );
}

export function useAuth() {
  const ctx = useContext(AuthContext);
  if (!ctx) throw new Error('useAuth AuthProvider ichida ishlatilishi kerak');
  return ctx;
}
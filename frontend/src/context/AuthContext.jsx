...
function readUserSnapshot() {
  try {
    const raw = sessionStorage.getItem(USER_SNAPSHOT_KEY) || localStorage.getItem(USER_SNAPSHOT_KEY);
    if (!raw) return null;
  ...
  const register = async (email, password, full_name, opts = {}) => {
    ...
    try {
      res = await fetch(`${API}/auth/register`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, password, full_name, remember_device: persistLocal }),
        credentials: 'include',
      });
    } catch (e) {
      throw mapFetchFailure(e);

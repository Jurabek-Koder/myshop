...
export function LedgerQtyEditor({
  ...
  if (expandOnClick && compact && !readOnly && !expanded) {
    // XAVFSIZLIK/QULAYLIK: Chiqim allaqachon tasdiqlangan bo'lsa, katakchani
    // bosib "tahrirlash"ga o'xshagan holat ochilsa ham, aslida hech narsani
    // o'zgartirib bo'lmaydi (chiqimni ikki marta tasdiqlab bo'lmaydi) — bu
    // ayniqsa mobilda (hover/tooltip ko'rinmaydigan joyda) juda chalkash
    // bo'lardi. Shu sabab bunday holatda katakcha umuman BOSILMAYDIGAN,
    // faqat ma'lumot ko'rsatuvchi qilib qo'yiladi.
    const isLockedChiqim = field === 'warehouse_chiqim_qty' && Boolean(confirmedAt);
    if (isLockedChiqim) {
      return (
        <div
          className="warehouse-admin-ledger warehouse-admin-ledger--compact warehouse-admin-ledger--locked"
          title="Chiqim tasdiqlangan. O'zgartirish uchun jadval oxiridagi «⋮» tugmasidan «Tasdiq bekor»ni tanlang."
        >
          <span className="warehouse-admin-ledger-readonly-qty">{fv}</span>
          <span className={confirmOkClass} title="Tasdiqlangan">
            ✓
          </span>
          <span className="warehouse-admin-ledger-locked-hint">Tasdiq bekor: «⋮»</span>
        </div>
      );
    }
    return (
      ...
      >
        <div className="warehouse-admin-actions-chiqim-overlay-inner">
          <button
            ...
            title={
              p.warehouse_chiqim_confirmed_at
                ? 'Chiqim allaqachon tasdiqlangan. O‘zgartirish uchun avval "Tasdiq bekor" tugmasini bosing.'
                : undefined
            }

...
function resolveTeamChatRoom(teamChatRoom) {
  const room = String(teamChatRoom || 'operators').trim();
  if (room === 'sklad' || room === 'target') return room;
  return 'operators';
}
...
export default function PickerLichka({
  ...
  apiPrefix = '/picker',
  /** MyShop jamoa tredi: `operators`, `sklad` yoki `target` */
  ...
  /** Target panel: tashqi pastki menyu (Kabinet, Market…) — ichki chat dock yashiriladi */
  hideMobileChatDock = false,
  ...
  const { startDmCall, isOnline: isCallPeerOnline } = useCall();
  const isTargetChatApi = apiBase === '/target';
  const resolvedTeamRoom = useMemo(() => resolveTeamChatRoom(teamChatRoom), [teamChatRoom]);
  const teamRoomQ = `&teamRoom=${encodeURIComponent(resolvedTeamRoom)}`;
  ...
  const { notificationsEnabled, setNotificationsEnabled, locale, setLocale, notifTone, setNotifTone } =
    usePickerUiSettings();
  const [systemPermVersion, setSystemPermVersion] = useState(0);
  const refreshSystemPerm = useCallback(() => setSystemPermVersion((v) => v + 1), []);
  const systemNotifPermission = useMemo(() => {
    void systemPermVersion;
      ...
      if (request && activePeer?.id) {
        ...
        void request(`${apiBase}/dm/send`, {
          ...
          body: JSON.stringify({
            ...
            text,
            payload,
            ...(peerParam === 'myshop' ? { teamRoom: resolvedTeamRoom } : {}),
          }),
        });
      }
    },
    [threadKey, pickerChatNick, setThreads, request, activePeer?.id, t, scrollDmToBottom, apiBase, resolvedTeamRoom]
  );
  ...
  }, [activePeer?.id, request, setThreads, skladPurgedRef, apiBase, teamRoomQ, documentVisible]);
  ...
  /** Yangi DM xabarlar — qurilma tizim bildirishnomasi (ruxsat berilgan bo‘lsa). */
  const seenIncomingByThreadRef = useRef({});
  useEffect(() => {
    if (!notificationsEnabled) return;
    const activeKey =
      activePeer?.id === 'myshop' ? 'myshop' : activePeer?.id != null ? String(activePeer.id) : '';
    const threadsObj = threads && typeof threads === 'object' ? threads : {};

    for (const [key, list] of Object.entries(threadsObj)) {
      const arr = Array.isArray(list) ? list : [];
      const incomingMsgs = arr.filter((m) => !pickerMsgIsOutgoing(m));

      let prevSet = seenIncomingByThreadRef.current[key];
      if (!prevSet) {
        prevSet = new Set();
        for (const m of incomingMsgs) {
          if (m?.id != null) prevSet.add(String(m.id));
        }
        seenIncomingByThreadRef.current[key] = prevSet;
        continue;
      }
      ...
      for (const m of incomingMsgs) {
        ...
        const suppress =
          typeof document !== 'undefined' && !document.hidden && hasFocus && activeKey === key;
        if (suppress) continue;

        const peerLabel =
          key === 'myshop'
            ? t.chatTeam || 'MyShop'
            : peers.find((q) => String(q.id) === key)?.displayName?.trim() || t.lichkaTitle || 'Lichka';
        showIncomingChatSystemNotification({
          title: peerLabel,
          body: dmSnippet(m, t),
          tag: `dm-${key}-${id}`,
  ...
  }, [threads, notificationsEnabled, activePeer?.id, peers, t]);
  ...
  }, [canUseTeamStories, apiBase, request, resolvedTeamRoom]);
  ...
  useEffect(() => {
    if (!canUseTeamStories || !documentVisible) return undefined;
    const id = window.setInterval(() => {
      if (document.hidden) return;
      void loadStaffStories();
    }, 45000);
    return () => clearInterval(id);
  }, [canUseTeamStories, loadStaffStories, documentVisible]);
  ...
  const handleStoryComplete = useCallback(
    ...
    [request, onAddStory, t, canUseTeamStories, resolvedTeamRoom, apiBase, pickerChatNick, loadStaffStories, staffUserId]
  );
  ...
  const logChatCallAndDial = useCallback(
    async (peer, mode = 'voice') => {
      if (!peer?.id) return;
      if (peer.id === 'myshop') {
        setDmToast('Bu maxsus kontaktga qo‘ng‘iroq qilib bo‘lmaydi.');
        window.setTimeout(() => setDmToast(''), 2800);
        return;
      }
      startDmCall({ id: peer.id, displayName: peer.displayName }, mode === 'video' ? 'video' : 'voice');
    },
    [startDmCall, isCallPeerOnline]
  );
  ...
  const loadProfileSettings = useCallback(async () => {
    ...
    setProfileError('');
    try {
      const res = await request(isTargetChatApi ? `${apiBase}/settings` : `${apiBase}/profile`);
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  }, [request, apiBase, isTargetChatApi]);
  ...
  const onPickProfileImage = useCallback(async (file) => {
    if (!file) return;
    ...
    if (file.size > 8 * 1024 * 1024) {
      setProfileError('Rasm hajmi 8 MB dan oshmasin.');
      return;
    }
    try {
      const url = await compressAvatarImage(file);
      setProfileForm((prev) => ({ ...prev, avatar_url: url }));
      setSelfAvatarUrl(url);
      setProfileError('');
    } catch (e) {
      setProfileError(e.message || 'Rasmni o‘qib bo‘lmadi.');
    }
  }, []);
  ...
  const saveProfileSettings = useCallback(async () => {
    ...
    try {
      if (isTargetChatApi) {
        const settingsRes = await request(`${apiBase}/settings`, {
          method: 'PATCH',
          body: JSON.stringify({
            first_name: first,
            last_name: last,
        ...
        if (!settingsRes.ok) throw new Error(settingsData.error || `HTTP ${settingsRes.status}`);

        const contactRes = await request(`${apiBase}/settings/contact`, {
          method: 'PATCH',
          body: JSON.stringify({ phone }),
        });
        const contactData = await contactRes.json().catch(() => ({}));
        if (!contactRes.ok) throw new Error(contactData.error || `HTTP ${contactRes.status}`);

        if (avatar) {
          const avatarRes = await request(`${apiBase}/settings/avatar`, {
            method: 'PATCH',
            body: JSON.stringify({ avatar_url: avatar }),
          });
          const avatarData = await avatarRes.json().catch(() => ({}));
          if (!avatarRes.ok) throw new Error(avatarData.error || `HTTP ${avatarRes.status}`);
          if (avatarData.avatar_url !== undefined) {
            const savedAvatar = String(avatarData.avatar_url || '').trim();
            setSelfAvatarUrl(savedAvatar);
            setProfileForm((prev) => ({ ...prev, avatar_url: savedAvatar }));
          }
        }
      } else {
  ...
  }, [profileForm, request, apiBase, isTargetChatApi, t.profileSaved]);
        ...
        <div className="picker-tg-scroll" ref={listRef}>
          ...
          {visibleMessages.length === 0 ? (
            ...
            visibleMessages.map((m) => {
              ...
              if (m.type === 'call_log') {
                const missed = ['missed_offline', 'no_answer', 'declined', 'cancelled'].includes(m.callStatus);
                const icon = m.callMode === 'video' ? '📹' : '📞';
                const mm = Math.floor((m.callDurationSeconds || 0) / 60);
                const ss = (m.callDurationSeconds || 0) % 60;
                const durText = m.callDurationSeconds ? `${mm}:${String(ss).padStart(2, '0')}` : '';
                const statusText = {
                  ended: durText ? `Suhbat davomiyligi ${durText}` : 'Tugadi',
                  missed_offline: 'Javob berilmadi (oflayn)',
                  no_answer: 'Javob berilmadi',
                  declined: 'Rad etildi',
                  cancelled: 'Bekor qilindi',
                }[m.callStatus] || '';
                return (
                  <div key={m.id} className="picker-tg-row picker-tg-row--call-log">
                    <div className={`chat-groups-call-log${missed ? ' chat-groups-call-log--missed' : ''}`}>
                      ...
                      <span className="chat-groups-call-log-text">
                        <span className={`chat-groups-call-log-title${missed ? ' chat-groups-call-log-title--missed' : ''}`}>
                          {isOut ? 'Chiquvchi' : 'Kiruvchi'} {m.callMode === 'video' ? 'video qo‘ng‘iroq' : 'qo‘ng‘iroq'}
                        </span>
                ...
                <section className="picker-lichka-settings-page" aria-label={dmListCenterTitle}>
                  ...
                  {dmSettingsPage === 'notifications' ? (
                    <>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (next && systemNotificationsSupported() && Notification.permission === 'default') {
                            void requestSystemNotificationPermission().finally(() => refreshSystemPerm());
                          }
                        }}
                      >
                      ...
                      {systemNotificationsSupported() ? (
                        <div className="picker-lichka-settings-system-notif">
                          <p className="picker-lichka-settings-page-hint">{t.dmSystemNotifHint || ''}</p>
                          {systemNotifPermission === 'default' ? (
                            ...
                            >
                              {t.dmSystemNotifRequest || 'Tizim bildirishnomasi'}
                            </button>
                          ) : null}
                          {systemNotifPermission === 'denied' ? (
                            <p className="picker-lichka-settings-page-hint picker-lichka-settings-page-hint--warn">
                              {t.dmSystemNotifDenied || ''}
                            </p>
                          ) : null}
                          {systemNotifPermission === 'granted' ? (
                            <p className="picker-lichka-settings-page-hint picker-lichka-settings-ok">
                              {t.dmSystemNotifGranted || ''}
                            </p>
                          ) : null}
                        </div>
                      ) : null}

...
import { roleDefaultPath } from '../utils/authRedirect.js';
...
export default function AuthModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let data;
      if (tab === 'login') {
        data = await login(email, password, { rememberDevice });
      } else {
        data = await register(email, password, full_name, { rememberDevice });
      }
      onSuccess?.();
      onClose?.();

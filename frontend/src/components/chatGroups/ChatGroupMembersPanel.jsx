 ...
 */
export default function ChatGroupMembersPanel({ group, onClose, onMessageUser, currentUserId }) {
                ...
                {members.map((m) => (
                  <li key={m.user_id} className="chat-groups-member-row">
                    ...
                    <div className="chat-groups-member-actions">
                      {onMessageUser && Number(m.user_id) !== Number(currentUserId) ? (
                        <button
                          type="button"
                          className="chat-groups-chip-btn chat-groups-chip-btn--message"
                          title={`${m.full_name || m.login}ga shaxsiy xabar`}
                          onClick={() =>
                            onMessageUser({
                              id: m.user_id,
                              displayName: m.full_name || m.login || `#${m.user_id}`,

...
export default function OperatorDashboard() {
  ...
  useEffect(() => {
    if (filter !== 'finance') return undefined;
    let cancelled = false;
    ...
    (async () => {
      try {
        const res = await request('/operator/work-role/balance');
        const data = await res.json().catch(() => ({}));
        if (cancelled) return;
        if (res.status === 404 && data.code === 'no_work_role') {
          setOpWithdrawNoRole(true);
          setOpWithdrawBal(null);
          return;
        }
        if (res.ok) setOpWithdrawBal(Number(data.balance) || 0);
        else setOpWithdrawBal(null);
      } catch {
        if (!cancelled) setOpWithdrawBal(null);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const filterTitle = useMemo(() => {
    ...
    return '';
  }, [filter, sideNavItems, pickerUiT.settingsTitle]);
                ...
                <section className="picker-subpage">
                  ...
                  <div className="picker-profile-card">
                    {opProfileLoading ? (
                      ...
                      <form className="picker-profile-form" onSubmit={handleOpProfileSave}>
                        {opProfileError ? (
                        ...
                        <label className="picker-profile-field">
                          ...
                          <input type="email" className="picker-profile-input" value={opProfileForm.email} readOnly />
                        </label>
                        <label className="picker-profile-field">
                          <span className="picker-profile-label">{pickerUiT.profileLogin}</span>
                          <input type="text" className="picker-profile-input" value={opProfileForm.login} readOnly />
                        </label>
                        <label className="picker-profile-field">
                    ...
                    <form
                      ...
                      onSubmit={async (e) => {
                        ...
                        try {
                          ...
                          setOpWithdrawErr(false);
                          setOpWithdrawAmount('');
                          const bRes = await request('/operator/work-role/balance');
                          const j = await bRes.json().catch(() => ({}));
                          if (bRes.ok) setOpWithdrawBal(Number(j.balance) || 0);
                        } catch (err) {

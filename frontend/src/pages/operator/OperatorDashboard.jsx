...
function orderStatusLabel(status) {
  switch (String(status || '').toLowerCase()) {
    case 'pending':
      return "Omborda yig'ilmoqda";
    case 'hold':
      return 'Omborda kutmoqda';
    case 'picked':
      return 'Packerga o\'tkazildi';
    case 'packaged':
      return 'Qadoqlangan';
    case 'assigned':
...
const OPERATOR_ORDERS_EMPTY = {
  picker: 'Hozircha omborda yig\'ilayotgan zakaz yo\'q.',
  packer: 'Hozircha packer qo\'lidagi zakaz yo\'q.',
  ...
  returned: 'Hozircha atkaz qilingan zakaz yo\'q.',
};
...
/** Zakaz hozir qaysi rol qo'lida ekanini chek pastida ko'rsatish uchun */
function operatorOrderHolder(order) {
  const st = String(order?.status || '').toLowerCase();
  const packer = order?.packer;
  const courier = order?.courier;
  const packerTxt = packer?.id
    ? `Packer ID: ${packer.id}${packer.full_name ? ` · ${packer.full_name}` : ''}`
    : null;
  const courierTxt = courier?.id
    ? `Kuryer ID: ${courier.id}${courier.full_name ? ` · ${courier.full_name}` : ''}`
    : null;

  if (st === 'pending' || st === 'hold') {
    return { icon: '🏬', tone: 'picker', text: "Zakaz omborda yig'ilmoqda" };
  }
  if (st === 'picked') {
    return {
      icon: '📦',
      tone: 'packer',
      text: packerTxt || "Yig'ildi — packer hali tanlanmagan",
    };
  }
  if (st === 'packaged') {
    return {
      icon: '📦',
      tone: 'packer',
      text: packerTxt ? `Qadoqlandi · ${packerTxt}` : 'Qadoqlandi — kuryer kutilmoqda',
    };
  }
  if (st === 'assigned' || st === 'picked_up' || st === 'on_the_way') {
    return { icon: '🚚', tone: 'courier', text: courierTxt || 'Kuryerga biriktirilgan' };
  }
  if (st === 'delivered') {
    return { icon: '✅', tone: 'sold', text: courierTxt ? `Sotildi · ${courierTxt}` : 'Sotildi' };
  }
  if (st === 'left_at_home') {
    return { icon: '↩️', tone: 'returned', text: courierTxt ? `Atkaz · ${courierTxt}` : 'Atkaz' };
  }
  if (st === 'cancelled') {
    return {
      icon: '⛔',
      tone: 'cancelled',
      text: courierTxt ? `Bekor qilindi · ${courierTxt}` : 'Bekor qilindi',
    };
  }
  return null;
}

function OperatorOrdersReceipts({ orders, filter }) {
  if (!orders?.length) {
    return (
  ...
  return (
    <div className="yangi-list">
      {orders.map((order) => {
        const timeShort = getDateTimePartsInUzbekistan(order.status_updated_at || order.created_at)?.time || '—';
        const firstItem = order.items?.[0];
        const qty = (order.items || []).reduce((s, i) => s + (Number(i.quantity) || 0), 0) || 1;
        const holder = operatorOrderHolder(order);
        const phoneRaw = String(order.contact_phone || '').trim();
        const phoneHref = phoneRaw.replace(/[^\d+]/g, '');
        return (
          <article key={order.id} className="yangi-receipt">
            <header className="yangi-receipt-head">
              <span className="yangi-receipt-head-title">
                Zakaz #{order.id} · {orderStatusLabel(order.status)}
              </span>
              <span className="yangi-receipt-head-time">{timeShort}</span>
            </header>
            ...
            <div className="yangi-receipt-body">
              ...
              <div className="yangi-receipt-info">
                  ...
                  <div className="yangi-receipt-field">
                    ...
                    {phoneRaw ? (
                      <a
                        ...
                        href={`tel:${phoneHref}`}
                      >
                        {formatPendingPhoneDisplay(phoneRaw)}
                      </a>
                    ) : (
                      <span className="yangi-receipt-value yangi-receipt-value--phone">—</span>
                    )}
                ...
                <div className="yangi-receipt-row">
                  ...
                  <div className="yangi-receipt-field">
                    <span className="yangi-receipt-label">Manzil</span>
                    <span className="yangi-receipt-value">{order.shipping_address || '—'}</span>
                  </div>
                </div>
                ...
                <div className="yangi-receipt-row">
                  ...
                  <div className="yangi-receipt-field">
                    <span className="yangi-receipt-label">Mahsulot nomi</span>
                    <span className="yangi-receipt-value">{formatOrderProducts(order.items)}</span>
                  </div>
                </div>
            ...
            {holder ? (
              <footer className={`yangi-receipt-holder yangi-receipt-holder--${holder.tone}`}>
                <span aria-hidden>{holder.icon}</span>
                <span>{holder.text}</span>
              </footer>
            ) : null}
          </article>
        );
...
export default function OperatorDashboard() {
  ...
  const isOrdersFilter = ['picker', 'packer', 'packaged', 'courier', 'delivered', 'cancelled', 'returned'].includes(filter);
  ...
  const [manualTopic, setManualTopic] = useState('home');
  ...
  const loadData = async () => {
    if (isLichkaFilter || isLichkaManualFilter || isProfileFilter || isSettingsFilter) {
      setLoading(false);
      return;
    }
  ...
  const operatorManualTopics = useMemo(
    () => [
      { id: 'home', icon: '🏠', label: pickerUiT.navHome, title: 'Bosh sahifa', description: 'Operator panelining asosiy sahifasi. Bu yerda yangi buyurtmalar, statistikalar va tezkor harakatlar ko‘rsatiladi. Ish boshlash, umumiy vaziyatni kuzatish va kerakli bo‘limga o‘tish uchun foydalaniladi.' },
      { id: 'pending', icon: '📥', label: 'Yangi', title: 'Yangi buyurtmalar', description: 'Yangi kelgan buyurtmalar va leadlar ro‘yxati. Bu bo‘limda yangi topshiriqlarni ko‘rib chiqasiz va ularga tezkor javob berasiz.' },
      { id: 'create_lead', icon: '➕', label: 'Lead yaratish', title: 'Lead yaratish', description: 'Yangi lead qo‘shish uchun ishlatiladi. Mijoz ma‘lumotlarini, kontakt raqamini va xabarni kiritib, yangi potentsial xaridorni tizimga qo‘shishingiz mumkin.' },
      { id: 'contacted', icon: '🔗', label: "Bog'landim", title: 'Bog‘langan leadlar', description: 'Siz bilan bog‘langan leadlar ro‘yxati. Bu bo‘limda siz bog‘langan mijozlarni ko‘rib chiqasiz va ular bilan aloqani davom ettirishingiz mumkin.' },
      { id: 'picker', icon: '🏬', label: 'Pickerlar', title: 'Pickerlar', description: 'Pickerlar bo‘limi omborda mahsulotlarni yig‘ishda ishlaydigan xodimlar holatini ko‘rsatadi. Bu yerda ularning vazifalari va buyurtmalarni yig‘ish statusini tekshirishingiz mumkin.' },
      { id: 'packer', icon: '📦', label: 'Packerlar', title: 'Packerlar', description: 'Packerlar bo‘limi buyurtmalarni qadoqlash jarayonini boshqaradi. Bu bo‘lim orqali qadoqlangan buyurtmalar va packerlar holatini kuzatishingiz mumkin.' },
      { id: 'courier', icon: '🚚', label: 'Kuryerlar', title: 'Kuryerlar', description: 'Kuryerlar bo‘limi yetkazib berish jarayonini boshqaradi. Bu yerda yangilangan yetkazib berish statuslari va yetkazuvchi xodimlar ma‘lumotlarini topasiz.' },
      { id: 'delivered', icon: '✅', label: 'Sotildi', title: 'Sotildi', description: 'Sotilgan buyurtmalar bo‘limi. Bu yerda muvaffaqiyatli yetkazilgan buyurtmalar ro‘yxatini ko‘rishingiz mumkin.' },
      { id: 'cancelled', icon: '⛔', label: 'Bekor qilindi', title: 'Bekor qilingan buyurtmalar', description: 'Bekor qilingan buyurtmalar ro‘yxati. Bu bo‘lim yordamida bekor qilingan topshiriqlarni tekshirishingiz mumkin.' },
      { id: 'returned', icon: '↩️', label: 'Atkaz', title: 'Atkaz', description: 'Atkaz qilingan buyurtmalar bo‘limi. Bu yerda mahsulot qaytarilgan yoki yetkazib berilmagan buyurtmalar haqida ma‘lumot olasiz.' },
      { id: 'archived_orders', icon: '🗄️', label: 'Arxiv zakazlar', title: 'Arxiv zakazlar', description: 'Arxivdagi buyurtmalar. Bu bo‘limda eski yoki saqlangan buyurtmalarni ko‘rib chiqishingiz mumkin.' },
      { id: 'konkurs', icon: '🏆', label: 'Konkurs', title: 'Konkurs', description: 'Konkurs bo‘limi operatorlar uchun raqobat va yutuqlarni ko‘rsatadi. Bu yerda faoliyat natijalari va reytinglar bilan tanishishingiz mumkin.' },
  ...
  const filterTitle = useMemo(() => {
    ...
    if (filter === 'lichka_manual') return 'Qollanma';
        ...
        <main className={`picker-main operator-picker-main${mainTelegramLayout ? ' picker-main--telegram' : ''}`}>
          ...
          {isLichkaFilter ? (
            <div className="chat-groups-inline-wrap">
              <ChatSectionTabs active={chatSubTab} onChange={setChatSubTab} groupLabel="Operatorlar guruhi" />
              {chatSubTab === 'guruh' ? (
                <ChatGroupsScreen
                  onMessageUser={(peer) => {
                    setDmActivePeer(peer);
          ...
          ) : isLichkaManualFilter ? (
            <section className="picker-subpage operator-manual-page">
              <h1 className="picker-title">Qollanma</h1>
              <p className="picker-profile-intro">Operator panelidagi bo‘limlar va ularning vazifalari haqida tezkor ko‘rsatma.</p>
              <div className="operator-manual-layout">
                <div className="operator-manual-list">
                  {operatorManualTopics.map((topic) => (
                    <button
                      key={topic.id}
                      type="button"
                      className={`operator-manual-topic-button ${manualTopic === topic.id ? 'operator-manual-topic-button--active' : ''}`}
                      onClick={() => setManualTopic(topic.id)}
                    >
                      <span className="operator-manual-topic-icon" aria-hidden>
                        {topic.icon}
                      </span>
                ...
                <div className="operator-manual-detail">
                  {operatorManualTopics
                    .filter((topic) => topic.id === manualTopic)
                    .map((topic) => (
                      <div key={topic.id} className="operator-manual-detail-card">
                        <h2>{topic.title}</h2>
            ...
            <>
              ...
              {!isCreateLeadFilter && !isLichkaFilter && !loading && isOrdersFilter ? (
                <div className="operator-leads operator-leads--yangi">
                  <OperatorOrdersReceipts orders={orders} filter={filter} />
                </div>
              ) : null}
                    ...
                    <OperatorPendingCards
                      ...
                      onAccept={(row) => {
                        if (row.rowType === 'order') {
                          ...
                          // Yangi (pending) lead avval «Bog'langan» bo'ladi, keyin tasdiqlash mumkin
                          if (row.lead.status === 'pending') handleStatus(row.lead.id, 'contacted');
                          else handleConfirmLead(row.lead.id);
                        }
                      }}
                      onCancel={(row) => {
                        if (row.rowType === 'order') handleCancelPendingOrder(row.orderId);
                        else if (row.lead) handleStatus(row.lead.id, 'cancelled');
                      }}
                    />
                  ) : leads.length === 0 ? (
                    <div className="picker-empty operator-leads-empty">
        ...
        <div className="operator-modal-overlay" onClick={() => setDetailModal(null)}>
          <div className="operator-modal operator-modal-detail" onClick={(e) => e.stopPropagation()}>
            <div className="operator-modal-header">
              <div>
                <span className="operator-modal-tag">Yangi chek</span>
                ...
                <>
                  <div className="operator-detail-grid">
                    <div className="operator-detail-card">
                      <div className="operator-detail-card-heading">
                        <h5>Mijoz</h5>
                        <span className="operator-detail-badge">{detailModal.lead.status}</span>
                      </div>
                      <div className="operator-detail-row">
                        <span>Ism</span>
                        <span>{detailModal.lead.full_name || '—'}</span>
                      </div>
                      <div className="operator-detail-row">
                        <span>Telefon</span>
                        <span>
                      ...
                      <div className="operator-detail-row">
                        <span>Vaqt</span>
                        <span>{formatDateTime(detailModal.lead.created_at)}</span>
                      </div>
                    </div>
                    <div className="operator-detail-card">
                      <div className="operator-detail-card-heading">
                        <h5>Seller kiritgan mahsulot</h5>
                      </div>
                      {detailModal.lead.product.image_url && (
                        <div className="operator-detail-product-image">
                          <img src={detailModal.lead.product.image_url} alt={detailModal.lead.product.name_uz || 'Mahsulot rasmi'} />
                        </div>
                      )}
                      <div className="operator-detail-row">
                        <span>Nomi (UZ)</span>
                        <span>{detailModal.lead.product.name_uz || '—'}</span>
                      </div>
                      {detailModal.lead.product.name_ru && (
                        <div className="operator-detail-row">
                          <span>Nomi (RU)</span>
                          <span>{detailModal.lead.product.name_ru}</span>
                        </div>
                      )}
                      <div className="operator-detail-row">
                        <span>Narx</span>
                        <span>{formatCurrency(detailModal.lead.product.price)}</span>
                      </div>
                      {detailModal.lead.product.category && (
                        <div className="operator-detail-row">
                          <span>Kategoriya</span>
                          <span>{detailModal.lead.product.category}</span>
                        </div>
                      )}
                      <div className="operator-detail-row">
                        <span>Ombordagi</span>
                        <span>{detailModal.lead.product.stock ?? 0} ta</span>
                      </div>
                      {detailModal.lead.product.description_uz && (
                        <div className="operator-detail-row operator-detail-row--stacked">
                          <span>Tarif</span>
                          <span>{detailModal.lead.product.description_uz}</span>
                        </div>
                      )}
                    </div>
                  </div>

...
function formatShippingLocation(address) {
  const raw = String(address || '').trim();
  if (!raw) return '—';
  const parts = raw.split(',').map((item) => item.trim()).filter(Boolean);
  return parts.length ? parts.slice(0, 3).join(' / ') : raw;
}

function parseAddressAndComment(fullAddress) {
  const raw = String(fullAddress || '').trim();
  if (!raw) return { address: '—', comment: '—' };
  
  const separatorMatch = raw.match(/\.\s*(Sharh|Izoh):\s*/i);
  if (separatorMatch) {
    const splitIndex = separatorMatch.index;
    const address = raw.slice(0, splitIndex).trim();
    const comment = raw.slice(splitIndex + separatorMatch[0].length).trim();
    return { address: address || '—', comment: comment || '—' };
  }
  return { address: raw, comment: '—' };
}
...
export default function OperatorDashboard() {
  ...
  const isProductDetailView = filter === 'home' && productId !== null;
  ...
  const counts = useMemo(() => {
    const list = productOrders?.orders || [];
    let pending = 0;
    let packaged = 0;
    let delivered = 0;
    let on_the_way = 0;
    ...
    for (const o of list) {
      ...
      else if (s === 'packaged') packaged++;
      else if (s === 'delivered') delivered++;
      else if (['on_the_way', 'assigned', 'picked_up'].includes(s)) on_the_way++;
      else if (s === 'cancelled') cancelled++;
    }
    return { pending, packaged, delivered, on_the_way, cancelled };
  }, [productOrders]);
  ...
  const filteredProductOrders = useMemo(() => {
    ...
    return list.filter((o) => {
      const s = String(o.status || '').toLowerCase();
      if (detailTab === 'pending') return s === 'pending';
      if (detailTab === 'packaged') return s === 'packaged';
      if (detailTab === 'delivered') return s === 'delivered';
      if (detailTab === 'on_the_way') return ['on_the_way', 'assigned', 'picked_up'].includes(s);
      if (detailTab === 'cancelled') return s === 'cancelled';
      return false;
    });
  }, [productOrders, detailTab]);

  const handleRedirectToCreateLead = (order, parsedComment = '') => {
    const prodId = productOrders?.product?.id || productId || '';
    const { address } = parseAddressAndComment(order.shipping_address);
    setLeadCreateForm({
  ...
  const loadProductOrders = useCallback(
    async (id) => {
      if (!id) {
      ...
      setProductOrdersError('');
      try {
        const res = await request(`/operator/product-orders?product_id=${id}`);
        if (!res.ok) {
          throw new Error('Buyurtmalar yuklanmadi');
        }
        const data = await res.json();
        setProductOrders({
        ...
        setProductOrders(null);
        setProductOrdersError(err.message || "Buyurtmalar yuklanmadi");
      } finally {
  ...
  const openProductOrders = useCallback(
    (product) => {
      ...
      navigate(`/operator?tab=home&product_id=${product.id}`);
    },
    [navigate]
  );
  ...
  useEffect(() => {
    if (isProductDetailView) {
      ...
      void loadProductOrders(productId);
    } else {
      setProductOrders(null);
      setProductOrdersError('');
    }
  }, [isProductDetailView, productId, loadProductOrders]);
  ...
  const handleCreateLead = async (e) => {
    ...
    try {
      ...
      setLeadCreateForm({ product_id: '', full_name: '', contact_phone: '', contact_email: '', notes: '', shipping_address: '' });
      goNav('contacted');
    } catch (e) {
      setError(e.message || 'Xatolik');
    } finally {
          ...
          {isLichkaFilter ? (
            ...
            <>
              ...
              {isCreateLeadFilter && !loading ? (
                <form className="operator-lead-create-form" onSubmit={handleCreateLead}>
                  ...
                  <div className="operator-form-group">
                    ...
                    <input
                      type="text"
                      value={leadCreateForm.shipping_address}
                      onChange={(e) => setLeadCreateForm((f) => ({ ...f, shipping_address: e.target.value }))}
                      placeholder="Mijoz manzili (ixtiyoriy)"
                    />
                  </div>
                  <div className="operator-form-group">
                    <label>Izoh</label>
              ...
              {!isCreateLeadFilter && !isLichkaFilter && !loading && isHomeFilter ? (
                isProductDetailView ? (
                  <section className="operator-home-catalog" aria-label="Mahsulot buyurtmalari">
                    <div className="operator-page-header">
                      <button
                        type="button"
                        className="operator-btn"
                        onClick={() => navigate(`/operator?tab=home`)}
                      >
                        ← Orqaga
                      </button>
                      <h2>{productOrders?.product?.name_uz || 'Mahsulot buyurtmalari'}</h2>
                    </div>
                    {productOrdersLoading ? (
                      ...
                      <div className="picker-error-inline operator-error" role="alert">
                        {productOrdersError}
                      </div>
                    ) : (
                      <>
                        <div className="operator-detail-tabs">
                          <button
                            type="button"
                            className={`operator-detail-tab-btn ${detailTab === 'pending' ? 'active' : ''}`}
                            onClick={() => setDetailTab('pending')}
                          >
                            Yangi zakazlar
                            <span className="operator-detail-tab-badge">{counts.pending}</span>
                          </button>
                          <button
                            type="button"
                            className={`operator-detail-tab-btn ${detailTab === 'packaged' ? 'active' : ''}`}
                            onClick={() => setDetailTab('packaged')}
                          >
                            Qadoqlangan zakazlar
                            <span className="operator-detail-tab-badge">{counts.packaged}</span>
                          </button>
                          <button
                            type="button"
                            className={`operator-detail-tab-btn ${detailTab === 'delivered' ? 'active' : ''}`}
                            onClick={() => setDetailTab('delivered')}
                          >
                            Sotilgan zakazlar
                            <span className="operator-detail-tab-badge">{counts.delivered}</span>
                          </button>
                          <button
                            type="button"
                            className={`operator-detail-tab-btn ${detailTab === 'on_the_way' ? 'active' : ''}`}
                            onClick={() => setDetailTab('on_the_way')}
                          >
                            Yo'ldagi zakazlar
                            <span className="operator-detail-tab-badge">{counts.on_the_way}</span>
                          </button>
                          <button
                            type="button"
                            className={`operator-detail-tab-btn ${detailTab === 'cancelled' ? 'active' : ''}`}
                            onClick={() => setDetailTab('cancelled')}
                          >
                            Bekor qilinganlar
                            <span className="operator-detail-tab-badge">{counts.cancelled}</span>
                          </button>
                        </div>

                        <div className="operator-detail-product">
                          {productOrders?.product?.image_url && (
                            <p>
                              <img src={productOrders.product.image_url} alt="" style={{ maxWidth: '100%', borderRadius: 8 }} />
                            </p>
                          )}
                          <p>
                            <strong>Nomi:</strong> {productOrders?.product?.name_uz || '—'}
                          </p>
                          <p>
                            <strong>Narx:</strong> {formatCurrency(productOrders?.product?.price)}
                          </p>
                          <p>
                            <strong>Ombordagi:</strong> {productOrders?.product?.stock ?? 0} ta
                          </p>
                          <p>
                            <strong>Buyurtma soni:</strong> {productOrders?.orders_count || 0}
                          </p>
                        </div>

                        {filteredProductOrders.length === 0 ? (
                          <p className="operator-detail-empty-orders">Bu statusda buyurtma yo‘q.</p>
                        ) : (
                          <div className="operator-product-orders-table-wrap">
                            <table className="operator-product-orders-table">
                              <thead>
                                <tr>
                                  <th>ID</th>
                                  <th>Miqdor</th>
                                  ...
                                  <th>Izoh</th>
                                  <th>Vaqt</th>
                                  {detailTab === 'pending' && <th>Amal</th>}
                                  {detailTab === 'packaged' && <th>Qadoqlovchi</th>}
                                </tr>
                              </thead>
                              <tbody>
                                {filteredProductOrders.map((order) => {
                                  const { address, comment } = parseAddressAndComment(order.shipping_address);
                                  return (
                                    <tr
                                      key={`${order.id}-${order.quantity}`}
                                      className={detailTab === 'pending' ? 'operator-row-clickable' : ''}
                                      onClick={() => {
                                        if (detailTab === 'pending') {
                                          handleRedirectToCreateLead(order, comment);
                                        }
                                      }}
                                      style={detailTab === 'pending' ? { cursor: 'pointer' } : {}}
                                    >
                                      <td>{order.id}</td>
                                      <td>{order.quantity}</td>
                                      <td>{formatCurrency(order.price_at_order)}</td>
                                      <td>{orderStatusLabel(order.status)}</td>
                                      <td>{order.customer_name || '—'}</td>
                                      <td>{order.contact_phone || '—'}</td>
                                      <td>{order.customer_email || '—'}</td>
                                      <td>{formatShippingLocation(address)}</td>
                                      <td>{comment}</td>
                                      <td>{formatDateTime(order.created_at)}</td>
                                      {detailTab === 'pending' && (
                                        <td>
                                          <span
                                            className="operator-table-action-link"
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              handleRedirectToCreateLead(order, comment);
                                            }}
                                      ...
                                      {detailTab === 'packaged' && (
                                        <td>
                                          {order.packer ? `${order.packer.full_name || '—'} (#${order.packer.id})` : '—'}
                                        </td>
                                      )}
                                    </tr>
                                  );
                        ...
                        return (
                          ...
                          >
                            <button
                              ...
                              onClick={() => !isDisabled && openProductOrders(p)}
                              disabled={isDisabled}
                            >
                              ...
                              <div className="product-card-body">
                                ...
                                <div className="operator-home-card-count">
                                  {Number(p.orders_count) || 0} zakaz
                                </div>
                              </div>
                            </button>
                          </article>
                          ...
                          <div className="operator-lead-compact">
                            ...
                            <div className="operator-lead-actions">
                              <button
                                ...
                                onClick={() => {
                                  const { address, comment } = parseAddressAndComment(order.shipping_address);
                                  const prodId = order.items?.[0]?.product_id || '';
                                  setLeadCreateForm({
                                    product_id: prodId ? String(prodId) : '',
                                    full_name: order.customer_name || '',
                          ...
                          <>
                            <div className="operator-lead-body">
                              ...
                              {lead.shipping_address && (
                                <p>
                                  <strong>Manzil:</strong> {lead.shipping_address}
                                </p>
                              )}
                              {lead.notes && (
                                <p>
              ...
              {detailLoading ? (
                ...
                <>
                  <section className="operator-detail-lead">
                    ...
                    {detailModal.lead.shipping_address && (
                      <p>
                        <strong>Manzil:</strong> {detailModal.lead.shipping_address}
                      </p>
                    )}
                    {detailModal.lead.notes && (
                      <p>
                        <strong>Izoh:</strong> {detailModal.lead.notes}
                      </p>
                    )}
                  </section>

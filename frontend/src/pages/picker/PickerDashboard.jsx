    ...
    <article className="picker-card picker-history-product-block">
      ...
      <div className="picker-card-body">
        ...
        <ul className="picker-history-order-list">
          {group.orders.map((o) => {
            const extraItems = (o.items || []).filter((i) => i.product_id !== group.product_id);
            return (
              <li key={o.id} className="picker-history-order-entry">
                <div className="picker-history-order-head">
                  <span>{t.orderZakaz} #{o.id}</span>
                  <span className="picker-history-order-qty">
                    {o.item_quantity} {t.orderPcs}
                  </span>
                </div>
                {extraItems.length > 0 ? (
                  <ul className="picker-order-items picker-order-items--nested">
                    {extraItems.map((i) => {
                      const lineImg = resolvePickerProductImageUrl(i.image_url);
                      return (
                        <li key={i.id} className="picker-order-item-row">
                          <div className="picker-order-item-thumb">
                            {lineImg ? (
                              <img src={lineImg} alt="" loading="lazy" />
                            ) : (
                              <span className="picker-order-item-thumb-ph" aria-hidden />
                            )}
                          </div>
            ...
            <div className="picker-side-panel-footer">
              <div className="picker-side-panel-theme-row">
                <span className="picker-side-panel-theme-label">
                  <span
                    className={`picker-side-panel-theme-icon${theme === 'dark' ? ' picker-side-panel-theme-icon--moon' : ''}`}
                    aria-hidden
                  >
                    {theme === 'dark' ? '🌙' : '☀️'}
                  </span>
                  <span className="picker-side-panel-theme-text">
                    {theme === 'dark' ? pickerUiT.themeMoonLabel : pickerUiT.themeSunLabel}
                  </span>
                </span>
                <button
                  type="button"

...
export default function OrderReceiverDashboard() {
  ...
  const [staffNotificationDetail, setStaffNotificationDetail] = useState(null);
      ...
      <div className="packer-workspace">
        <header className="packer-sticky-topbar packer-sticky-topbar--expeditor">
          <div className="packer-sticky-topbar-title">
            ...
            {topMeta.showBrand ? <span className="packer-sticky-brand">Buyurtma qabul</span> : null}
          </div>
          <div className="expeditor-topbar-filters">
            <div className="expeditor-topbar-select-wrap">
              <label htmlFor="qabul-viloyat-select" className="packer-sr-only">
                Viloyat
              </label>
              <select
                id="qabul-viloyat-select"
            ...
            <div className="expeditor-topbar-select-wrap">
              <label htmlFor="qabul-courier-select" className="packer-sr-only">
                Kuryer
              </label>
              <select
                id="qabul-courier-select"
                className="packer-topbar-region-select"
          ...
          <div className="expeditor-topbar-trailing">
            <div className="packer-staff-topbar-actions expeditor-staff-topbar-actions">
              ...
              >
                {staffNotifOpen && (
                  <>
                    <div className="picker-bell-backdrop" onClick={() => setStaffNotifOpen(false)} aria-hidden="true" />
                    <div className="picker-bell-dropdown">
                      <StaffNotifModalHeader
                        t={STAFF_TOPBAR_T_UZ}
                        notificationsEnabled={staffNotifSoundOn}
                        setNotificationsEnabled={setStaffNotifSoundOn}
                      />
                      {staffNotifications.length === 0 ? (
                        <p className="picker-bell-empty">{STAFF_TOPBAR_T_UZ.bellEmpty}</p>
                      ) : (
                          ...
                          {staffNotifications.map((n) => (
                            <li key={n.id}>
                              <button
                                ...
                                onClick={() => {
                                  setStaffNotificationDetail(n);
                                  setStaffNotifOpen(false);
                                  if (!n.read_at) void markStaffNotificationRead(n.id);
                                }}
                              >
                                <span className="picker-bell-item-title">{n.title}</span>
                                <span className="picker-bell-item-date">
                                  {formatDateTimeUzCompact(n.created_at, { empty: '—' })}
                                </span>
                              </button>
                            </li>
                          ))}
              ...
              <StaffTopbarProfileMenu
                ...
                onHome={() => navigate('/')}
                onProfile={() => navigate('/qabul/profile')}
                onSettings={() => navigate('/qabul/profile')}
                onLogout={() => {
                  logout();
                  navigate('/login');
                }}
        ...
        {staffNotificationDetail ? (
          <div
            ...
            role="dialog"
            aria-modal="true"
            onClick={() => setStaffNotificationDetail(null)}
          >
            <div className="picker-modal" onClick={(e) => e.stopPropagation()}>
              <h2 className="picker-modal-title">{staffNotificationDetail.title}</h2>
              <p className="picker-notification-body">{staffNotificationDetail.body}</p>
        ...
        >
          <Routes>
            <Route
              ...
              element={
                <>
                  <OrderReceiverAtkazUnsoldSection
                    regionFilterId={regionFilterId}
                    courierStaffId={courierStaffId}
                    request={request}
                    refreshNonce={queueRefreshNonce}
                    onRecordWarehouseReceive={() => setAtkazWarehouseNonce((n) => n + 1)}
                  />
                  <ExpeditorHomeQueues
                    regionFilterId={regionFilterId}
                    courierStaffId={courierStaffId}

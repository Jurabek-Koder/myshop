...
const VIEW_KEYS = new Set(['home', 'kirim_chiqim', 'atkaz', 'brak', 'chat', 'delisted', 'deleted', 'profile', 'settings']);
...
export default function WarehouseAdminDashboard() {
  ...
  const handleChiqimLedgerSaveAndConfirm = useCallback(
    async (p, n) => {
      ...
      setConfirmingChiqimId(id);
      try {
        const patchRes = await request(`/warehouse-admin/products/${id}/warehouse-ledger`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ warehouse_chiqim_qty: n }),
        });
        const patchD = await patchRes.json().catch(() => ({}));
        if (!patchRes.ok) {
          alert(patchD?.error || 'Chiqim saqlanmadi');
          return;
        }
                        ...
                        pendingProducts.map((p) => (
                          <div key={p.id} className="warehouse-admin-sheet-row-group">
                            ...
                            <div
                              ...
                              role="gridcell"
                            >
                              <span className="warehouse-admin-num-muted" title="Kirimni tasdiqlashdan keyin">
                                —
                              </span>
                            </div>
                            <div
                            ...
                            >
                              <LedgerQtyEditor
                                ...
                                peerExclusiveExpandedProductId={chiqimLedgerOpen?.id ?? null}

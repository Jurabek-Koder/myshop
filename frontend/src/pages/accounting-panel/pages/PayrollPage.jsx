...
import SuperuserAuditSection from '../components/SuperuserAuditSection.jsx';
...
export default function PayrollPage() {
  ...
  return (
    <div className="ap-page">
      ...
      <SuperuserAuditSection />

      <section className="ap-panel">
        <div className="ap-panel-head-row">
          <div>
            <h3 style={{ margin: 0 }}>{uz.payroll.roleDefaults}</h3>
            <p className="ap-sub" style={{ marginTop: '0.5rem', marginBottom: 0 }}>
              {uz.payroll.roleDefaultsHint}
            </p>
          </div>
          <button type="button" className="ap-btn ap-btn--primary" onClick={() => void handleSync()} disabled={loading}>
            {uz.payroll.syncAllRoles}
          </button>
        </div>
        ...
        <div className="ap-table-wrap">
          <table className="ap-table ap-table--roles">
            <colgroup>
              <col style={{ width: '22%' }} />
              <col style={{ width: '24%' }} />
              <col style={{ width: '24%' }} />
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setRoleForm({
                          ...
                          advance_percent: rd.advance_percent ?? '0.1',
                          advance_due_day: String(rd.advance_due_day ?? 15),
                          balance_due_day: String(rd.balance_due_day ?? 0),
                        });
                      }}
                    >
                      {uz.payroll.editRoleSalary}
                    </button>
                  </td>
                </tr>
              ))}
          ...
          {!roleDefaults.length && !loading ? (
            <div className="ap-empty" style={{ padding: '1.25rem 1rem', textAlign: 'center' }}>
              <p style={{ margin: '0 0 1rem' }}>{uz.noData}</p>
              <button type="button" className="ap-btn ap-btn--primary" onClick={() => void handleSync()} disabled={loading}>
                {uz.payroll.syncAllRoles}
              </button>
            </div>
            ...
            <tbody>
              {employees.map((e) => (
                <tr key={e.id}>
                  ...
                  <td className="ap-center">{Math.round(Number(e.advance_percent) * 100)}%</td>
                  <td className="ap-col-action">
                    <button
                      type="button"
            ...
            <tbody>
              {cycles.map((c) => (
                <tr key={c.id}>
                  ...
                  <td className="ap-col-action ap-actions">
                    {c.status !== 'paid' ? (
                      <button
                        type="button"
          ...
          <form className="ap-modal" onSubmit={submitRoleDefault}>
            ...
            <div className="ap-form-stack">
              ...
              <div className="ap-field">
                ...
                />
                <span className="ap-sub" style={{ fontSize: '0.82rem' }}>
                  Masalan: 0.1 = 10% avans
                </span>
              </div>
              <div className="ap-field">

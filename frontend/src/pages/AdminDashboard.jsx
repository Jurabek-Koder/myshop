                          ...
                          <li key={n.id} className={n.read_at ? '' : 'unread'}>
                            <div className="admin-bell-item">
                              ...
                              {(n.link_type === 'seller_product_approved' ||
                                n.link_type === 'seller_product_warehouse_kirim') && (
                                <button
                                  type="button"
                              ...
                              {n.link_type === 'withdrawal_payout' && n.link_id && (
                                <div className="admin-bell-item-actions">
                                  <button
                                    ...
                                    disabled={adminNotifyApproveBusy === n.id}
                                    onClick={() => {
                                      handleAdminNotifyMarkPaid(n);
                                      setAdminNotificationsOpen(false);
                                    }}
                                  >
                                    {adminNotifyApproveBusy === n.id ? '...' : 'Pul berildi'}
                                  </button>
                                  <button
                                    type="button"
                                    className="btn-neo btn-sm"
          ...
          {activeView === 'staff_chat' ? (
            ...
            <>
              ...
              {activeView === 'admin_profile' && (
                <div className="panel-card admin-profile-panel">
                  <div className="panel-head">
              ...
              {(activeView === 'atkaz_products' || activeView === 'brak_products' || activeView === 'warehouse_kirim_chiqim') &&
                (() => {
                  if (activeView === 'warehouse_kirim_chiqim') {
                    ...
                    const kirimList = baseList.filter(
                      (p) => Boolean(p.warehouse_approved_at) && Number(p.kirim_soni) > 0,
                    );
                    const chiqimList = baseList.filter(
                      (p) =>
                        Boolean(p.warehouse_chiqim_confirmed_at) && Number(p.warehouse_chiqim_qty) > 0,
                    );
                    const rowEl = (p, qty, tone) => (
                      <div
                        key={`${p.id}-${tone}`}
                        style={{
                          display: 'flex',
                      ...
                      >
                        ...
                        <div style={{ fontWeight: 700, color: tone, minWidth: 48, textAlign: 'right' }}>{qty}</div>
                      </div>
                    );
                    return (
                      <div className="panel-card">
                        <div className="panel-head">
                          <h4>
                            ...
                            <span className="muted" style={{ fontWeight: 400 }}>
                              (kirim {kirimList.length} · chiqim {chiqimList.length})
                            </span>
                          </h4>
                        </div>
                        <p className="muted" style={{ padding: '0.25rem 0 0.75rem' }}>
                          Ombor boshqaruvchisi tasdiqlagan kirim va chiqim yozuvlari — bu yerda faqat ko‘rish.
                        </p>
                        {warehouseProductsLoading && <p className="muted">Yuklanmoqda…</p>}
                        {!warehouseProductsLoading && (
                          <>
                            <h5 className="muted" style={{ margin: '0.75rem 0 0.35rem', fontSize: '0.95rem' }}>
                              Tasdiqlangan kirim ({kirimList.length})
                            </h5>
                            {kirimList.length === 0 ? (
                              <p className="muted">Tasdiqlangan kirim yo‘q.</p>
                            ) : (
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                {kirimList.map((p) => rowEl(p, Number(p.kirim_soni) || 0, '#16a34a'))}
                              </div>
                            )}
                            <h5 className="muted" style={{ margin: '1rem 0 0.35rem', fontSize: '0.95rem' }}>
                              Tasdiqlangan chiqim ({chiqimList.length})
                            </h5>
                            {chiqimList.length === 0 ? (
                              <p className="muted">Tasdiqlangan chiqim yo‘q.</p>
                            ) : (
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                {chiqimList.map((p) =>
                                  rowEl(p, Number(p.warehouse_chiqim_qty) || 0, '#2563eb'),
                                )}
                              </div>
                            )}
                          </>

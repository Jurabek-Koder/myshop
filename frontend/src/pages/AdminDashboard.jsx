...
import AdminPayrollOversight from './admin/AdminPayrollOversight.jsx';
import { LedgerQtyEditor, WarehouseActionsColumn, WarehouseGridColumnHeaders, WarehouseChiqimConfirmedDuplicateRow, warehouseKirimChiqimSheetMainRowClass, formatWarehouseProductSumUz } from '../components/warehouse/WarehouseLedgerParts.jsx';
import { PACKER_UZ_VILOYATLAR, TOSHKENT_SH_TUMANS } from '../constants/uzViloyatlarPacker';
import ModalWrapper from '../components/ModalWrapper.jsx';
import StaffArchivedOrdersTable from '../components/staff/StaffArchivedOrdersTable.jsx';
import AdminAccountingMoliyaHub from './admin/AdminAccountingMoliyaHub.jsx';
import { connectEnterpriseRealtime } from '../utils/enterpriseRealtime.js';
import {
  formatDateTimeUz,
...
const MENU_GROUPS = [
  {
    ...
    items: [
      ...
      { key: 'warehouse_kirim_chiqim', icon: 'fa-right-left', label: 'Ombor kirim va chiqim' },
      { key: 'atkaz_products', icon: 'fa-ban', label: 'Atkaz mahsulot' },
      { key: 'brak_products', icon: 'fa-triangle-exclamation', label: 'Brak mahsulot' },
      { key: 'staff_chat', icon: 'fa-comments', label: 'Chat' },
      { key: 'chat_groups', icon: 'fa-people-group', label: 'Guruhlar' },
      { key: 'roles', icon: 'fa-user-tag', label: 'Rollar' },
      { key: 'roles_archive', icon: 'fa-box-archive', label: 'Rollar arxivi', badgeKey: 'deletedRoles' },
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'accounting', icon: 'fa-calculator', label: 'Buxgalteriya', badgeKey: 'awaitingPayoutWithdrawals' },
      ...
      { key: 'activity_log', icon: 'fa-history', label: 'Loyiha jurnali' },
...
const HIDDEN_VIEW_KEYS = ['seller_products', 'customer_detail', 'admin_profile', 'admin_settings', 'recent_users', 'roles_safe'];
...
const ADMIN_HEADER_EXTRA = {
  ...
  recent_users: { key: 'recent_users', label: "So'nggi harakatlar", icon: 'fa-user-plus' },
};
const DEFAULT_ADMIN_VIEW = 'dashboard';

const SAFE_MENU_GROUPS = [
  {
    title: 'Asosiy',
    items: [
      { key: 'umumiy', icon: 'fa-house', label: 'Umumiy' },
      { key: 'balans', icon: 'fa-wallet', label: 'Balans' },
      { key: 'kirim', icon: 'fa-arrow-down', label: 'Kirim' },
      { key: 'chiqim', icon: 'fa-arrow-up', label: 'Chiqim' },
      { key: 'tarix', icon: 'fa-clock-rotate-left', label: 'Tarix' },
      { key: 'sozlamalar', icon: 'fa-gear', label: 'Sozlamalar' },
    ],
  },
  {
    title: 'Arxiv rollar',
    items: [
      { key: 'archive_operator', icon: 'fa-headset', label: 'Operator', portalRole: 'operator' },
      { key: 'archive_expeditor', icon: 'fa-truck-fast', label: 'Ekspeditor', portalRole: 'expeditor' },
      { key: 'archive_order_receiver', icon: 'fa-inbox', label: 'Qabul qiluvchi', portalRole: 'order_receiver' },
      { key: 'archive_packer', icon: 'fa-box', label: 'Packer', portalRole: 'packer' },
      { key: 'archive_picker', icon: 'fa-hand-pointer', label: 'Picker', portalRole: 'picker' },
      { key: 'archive_courier', icon: 'fa-motorcycle', label: 'Kuryer', portalRole: 'courier' },
      { key: 'archive_target', icon: 'fa-bullseye', label: 'Target', portalRole: 'target' },
    ],
  },
];

const SAFE_MENU_FLAT = SAFE_MENU_GROUPS.flatMap((g) => g.items);

const SAFE_BALANS_TYPES = [
  { key: 'courier', icon: 'fa-motorcycle', label: 'Kuryer' },
  { key: 'operator', icon: 'fa-headset', label: 'Operator' },
  { key: 'expeditor', icon: 'fa-barcode', label: 'Ekspeditor' },
  { key: 'order_receiver', icon: 'fa-inbox', label: 'Qabul qiluvchi' },
  { key: 'target', icon: 'fa-bullseye', label: 'Target' },
  { key: 'seller', icon: 'fa-store', label: 'Seller' },
  { key: 'packer', icon: 'fa-box', label: 'Packer' },
  { key: 'picker', icon: 'fa-hand-pointer', label: 'Picker' },
  { key: 'warehouse_admin', icon: 'fa-warehouse', label: 'Ombor admini' },
];

function groupActivityByDay(list = []) {
  const map = new Map();
  for (const item of list) {
    const day = String(item.created_at || '').slice(0, 10);
    if (!day) continue;
    if (!map.has(day)) map.set(day, { day, kirim: 0, chiqim: 0, items: [] });
    const amt = Number(item.signed_amount) || 0;
    if (amt > 0) map.get(day).kirim += amt;
    else map.get(day).chiqim += Math.abs(amt);
    map.get(day).items.push(item);
  }
  return Array.from(map.values()).sort((a, b) => b.day.localeCompare(a.day));
}

function SafeActivityTable({ list, withDate = true }) {
  if (!list || list.length === 0) return <p className="muted">Faoliyat yo‘q.</p>;
  return (
    <table className="safe-activity-table">
      <thead>
        <tr>
          {withDate && <th>Sana</th>}
          <th>Vaqt</th>
          <th>Tur</th>
          <th>Tavsif</th>
          <th className="safe-activity-amt-col">Miqdor</th>
        </tr>
      </thead>
      <tbody>
        {list.map((t) => (
          <tr key={`${t.source}-${t.id}`}>
            {withDate && <td>{formatDate(t.created_at)}</td>}
            <td>{String(t.created_at || '').split(' ')[1]?.slice(0, 5) || '—'}</td>
            <td>{t.kind_label}</td>
            <td>{t.title || '—'}</td>
            <td className={`safe-activity-amt ${Number(t.signed_amount) >= 0 ? 'pos' : 'neg'}`}>
              {Number(t.signed_amount) >= 0 ? '+' : '−'}
              {formatCurrency(Math.abs(Number(t.signed_amount)))}
            </td>
          </tr>
        ))}
      </tbody>
...
function normalizeAdminView(value) {
  ...
  if (value === 'warehouse_kirim' || value === 'warehouse_chiqim') return 'warehouse_kirim_chiqim';
...
function matchesStaffSimpleSearch(row, debouncedRaw, emptyShowsAll = false) {
  const raw = String(debouncedRaw || '').trim();
  if (!raw) return Boolean(emptyShowsAll);
...
const ROLE_PORTAL_OPTIONS = [
  ...
  { value: 'target', label: 'Target paneli (affiliate / oqim)' },
  ...
  { value: 'warehouse_admin', label: 'Ombor admin paneli' },
  { value: 'accounting', label: 'Buxgalteriya paneli' },
];

/**
 * Hudud (viloyat) biriktiriladigan panel turlari.
 * Kuryerdan tashqari ekspeditor ham o'z hududi bo'yicha ishlaydi — aks holda
 * ekspeditor paneliga hech qanday kuryer/zakaz ko'rinmay qoladi (backend
 * validateRegionAccess shu maydonga tayanadi).
 */
const REGION_SCOPED_PORTAL_ROLES = new Set(['courier', 'expeditor']);
...
function badgeValue(item, stats, usersCount, pendingWithdrawalsCount, orderStats) {
  if (!stats && pendingWithdrawalsCount == null && !orderStats) return null;
  if (item.badgeKey === 'users') return usersCount || (stats && stats.users) || 0;
  if (item.badgeKey === 'orders') return orderStats?.total ?? stats?.orders ?? 0;
  if (item.badgeKey === 'pending') return orderStats ? orderStats.hold || 0 : Math.max(0, Math.round((stats?.orders || 0) * 0.14));
  if (item.badgeKey === 'atkaz') return orderStats?.cancelled ?? Math.max(0, Math.round((stats?.orders || 0) * 0.06));
  ...
  if (item.badgeKey === 'deletedRoles') return null;
  return null;
}
...
function AdminTwilioOperatorPanel({
  settings,
  authDraft,
  onAuthDraftChange,
  onChangeField,
  ...
  const hookUrl = settings.inbound_webhook_url || '';
  return (
    <div className="panel-card admin-twilio-panel" style={{ marginBottom: '1.25rem' }}>
      <div className="panel-head">
        <h4><i className="fab fa-twilio" aria-hidden /> Twilio SMS — ulanish</h4>
      </div>
      <p className="muted" style={{ marginTop: 0, fontSize: '0.92rem' }}>
        Twilio Console da raqamingiz uchun <strong>Messaging</strong> webhook ni quyidagi manzilga qo‘ying (POST). Kiruvchi SMS lar «Mijoz smslari» jadvaliga tushadi;
        operator yuborganlar «Rollardan kelgan smslar» da ko‘rinadi.
      </p>
      {hookUrl ? (
        <p className="admin-twilio-webhook-url">
          <strong>Webhook:</strong>{' '}
          <code className="admin-ai-target-code">{hookUrl}</code>
        </p>
      ) : (
        <p className="muted" style={{ fontSize: '0.88rem' }}>
          To‘liq HTTPS URL uchun serverda <code className="admin-ai-target-code">TWILIO_WEBHOOK_BASE_URL</code> yoki{' '}
          <code className="admin-ai-target-code">RENDER_EXTERNAL_URL</code> ni API domeningizga qo‘ying (Twilio imzo tekshiruvi uchun ham kerak).
        </p>
      )}
      <div className="admin-ai-target-manager-quick" style={{ marginTop: '1rem', flexWrap: 'wrap', gap: '0.75rem' }}>
        <label className="admin-ai-target-field admin-ai-target-field--compact" style={{ minWidth: 200 }}>
          ...
          <input
            ...
            onChange={(e) => onChangeField('account_sid', e.target.value)}
            placeholder="ACxxxxxxxx"
          />
        </label>
        <label className="admin-ai-target-field admin-ai-target-field--compact" style={{ minWidth: 200 }}>
          <span>Kimdan (Twilio raqam, E.164)</span>
          <input
            type="text"
            className="neo-input"
            autoComplete="off"
            value={settings.from_number}
            onChange={(e) => onChangeField('from_number', e.target.value)}
            placeholder="+998..."
          />
        </label>
      </div>
      <label className="admin-ai-target-field" style={{ marginTop: '0.75rem', display: 'block' }}>
        <span>
          Auth Token{' '}
          {settings.auth_token_configured ? <span className="muted">(saqlangan)</span> : null}
        </span>
        <input
          type="password"
          className="neo-input"
          ...
          placeholder={settings.auth_token_configured ? 'Yangi token (ixtiyoriy)' : 'Twilio Auth Token'}
        />
      </label>
      <label
        className="admin-twilio-relaxed"
        style={{
          marginTop: '0.65rem',
          display: 'flex',
          alignItems: 'center',
      ...
      >
        <input
          type="checkbox"
          checked={settings.signing_relaxed}
          onChange={(e) => onChangeField('signing_relaxed', e.target.checked)}
        />
        Rivojlantirish: webhook imzosini tekshirmaslik (faqat sinov)
      </label>
      {message ? (
        <p
          className={
            message === 'Saqlandi.'
              ? 'admin-ai-target-hero-status admin-ai-target-hero-status--ok'
              : 'admin-ai-target-hero-status admin-ai-target-hero-status--err'
          }
      ...
      <div className="admin-ai-target-hub-actions" style={{ marginTop: '1rem' }}>
        <button type="button" className="btn-neo btn-neo-primary" disabled={saving} onClick={onSave}>
          {saving ? 'Saqlanmoqda…' : 'Twilio sozlamalarini saqlash'}
        </button>
        <a className="btn-neo" href="https://console.twilio.com/" target="_blank" rel="noopener noreferrer">
          <i className="fas fa-external-link-alt" aria-hidden /> Twilio Console
        </a>
      </div>
      <hr className="admin-twilio-hr" />
      <h5 style={{ margin: '0 0 0.5rem', fontSize: '1rem' }}>Operator SMS yuborish</h5>
      <div className="admin-ai-target-manager-quick" style={{ flexWrap: 'wrap', gap: '0.75rem', alignItems: 'flex-end' }}>
        <label className="admin-ai-target-field admin-ai-target-field--compact" style={{ minWidth: 160 }}>
          <span>Telefon</span>
          <input
            type="text"
            className="neo-input"
        ...
        <label className="admin-ai-target-field" style={{ flex: '1 1 220px', minWidth: 200 }}>
          <span>Matn</span>
          <input
            type="text"
            className="neo-input"
            ...
            placeholder="Salom, buyurtmangiz..."
          />
        </label>
        <button type="button" className="btn-neo btn-neo-primary" disabled={sendBusy || !settings.auth_token_configured} onClick={onSend}>
          {sendBusy ? '…' : 'Yuborish'}
        </button>
      </div>
      {!settings.auth_token_configured ? (
        <p className="muted" style={{ fontSize: '0.85rem', marginTop: '0.5rem' }}>
          SMS yuborish uchun Auth Token saqlang (yoki server .env da TWILIO_AUTH_TOKEN).
        </p>
      ) : null}
    </div>
  );
...
export default function AdminDashboard() {
  ...
  const isSuperuser = String(user?.role || '').toLowerCase() === 'superuser' || user?.role_id === 1;
  ...
  const [recentUsersMsgUser, setRecentUsersMsgUser] = useState(null);
  const [recentUsersMessages, setRecentUsersMessages] = useState([]);
  const [recentUsersMsgLoading, setRecentUsersMsgLoading] = useState(false);
  const [recentUsersMsgTitle, setRecentUsersMsgTitle] = useState('');
  const [recentUsersMsgBody, setRecentUsersMsgBody] = useState('');
  const [recentUserDeleteNotice, setRecentUserDeleteNotice] = useState(null);
  const [deletionInsightsOpen, setDeletionInsightsOpen] = useState(false);
  const [deletionInsightsUser, setDeletionInsightsUser] = useState(null);
  const [deletionInsightsData, setDeletionInsightsData] = useState(null);
  const [deletionInsightsLoading, setDeletionInsightsLoading] = useState(false);
  const [insightDetailOpen, setInsightDetailOpen] = useState(false);
  const [insightDetailBlock, setInsightDetailBlock] = useState(null);
  const [insightDetailData, setInsightDetailData] = useState(null);
  const [insightDetailLoading, setInsightDetailLoading] = useState(false);
  const [insightDeleteConfirm, setInsightDeleteConfirm] = useState(null);
  const [insightDeleteBusy, setInsightDeleteBusy] = useState(false);
  ...
  const [warehouseProductsOverview, setWarehouseProductsOverview] = useState([]);
  ...
  const [settingsNewPwd2, setSettingsNewPwd2] = useState('');
  const [settingsShowPwd, setSettingsShowPwd] = useState(false);
  const [settingsBusy, setSettingsBusy] = useState(false);
  const [settingsMsg, setSettingsMsg] = useState({ type: '', text: '' });
  const [backupPanelBusy, setBackupPanelBusy] = useState(false);
  const [backupPanelMsg, setBackupPanelMsg] = useState({ type: '', text: '' });
  const [backupList, setBackupList] = useState([]);
  const [backupListLoading, setBackupListLoading] = useState(false);
  const [driveBackupStatus, setDriveBackupStatus] = useState(null);
  const [driveRemoteFiles, setDriveRemoteFiles] = useState([]);
  const [driveRemoteLoading, setDriveRemoteLoading] = useState(false);
  const [driveDeleteBusyId, setDriveDeleteBusyId] = useState(null);
  ...
  const [accountingMoliyaHubOpen, setAccountingMoliyaHubOpen] = useState(false);
  ...
  const [safeArchivedRoles, setSafeArchivedRoles] = useState([]);
  ...
  const [safePasswordSet, setSafePasswordSet] = useState(false);
  const [safeSubView, setSafeSubView] = useState('umumiy');
  const [safeOpen, setSafeOpen] = useState(false);
  const [safeBalansRoleId, setSafeBalansRoleId] = useState(null);
  const [safeBalansType, setSafeBalansType] = useState(null);
  const [safeBalansFilter, setSafeBalansFilter] = useState('');
  const [safeTypeAgg, setSafeTypeAgg] = useState(null);
  const [safeTypeAggLoading, setSafeTypeAggLoading] = useState(false);
  const [safeRoleFinance, setSafeRoleFinance] = useState(null);
  const [safeRoleFinanceLoading, setSafeRoleFinanceLoading] = useState(false);
  ...
  /** Zakazlar: tasdiqlangan (pending emas) yoki tasdiqlanmagan (pending). */
  const [ordersConfirmTab, setOrdersConfirmTab] = useState('unconfirmed');
  ...
  const [adminCourierMoliyaOpen, setAdminCourierMoliyaOpen] = useState(false);
  const [adminCourierMoliyaData, setAdminCourierMoliyaData] = useState(null);
  const [adminCourierMoliyaLoading, setAdminCourierMoliyaLoading] = useState(false);
  ...
  const [awaitingPayoutWithdrawalsCount, setAwaitingPayoutWithdrawalsCount] = useState(0);
  const [withdrawalPayoutQueue, setWithdrawalPayoutQueue] = useState([]);
  const [withdrawalPayoutLoading, setWithdrawalPayoutLoading] = useState(false);
  const [withdrawalMarkPaidBusy, setWithdrawalMarkPaidBusy] = useState(null);
  ...
  const [adSlideRemoveImageMobile, setAdSlideRemoveImageMobile] = useState(false);
  const [footerLive, setFooterLive] = useState({ promo_text: '', phone: '' });
  const [footerDraftText, setFooterDraftText] = useState('');
  const [footerDraftPhone, setFooterDraftPhone] = useState('');
  const [footerStripLoading, setFooterStripLoading] = useState(false);
  const [footerStripBusy, setFooterStripBusy] = useState(null);
  const [footerStripNote, setFooterStripNote] = useState('');
  const [hbLive, setHbLive] = useState({
    section_title: '',
    items: Array.from({ length: 4 }, () => ({ title: '', text: '' })),
  });
  const [hbDraftSectionTitle, setHbDraftSectionTitle] = useState('');
  const [hbDraftItems, setHbDraftItems] = useState(() =>
    Array.from({ length: 4 }, () => ({ title: '', text: '' })),
  );
  ...
  const [twilioPanelMsg, setTwilioPanelMsg] = useState('');
  const [twilioSendTo, setTwilioSendTo] = useState('');
  const [twilioSendBody, setTwilioSendBody] = useState('');
  const [twilioSendBusy, setTwilioSendBusy] = useState(false);

  const [adminProfileEditing, setAdminProfileEditing] = useState(false);
  const [adminProfileDraft, setAdminProfileDraft] = useState({ full_name: '', email: '', login: '' });
  const [adminProfileSaveBusy, setAdminProfileSaveBusy] = useState(false);
  ...
  const [auditLogQueryInput, setAuditLogQueryInput] = useState('');
  const [auditLogAppliedQuery, setAuditLogAppliedQuery] = useState('');
  const [auditLogShowHidden, setAuditLogShowHidden] = useState(false);
  const [auditMemoSummary, setAuditMemoSummary] = useState('');
  const [auditMemoDetails, setAuditMemoDetails] = useState('');
  const [auditMemoBusy, setAuditMemoBusy] = useState(false);
  const [auditEditId, setAuditEditId] = useState(null);
  const [auditEditBusy, setAuditEditBusy] = useState(false);
  const [auditEditDraft, setAuditEditDraft] = useState({
    summary_edited: '',
    note_superuser: '',
    payload_edited: '',
    hidden: false,
  ...
  const loadData = async (withLoader = true) => {
    ...
    try {
      const [
        ...
        request('/admin/enterprise-kpis').then((r) => ensureOk(r, 'Enterprise KPI yuklanmadi')),
        ...
        request('/admin/portal/withdrawal-requests?queue=awaiting_payout').then((r) => (r.ok ? r.json() : {})).then((d) => { setAwaitingPayoutWithdrawalsCount((d.requests || []).length); return null; }),
        request('/admin/notifications').then((r) => (r.ok ? r.json() : {})).then((d) => { setAdminNotifications(d.notifications || []); return null; }),
      ]);

      setStats(statsData || null);
      setEnterpriseKpis(enterpriseKpisData || null);
      ...
      try {
        const safeArchivedRes = await request('/admin/portal/work-roles/safe-archived');
        const safeArchivedData = await safeArchivedRes.json().catch(() => ({}));
        setSafeArchivedRoles(safeArchivedData?.roles || []);
      } catch {
        setSafeArchivedRoles([]);
      }
      try {
        const safeRes = await request('/admin/portal/work-roles/safe/status');
        const safeData = await safeRes.json().catch(() => ({}));
        setSafePasswordSet(Boolean(safeData?.set));
      } catch {
        setSafePasswordSet(false);
      }
    } catch (err) {
  ...
  const loadSafePasswordStatus = useCallback(async () => {
    try {
      const res = await request('/admin/portal/work-roles/safe/status');
      const data = await res.json().catch(() => ({}));
      setSafePasswordSet(Boolean(data?.set));
    } catch {
      setSafePasswordSet(false);
    }
  }, [request]);
  ...
  const dashboardOrderAgg = useMemo(() => {
    const o = { ...EMPTY_ORDER_STATS, ...(orderStats || {}) };
    const total = Number(o.total) || 0;
    const done = (Number(o.delivered) || 0) + (Number(o.completed) || 0);
    const queue = (Number(o.pending) || 0) + (Number(o.processing) || 0);
    const shipping = (Number(o.delivery) || 0) + (Number(o.on_the_way) || 0);
    const ordersForAvg = Number(stats?.orders) || total;
    const avgCheck =
      ordersForAvg > 0 ? Math.round((Number(stats?.totalRevenue) || 0) / ordersForAvg) : 0;
    const donePct = total > 0 ? Math.min(100, Math.round((done / total) * 100)) : 0;
    return { total, done, queue, shipping, cancelled: Number(o.cancelled) || 0, avgCheck, donePct };
  }, [orderStats, stats]);
  ...
  useEffect(() => {
    if (!user || !isSuperuser) return undefined;
    let refreshTimer = null;
    const socket = connectEnterpriseRealtime({
      onEvent: () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const res = await request(`/admin/sales-series?period=${encodeURIComponent(salesChartPeriod)}`);
  ...
  }, [user, isSuperuser, activeView, salesChartPeriod, request]);
  ...
  useEffect(() => {
    if (activeView !== 'accounting') setAccountingMoliyaHubOpen(false);
  }, [activeView]);
  ...
  useEffect(() => {
    ...
    setOrderFilter((prev) =>
      prev.status === orderStatus ? prev : { ...prev, status: orderStatus },
    );
  ...
  const loadWithdrawalPayoutQueue = useCallback(async () => {
    setWithdrawalPayoutLoading(true);
    try {
      const res = await request('/admin/portal/withdrawal-requests?queue=awaiting_payout');
      const d = res.ok ? await res.json() : { requests: [] };
      const list = Array.isArray(d.requests) ? d.requests : [];
      setWithdrawalPayoutQueue(list);
      setAwaitingPayoutWithdrawalsCount(list.length);
    } catch (_) {
      setWithdrawalPayoutQueue([]);
    } finally {
      setWithdrawalPayoutLoading(false);
    }
  }, [request]);
  ...
  useEffect(() => {
    if (activeView !== 'accounting') return undefined;
    loadWithdrawalPayoutQueue();
    return undefined;
  }, [activeView, loadWithdrawalPayoutQueue]);
  ...
  const loadFooterStrip = useCallback(async () => {
    setFooterStripLoading(true);
    setFooterStripNote('');
    try {
      const res = await request('/admin/portal/footer-strip');
      const d = res.ok ? await res.json().catch(() => ({})) : {};
      const live = d.live || {};
      const draft = d.draft || {};
      setFooterLive({
        promo_text: String(live.promo_text || ''),
        phone: String(live.phone || ''),
      });
      setFooterDraftText(String(draft.promo_text ?? ''));
      setFooterDraftPhone(String(draft.phone ?? ''));
    } catch {
      setFooterLive({ promo_text: '', phone: '' });
      setFooterDraftText('');
      setFooterDraftPhone('');
    } finally {
      setFooterStripLoading(false);
    }
  }, [request]);

  const loadHomeBenefits = useCallback(async () => {
    setHbLoading(true);
    setHbNote('');
    try {
      const res = await request('/admin/portal/home-benefits');
      const d = res.ok ? await res.json().catch(() => ({})) : {};
      const live = d.live || {};
      const draft = d.draft || {};
      const liveItems = Array.isArray(live.items) ? live.items : [];
      setHbLive({
        section_title: String(live.section_title || ''),
        items: Array.from({ length: 4 }, (_, i) => ({
          title: String(liveItems[i]?.title ?? ''),
          text: String(liveItems[i]?.text ?? ''),
        })),
      });
      const draftItems = Array.isArray(draft.items) ? draft.items : [];
      setHbDraftSectionTitle(String(draft.section_title ?? ''));
      setHbDraftItems(
        Array.from({ length: 4 }, (_, i) => ({
          title: String(draftItems[i]?.title ?? ''),
          text: String(draftItems[i]?.text ?? ''),
        })),
      );
    } catch {
      setHbLive({ section_title: '', items: Array.from({ length: 4 }, () => ({ title: '', text: '' })) });
      setHbDraftSectionTitle('');
      setHbDraftItems(Array.from({ length: 4 }, () => ({ title: '', text: '' })));
    } finally {
      setHbLoading(false);
    }
  }, [request]);
  ...
  useEffect(() => {
    ...
    loadHomeBenefits();
    return undefined;
  }, [activeView, loadAdSlides, loadFooterStrip, loadHomeBenefits]);
  ...
  const loadTwilioSettings = useCallback(async () => {
    try {
      const res = await request('/admin/twilio/settings');
      const d = await res.json().catch(() => ({}));
      if (!res.ok) return;
      setTwilioSettings({
        account_sid: String(d.account_sid || ''),
        from_number: String(d.from_number || ''),
        auth_token_configured: Boolean(d.auth_token_configured),
        signing_relaxed: Boolean(d.signing_relaxed),
        inbound_webhook_path: String(d.inbound_webhook_path || '/api/webhooks/twilio/sms'),
        inbound_webhook_url: String(d.inbound_webhook_url || ''),
        updated_at: d.updated_at || null,
      });
    } catch {
      /* opsiyonel */
  ...
  useEffect(() => {
    if (activeView !== 'ai_calls') return undefined;
    void loadTwilioSettings();
    return undefined;
  }, [activeView, loadTwilioSettings]);
  ...
  const loadAuditLog = useCallback(async () => {
    if (activeView !== 'activity_log') return;
    setAuditLogLoading(true);
    setError('');
    try {
      const q = auditLogAppliedQuery.trim();
      const qs = `/admin/audit-log?page=${auditLogPage}&limit=35&hidden=${auditLogShowHidden ? '1' : '0'}${
        q ? `&q=${encodeURIComponent(q)}` : ''
      }`;
      const res = await request(qs);
      const d = await ensureOk(res, 'Jurnal yuklanmadi');
      setAuditLogEntries(Array.isArray(d.entries) ? d.entries : []);
      setAuditLogTotal(Number(d.total) || 0);
    } catch (e) {
      setError(String(e.message || e));
    } finally {
      setAuditLogLoading(false);
    }
  }, [activeView, auditLogAppliedQuery, auditLogPage, auditLogShowHidden, request]);

  useEffect(() => {
    if (activeView !== 'activity_log') return undefined;
    void loadAuditLog();
    return undefined;
  }, [activeView, loadAuditLog]);
  ...
  const applyAdSlideSlotFromList = useCallback((slotIdx, list) => {
    ...
    if (s) {
      ...
      setAdSlideForm({ title: '', subtitle: '', link_path: '', image_url: '', image_url_mobile: '', video_url: '', active: true });
    }
    setAdSlideRemoveImage(false);
    setAdSlideRemoveImageMobile(false);
  }, []);
  ...
  const adSlidePreviewMobile =
    adSlideRemoveImageMobile ? null : String(adSlideForm.image_url_mobile || '').trim() || null;
  ...
  const handleAdminNotifyApprove = useCallback(async (notif) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await loadWithdrawalPayoutQueue();
      }
    } finally {
      setAdminNotifyApproveBusy(null);
    }
  }, [request, loadAdminNotifications, loadWithdrawalRequests, loadWithdrawalPayoutQueue]);

  const handleAdminNotifyMarkPaid = useCallback(async (notif) => {
    if (notif?.link_type !== 'withdrawal_payout' || !notif?.link_id) return;
    setAdminNotifyApproveBusy(notif.id);
    try {
      const res = await request(`/admin/portal/withdrawal-requests/${notif.link_id}/mark-paid`, {
        method: 'PATCH',
        body: JSON.stringify({}),
      });
      if (res.ok) {
  ...
  }, [request, loadAdminNotifications, loadWithdrawalPayoutQueue]);

  const handleAdminPayrollAdvanceApprove = useCallback(
    async (notif) => {
      if (notif?.link_type !== 'payroll_advance' || !notif?.link_id) return;
      setAdminNotifyApproveBusy(notif.id);
      try {
        const res = await request(`/admin/payroll-advances/runs/${notif.link_id}/approve`, { method: 'POST', body: '{}' });
        if (res.ok) {
          await request(`/admin/notifications/${notif.id}/read`, { method: 'PATCH' });
          await loadAdminNotifications();
        }
    ...
    [request, loadAdminNotifications],
  );

  const handleAdminAiTargetApprove = useCallback(async (notif) => {
    if (notif?.link_type !== 'seller_product_ai_target' || !notif?.link_id) return;
    setAdminNotifyApproveBusy(notif.id);
    try {
      const res = await request(`/admin/portal/products/${notif.link_id}/approve-ai-target`, {
        method: 'POST',
        body: JSON.stringify({}),
      });
      if (res.ok) {
  ...
  useEffect(() => {
    if (activeView !== 'admin_settings') return;
    setSettingsPhone(String(user?.phone || '').trim());
    setSettingsCurrentPwd('');
    setSettingsNewPwd('');
    setSettingsNewPwd2('');
    setSettingsMsg({ type: '', text: '' });
    setBackupPanelMsg({ type: '', text: '' });
  }, [activeView, user?.phone]);

  const loadBackupList = useCallback(async () => {
    setBackupListLoading(true);
    try {
      const res = await request('/admin/backup/list');
      const d = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(d.error || 'Ro‘yxat yuklanmadi.');
      setBackupList(Array.isArray(d.backups) ? d.backups : []);
    } catch (e) {
      setBackupList([]);
      setBackupPanelMsg({ type: 'err', text: String(e.message || e) });
    } finally {
      setBackupListLoading(false);
    }
  }, [request]);

  const loadDriveBackupPanel = useCallback(async () => {
    try {
      const res = await request('/admin/backup/drive-status');
      const s = await res.json().catch(() => ({}));
      setDriveBackupStatus(res.ok ? s : null);
    } catch {
      setDriveBackupStatus(null);
    }
    setDriveRemoteLoading(true);
    try {
      const res2 = await request('/admin/backup/drive-list');
      const d2 = await res2.json().catch(() => ({}));
      setDriveRemoteFiles(res2.ok && Array.isArray(d2.files) ? d2.files : []);
    } catch {
      setDriveRemoteFiles([]);
    } finally {
      setDriveRemoteLoading(false);
    }
  }, [request]);

  const refreshBackupSection = useCallback(async () => {
    await loadBackupList();
    await loadDriveBackupPanel();
  }, [loadBackupList, loadDriveBackupPanel]);

  const handleBackupRun = useCallback(async () => {
    setBackupPanelBusy(true);
    setBackupPanelMsg({ type: '', text: '' });
    try {
      const res = await request('/admin/backup/run', { method: 'POST', body: JSON.stringify({}) });
      const d = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(d.error || 'Backup xatosi.');
      const bytes = Number(d.bytes) || 0;
      const mb = bytes >= 1048576 ? `${(bytes / 1048576).toFixed(2)} MB` : `${Math.max(1, Math.round(bytes / 1024))} KB`;
      const pathShort = d.zipPath ? String(d.zipPath) : '';
      let text = pathShort ? `Backup yaratildi (${mb}): ${pathShort}` : `Backup yaratildi (${mb}).`;
      if (d.drive?.ok && d.drive.webViewLink) {
        text += ' Google Drive ga yuklandi.';
      }
      if (d.driveUploadError) {
        text += ` ⚠️ Drive yuklash xatosi: ${d.driveUploadError}`;
      }
      setBackupPanelMsg({ type: 'ok', text });
      await refreshBackupSection();
    } catch (e) {
      setBackupPanelMsg({ type: 'err', text: String(e.message || e) });
    } finally {
      setBackupPanelBusy(false);
    }
  }, [request, refreshBackupSection]);

  const handleDriveBackupDelete = useCallback(
    async (fileId) => {
      if (!fileId || !window.confirm('Bu zip ni Google Drive dan o‘chirishni tasdiqlaysizmi?')) return;
      setDriveDeleteBusyId(fileId);
      try {
        const res = await request(`/admin/backup/drive/${encodeURIComponent(fileId)}`, { method: 'DELETE' });
        const d = await res.json().catch(() => ({}));
        ...
        setDriveDeleteBusyId(null);
      }
    },
    [request, loadDriveBackupPanel],
  );

  useEffect(() => {
    if (activeView !== 'admin_settings') return undefined;
    void refreshBackupSection();
    return undefined;
  }, [activeView, refreshBackupSection]);

  const handleSavePhone = useCallback(async () => {
    setSettingsMsg({ type: '', text: '' });
    setSettingsBusy(true);
    try {
      await updateProfile({ phone: String(settingsPhone || '').trim() });
      setSettingsMsg({ type: 'ok', text: 'Telefon raqami saqlandi.' });
    } catch (e) {
      setSettingsMsg({ type: 'err', text: e?.message || 'Saqlashda xatolik.' });
    } finally {
      setSettingsBusy(false);
    }
  }, [updateProfile, settingsPhone]);

  const handleChangePassword = useCallback(async () => {
    setSettingsMsg({ type: '', text: '' });
    const cur = String(settingsCurrentPwd || '');
    const nw = String(settingsNewPwd || '');
    const nw2 = String(settingsNewPwd2 || '');
    if (!cur) { setSettingsMsg({ type: 'err', text: 'Joriy parolingizni kiriting.' }); return; }
    if (nw.length < 6) { setSettingsMsg({ type: 'err', text: 'Yangi parol kamida 6 belgi bo‘lsin.' }); return; }
    if (nw !== nw2) { setSettingsMsg({ type: 'err', text: 'Yangi parollar mos emas.' }); return; }
    setSettingsBusy(true);
    try {
      const verifyRes = await request('/auth/login', {
        method: 'POST',
        body: JSON.stringify({ email: user?.email || user?.login || '', password: cur }),
      });
      if (!verifyRes.ok) {
        setSettingsBusy(false);
        setSettingsMsg({ type: 'err', text: 'Joriy parol noto‘g‘ri.' });
        return;
      }
      await updateProfile({ password: nw });
      setSettingsCurrentPwd('');
      setSettingsNewPwd('');
      setSettingsNewPwd2('');
      setSettingsMsg({ type: 'ok', text: 'Parol almashtirildi.' });
    } catch (e) {
      setSettingsMsg({ type: 'err', text: e?.message || 'Parolni almashtirishda xatolik.' });
    } finally {
      setSettingsBusy(false);
    }
  }, [request, updateProfile, user?.email, user?.login, settingsCurrentPwd, settingsNewPwd, settingsNewPwd2]);

  useEffect(() => {
    if (!['atkaz_products', 'brak_products', 'warehouse_kirim_chiqim'].includes(activeView)) return;
    let cancelled = false;
    setWarehouseProductsLoading(true);
    request('/warehouse-admin/products-overview?bucket=active')
      .then((res) => (res.ok ? res.json() : { products: [] }))
      .then((data) => {
        if (cancelled) return;
        setWarehouseProductsOverview(Array.isArray(data?.products) ? data.products : []);
      })
      .catch(() => {
        if (!cancelled) setWarehouseProductsOverview([]);
      })
      .finally(() => {
  ...
  async function handleTopbarRefresh() {
    ...
    if (activeView === 'accounting') extras.push(loadWithdrawalPayoutQueue());
    if (activeView === 'reklama') {
      extras.push(loadAdSlides());
      extras.push(loadFooterStrip());
      extras.push(loadHomeBenefits());
    }
    if (activeView === 'product_moderation') extras.push(loadSellerModerationCatalog());
    if (activeView === 'activity_log') extras.push(loadAuditLog());
  ...
  const setActiveViewWithUrl = (nextView, urlExtras = {}) => {
    ...
    if (!Object.prototype.hasOwnProperty.call(urlExtras, 'order_status')) {
      params.delete('order_status');
    }
    Object.entries(urlExtras).forEach(([key, value]) => {
  ...
  /** Dashboard bloklaridan asosiy sahifaga o‘tish (Barchasi / mini KPI). */
  const openDashboardDrill = useCallback(
    (drillKey) => {
      setSidebarOpen(false);
      switch (drillKey) {
        case 'activity':
          setActiveViewWithUrl('recent_users');
          break;
        case 'products':
          setSellerModFilter('all');
          setActiveViewWithUrl('product_moderation');
          break;
        case 'delivered':
          setOrdersConfirmTab('confirmed');
          setOrderFilter({ search: '', status: 'delivered_done' });
          setActiveViewWithUrl('orders', { order_status: 'delivered_done' });
          break;
        case 'shipping':
          setOrderFilter({ search: '', status: 'all' });
          setActiveViewWithUrl('on_the_way');
          break;
        case 'cancelled':
          setOrderFilter({ search: '', status: 'all' });
          setActiveViewWithUrl('atkaz');
          break;
        case 'avg_check':
          setOrdersConfirmTab('confirmed');
          setOrderFilter({ search: '', status: 'all' });
          setActiveViewWithUrl('orders');
          break;
        default:
          break;
      }
  ...
  const handleSellerProductApprovedBellOpen = async (notif) => {
    if (
      notif?.link_type !== 'seller_product_approved' &&
      notif?.link_type !== 'seller_product_warehouse_kirim' &&
      notif?.link_type !== 'seller_product_warehouse_chiqim'
    ) {
      return;
    }
    if (notif.link_type === 'seller_product_warehouse_chiqim') {
      setActiveViewWithUrl('warehouse_kirim_chiqim');
    } else {
  ...
  const filteredPackers = useMemo(
    () => staff.filter((row) => row.staff_type === 'packer').filter((row) => matchesStaffSimpleSearch(row, packerSearchDebounced, true)),
    [staff, packerSearchDebounced],
  );

  const filteredPickers = useMemo(
    () => staff.filter((row) => row.staff_type === 'picker').filter((row) => matchesStaffSimpleSearch(row, pickerSearchDebounced, true)),
  ...
  const filteredRecentUsers = useMemo(() => {
    const q = recentUsersSearch.trim().toLowerCase();
    return [...users]
      .sort(
        (a, b) =>
      ...
      .filter((u) => {
        if (!q) return true;
        return [u.id, u.full_name, u.email, u.login, u.phone, u.role]
          .map((v) => String(v || '').toLowerCase())
          .join(' ')
          .includes(q);
      });
  }, [users, recentUsersSearch]);
  ...
  const safeBalansFiltered = useMemo(() => {
    const q = safeBalansFilter.trim().toLowerCase();
    if (!q) return workRoles;
    return workRoles.filter((role) => {
      const source = `${role.id} ${role.role_name || ''} ${role.login || ''} ${role.portal_role || ''}`.toLowerCase();
      return source.includes(q);
    });
  }, [safeBalansFilter, workRoles]);

  const safeBalansTypeRoles = useMemo(() => {
    if (!safeBalansType) return [];
    const q = safeBalansFilter.trim().toLowerCase();
    const seen = new Set();
    return [...workRoles, ...safeArchivedRoles]
      .filter((role) => {
        if (seen.has(role.id)) return false;
        seen.add(role.id);
        if (String(role.portal_role || '').trim().toLowerCase() !== safeBalansType) return false;
        if (!q) return true;
        return `${role.role_name || ''} ${role.login || ''}`.toLowerCase().includes(q);
      });
  }, [safeBalansType, safeBalansFilter, workRoles, safeArchivedRoles]);
  ...
  const aiOperatorFeedOperatorSide = useMemo(
    () =>
      customerConversationEvents.filter(
        (e) => e.source !== 'ai_call' && e.source !== 'twilio_sms',
      ),
    [customerConversationEvents],
  );
  ...
  const aiOperatorSmsMijoz = useMemo(
    () =>
      customerConversationEvents.filter(
        (e) =>
          (e.source === 'staff_chat_customer' && /^customer[:_]/i.test(String(e.event_type || '')))
          || (e.source === 'twilio_sms' && e.event_type === 'twilio:inbound'),
      ),
    [customerConversationEvents],
  );
    ...
    () =>
      customerConversationEvents.filter(
        (e) =>
          ...
          || (e.source === 'twilio_sms' && e.event_type === 'twilio:outbound'),
      ),
    [customerConversationEvents],
  );
  ...
  const saveTwilioOperatorSettings = async () => {
    ...
    try {
      const body = {
        ...
        signing_relaxed: twilioSettings.signing_relaxed,
      };
      if (twilioAuthDraft.trim()) body.auth_token = twilioAuthDraft.trim();
      const res = await request('/admin/twilio/settings', {
        method: 'PATCH',
      ...
      if (!res.ok) throw new Error(d.error || 'Saqlanmadi.');
      setTwilioAuthDraft('');
      setTwilioPanelMsg('Saqlandi.');
      setTwilioSettings((p) => ({
        ...p,
        auth_token_configured: Boolean(d.auth_token_configured),
        signing_relaxed: Boolean(d.signing_relaxed),
        inbound_webhook_url: String(d.inbound_webhook_url || p.inbound_webhook_url),
        updated_at: d.updated_at || p.updated_at,
      }));
    } catch (err) {
      setTwilioPanelMsg(String(err.message || err));
    } finally {
      setTwilioSaving(false);
    }
  };

  const sendTwilioOperatorSms = async () => {
    setTwilioSendBusy(true);
    setTwilioPanelMsg('');
    try {
      const res = await request('/admin/twilio/sms/send', {
        method: 'POST',
        body: JSON.stringify({ to: twilioSendTo.trim(), body: twilioSendBody.trim() }),
      });
      const d = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(d.error || 'Yuborilmadi.');
      setTwilioSendBody('');
      setTwilioPanelMsg(d.sid ? `Yuborildi (${d.sid}).` : 'Yuborildi.');
      await loadAiOperatorPanel();
    } catch (err) {
      setTwilioPanelMsg(String(err.message || err));
    } finally {
      setTwilioSendBusy(false);
    }
  };
  ...
  const handleCreateRole = async (e) => {
    ...
    await runMutation('role-create', async () => {
      const isCourier = REGION_SCOPED_PORTAL_ROLES.has(roleForm.portal_role);
  ...
  const handleSaveRolePortal = async () => {
    ...
    await runMutation(`role-portal-${selectedRoleLive.id}`, async () => {
      const isC = REGION_SCOPED_PORTAL_ROLES.has(rolePortalDraft);
  ...
  const handleRoleAction = async (action, roleOverride = null) => {
    ...
    try {
      ...
      if (updated?.deleted_at) {
        ...
        setActiveViewWithUrl('roles_archive');
      }
    } catch (err) {
  ...
  const handlePermanentDeleteRole = async (role) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || "Rol o'chirilmadi");
      }
      await loadWorkRolesAndTrash();
      setActiveViewWithUrl('roles');
    } catch (err) {
  ...
  const handleClearTrash = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || 'Savat tozalanmadi');
      }
      const data = await res.json().catch(() => ({}));
      await loadWorkRolesAndTrash();
      if (data.skipped > 0 && data.message) alert(data.message);
      setActiveViewWithUrl('roles');
    } catch (err) {
      setError(err.message);
      alert(err.message);
  ...
  const handleSafeExit = () => {
    setSafeOpen(false);
    setSafeSubView('umumiy');
    setActiveViewWithUrl('dashboard');
  };

  const loadSafeRoleFinance = async (roleId) => {
    setSafeRoleFinanceLoading(true);
    setSafeRoleFinance(null);
    try {
      const res = await request(`/admin/portal/work-roles/${roleId}/finance`);
      if (!res.ok) throw new Error('Ma’lumot yuklanmadi');
      setSafeRoleFinance(await res.json());
    } catch (err) {
      alert(err?.message || 'Tarmoq xatosi');
    } finally {
      setSafeRoleFinanceLoading(false);
    }
  };

  const loadSafeTypeAggregate = async (typeKey) => {
    setSafeTypeAggLoading(true);
    setSafeTypeAgg(null);
    try {
      const roles = [...workRoles, ...safeArchivedRoles]
        .filter((r) => String(r.portal_role || '').trim().toLowerCase() === typeKey)
        .slice(0, 100);
      if (roles.length === 0) {
        setSafeTypeAgg({ totalBalance: 0, totalKirim: 0, totalChiqim: 0, activity: [], roleCount: 0 });
        return;
      }
      const results = await Promise.all(
        roles.map((r) =>
          request(`/admin/portal/work-roles/${r.id}/finance`).then((res) => (res.ok ? res.json() : null)),
        ),
      );
      ...
      valid.forEach((d) => {
        ...
        totalKirim += Number(d.kirim) || 0;
        totalChiqim += Number(d.chiqim) || 0;
        (d.activity || []).forEach((a) => activity.push(a));
      });
      ...
      setSafeTypeAgg({
        ...
        totalChiqim,
        activity,
        roleCount: valid.length,
      });
    } catch (err) {
      alert(err?.message || 'Tarmoq xatosi');
    } finally {
      setSafeTypeAggLoading(false);
    }
  };

  const renderSafeArchiveList = (row) => (
    <div key={row.id} className="role-card roles-ok-card">
      <div className="role-card-head">
        <div>
          <strong>{row.role_name || '—'}</strong>
          <span className="roles-archive-id"> #{row.id}</span>
        </div>
        <span className="status-pill blocked">arxiv</span>
      </div>
      <div className="roles-archive-meta muted">
        {row.login ? <span>Login: {row.login}</span> : null}
        {row.phone ? <span> · {row.phone}</span> : null}
        {row.deleted_at ? <span> · {formatDate(row.deleted_at)}</span> : null}
      </div>
      <div className="row-actions roles-archive-actions">
  ...
  const handleSafePasswordSubmit = async (e) => {
    e.preventDefault();
    const password = String(safePasswordInput || '').trim();
    if (!password) {
      alert('Parolni kiriting.');
      return;
    }
    setBusyKey('safe-open');
    try {
      const res = await request(`/admin/portal/work-roles/safe/open`, {
        method: 'POST',
        body: JSON.stringify({ password }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        alert(data?.error || 'Seyf ochilmadi');
        return;
      }
      setSafeModalOpen(false);
      setSafePasswordInput('');
      setSafeOpen(true);
      setActiveViewWithUrl('roles_safe');
    } catch (err) {
      alert(err?.message || 'Tarmoq xatosi');
    } finally {
      setBusyKey('');
  ...
  const handleSetSafePassword = async () => {
    ...
    if (!password || password.length < 4) {
      alert('Parolni kiriting (kamida 4 belgi).');
      return;
    }
    setBusyKey('set-safe-password');
    try {
      const res = await request('/admin/portal/work-roles/safe/password', {
        method: 'POST',
        body: JSON.stringify({ password }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        alert(data?.error || 'Parol o‘rnatilmadi');
        return;
      }
      setSafePasswordSet(true);
      setSafePasswordInput('');
      alert('Seyf paroli muvaffaqiyatli o‘rnatildi.');
    } catch (err) {
      alert(err?.message || 'Tarmoq xatosi');
    } finally {
      setBusyKey('');
  ...
  const handleRecentUserStatusToggle = async (row) => {
    const current = String(row?.status || 'active').toLowerCase();
    const next = current === 'blocked' ? 'active' : 'blocked';
    const name = row.full_name || row.email || `#${row.id}`;
    const ask =
      next === 'blocked'
        ? `"${name}" foydalanuvchisini bloklaysizmi? (Login qila olmaydi)`
        : `"${name}" foydalanuvchisini aktiv qilasizmi?`;
    if (!window.confirm(ask)) return;
    await runMutation(`recent-user-status-${row.id}`, async () => {
      const res = await request(`/admin/users/${row.id}/status`, {
        method: 'PATCH',
        body: JSON.stringify({ status: next }),
      });
      await ensureOk(res, 'Foydalanuvchi statusi yangilanmadi');
    }, 'Foydalanuvchi statusi yangilanmadi');
  };

  const loadRecentUserMessages = useCallback(async (userId) => {
    const id = Number(userId);
    if (!Number.isInteger(id) || id < 1) return;
    setRecentUsersMsgLoading(true);
    try {
      const res = await request(`/admin/users/${id}/messages`);
      const data = await ensureOk(res, 'Xabarlar yuklanmadi');
      setRecentUsersMessages(Array.isArray(data?.messages) ? data.messages : []);
    } catch (e) {
      setRecentUsersMessages([]);
      setError(e?.message || 'Xabarlar yuklanmadi');
    } finally {
      setRecentUsersMsgLoading(false);
    }
  }, []);

  const openRecentUserMessages = useCallback(
    (row) => {
      const id = Number(row?.id);
      if (!Number.isInteger(id) || id < 1) return;
      setRecentUsersMsgUser(row);
      setRecentUsersMsgTitle('');
      setRecentUsersMsgBody('');
      void loadRecentUserMessages(id);
    },
    [loadRecentUserMessages],
  );

  const closeRecentUserMessages = useCallback(() => {
    setRecentUsersMsgUser(null);
    setRecentUsersMessages([]);
    setRecentUsersMsgTitle('');
    setRecentUsersMsgBody('');
  }, []);

  const handleSendRecentUserMessage = async () => {
    const target = recentUsersMsgUser;
    const id = Number(target?.id);
    if (!Number.isInteger(id) || id < 1) return;
    const body = recentUsersMsgBody.trim();
    if (!body) {
      setError('Xabar matnini kiriting.');
      return;
    }
    await runMutation(`recent-user-msg-${id}`, async () => {
      const res = await request(`/admin/users/${id}/messages`, {
        method: 'POST',
        body: JSON.stringify({
          title: recentUsersMsgTitle.trim() || 'MyShop xabari',
          body,
        }),
      });
      await ensureOk(res, 'Xabar yuborilmadi');
      setRecentUsersMsgTitle('');
      setRecentUsersMsgBody('');
      await loadRecentUserMessages(id);
    }, 'Xabar yuborilmadi');
  };

  const handleDeleteRecentUser = async (row) => {
    const id = Number(row?.id);
    if (!Number.isInteger(id) || id < 1) return;
    const label = row.full_name || row.email || `#${id}`;
    const ordersCount = Number(row.orders_count) || 0;
    const warn =
      ordersCount > 0
        ? `\n\nDiqqat: bu foydalanuvchida ${ordersCount} ta buyurtma bor — o‘chirish mumkin emas.`
        : '';
    if (
      !window.confirm(
        `"${label}" foydalanuvchisini butunlay o‘chirasizmi? Bu amalni qaytarib bo‘lmaydi.${warn}`,
      )
    ) {
      return;
    }
    setBusyKey(`recent-user-delete-${id}`);
    setRecentUserDeleteNotice(null);
    setError('');
    try {
      const res = await request(`/admin/users/${id}`, { method: 'DELETE' });
      if (!res.ok) {
        let message = 'Foydalanuvchi o‘chirilmadi';
        try {
        ...
        if (res.status === 409) {
          setRecentUserDeleteNotice({ user: row, message });
          return;
        }
        throw new Error(message);
      }
      if (Number(recentUsersMsgUser?.id) === id) closeRecentUserMessages();
      await loadData(false);
    } catch (err) {
      setError(err.message || 'Foydalanuvchi o‘chirilmadi');
    } finally {
      setBusyKey('');
    }
  };

  const closeDeletionInsights = useCallback(() => {
    setDeletionInsightsOpen(false);
    setDeletionInsightsUser(null);
    setDeletionInsightsData(null);
    setDeletionInsightsLoading(false);
    setInsightDetailOpen(false);
    setInsightDetailBlock(null);
    setInsightDetailData(null);
  ...
  const closeInsightDetail = useCallback(() => {
    ...
    setInsightDetailData(null);
    setInsightDetailLoading(false);
    setInsightDeleteConfirm(null);
    setInsightDeleteBusy(false);
  }, []);

  const openDeletionInsights = useCallback(async (row) => {
    const id = Number(row?.id);
    if (!Number.isInteger(id) || id < 1) return;
    setDeletionInsightsUser(row);
    setDeletionInsightsData(null);
    setDeletionInsightsOpen(true);
    setDeletionInsightsLoading(true);
    try {
      const res = await request(`/admin/users/${id}/deletion-insights`);
      const data = await ensureOk(res, 'Ma’lumotlar yuklanmadi');
      setDeletionInsightsData(data);
    } catch (e) {
      setError(e?.message || 'Ma’lumotlar yuklanmadi');
      closeDeletionInsights();
    } finally {
      setDeletionInsightsLoading(false);
    }
  }, [closeDeletionInsights]);
  ...
  const openInsightDetail = useCallback(
    async (block) => {
      const userId = Number(deletionInsightsUser?.id);
      if (!Number.isInteger(userId) || userId < 1 || !block?.key) return;
      if (!(block.drillable || Number(block.value) > 0)) return;
      setInsightDetailBlock(block);
      setInsightDetailData(null);
      setInsightDetailOpen(true);
      setInsightDetailLoading(true);
      try {
        const res = await request(`/admin/users/${userId}/deletion-insights/${encodeURIComponent(block.key)}`);
        const data = await ensureOk(res, 'Tafsilotlar yuklanmadi');
        setInsightDetailData(data);
      } catch (e) {
        setError(e?.message || 'Tafsilotlar yuklanmadi');
        closeInsightDetail();
      } finally {
        setInsightDetailLoading(false);
      }
    },
    [closeInsightDetail, deletionInsightsUser?.id],
  );

  const formatInsightValue = useCallback((block) => {
    const unit = String(block?.unit || '').toLowerCase();
    const value = Number(block?.value) || 0;
    if (unit.includes('so')) return formatCurrency(value);
    return new Intl.NumberFormat('uz-UZ').format(value);
  }, []);

  const refreshDeletionInsightsSummary = useCallback(async (userId) => {
    const id = Number(userId);
    if (!Number.isInteger(id) || id < 1) return;
    try {
      const res = await request(`/admin/users/${id}/deletion-insights`);
  ...
  const confirmDeleteInsightItem = useCallback(async () => {
    const target = insightDeleteConfirm;
    const userId = Number(deletionInsightsUser?.id);
    const blockKey = insightDetailBlock?.key;
    if (!target || !Number.isInteger(userId) || userId < 1 || !blockKey) return;
    setInsightDeleteBusy(true);
    setError('');
    try {
      const res = await request(
        `/admin/users/${userId}/deletion-insights/${encodeURIComponent(blockKey)}/items/${encodeURIComponent(String(target.id))}`,
        { method: 'DELETE' },
      );
      const data = await ensureOk(res, 'Ma’lumot o‘chirilmadi');
      if (data?.detail) setInsightDetailData(data.detail);
      if (data?.summary) setDeletionInsightsData(data.summary);
      else await refreshDeletionInsightsSummary(userId);
      setInsightDeleteConfirm(null);
      await loadData(false);
    } catch (e) {
      setError(e?.message || 'Ma’lumot o‘chirilmadi');
      setInsightDeleteConfirm(null);
    } finally {
      setInsightDeleteBusy(false);
    }
  }, [
    deletionInsightsUser?.id,
    insightDeleteConfirm,
    insightDetailBlock?.key,
    refreshDeletionInsightsSummary,
  ]);
  ...
  const openAdminCourierMoliya = async (courier) => {
    setAdminCourierMoliyaOpen(true);
    setAdminCourierMoliyaData(null);
    setAdminCourierMoliyaLoading(true);
    try {
      const res = await request(`/admin/couriers/${courier.id}/moliya`);
      if (res.ok) {
        setAdminCourierMoliyaData(await res.json());
      } else {
        setError('Kuryer moliyasini yuklab bo\'lmadi');
        setAdminCourierMoliyaOpen(false);
      }
    } catch (e) {
      setError(e.message);
      setAdminCourierMoliyaOpen(false);
    } finally {
      setAdminCourierMoliyaLoading(false);
    }
  };

  const closeAdminCourierMoliya = () => {
    setAdminCourierMoliyaOpen(false);
    setAdminCourierMoliyaData(null);
  };
  ...
  return (
    <div className={`admin-dashboard ${darkMode ? 'admin-dark' : ''}`}>
            ...
            {filteredGroups.map((group) => (
              <div key={group.label} className="menu-group">
                ...
                {group.items.map((item) => {
                  let badge = badgeValue(item, stats, users.length, pendingWithdrawalsCount, orderStats);
                  if (item.badgeKey === 'pendingWithdrawals') badge = pendingWithdrawalsCount;
                  if (item.badgeKey === 'awaitingPayoutWithdrawals') badge = awaitingPayoutWithdrawalsCount > 0 ? awaitingPayoutWithdrawalsCount : null;
                  if (item.badgeKey === 'pendingSellerProducts') {
                    const n = Number(stats?.pendingSellerProducts) || 0;
                    badge = n > 0 ? n : null;
                  }
                  if (item.badgeKey === 'deletedRoles') {
                    const n = trashRoles.length;
                    badge = n > 0 ? n : null;
                  }
      ...
      <main
        className={`admin-main${activeView === 'staff_chat' || activeView === 'chat_groups' ? ' admin-main--staff-chat' : ''}${accountingMoliyaHubActive ? ' admin-main--moliya-hub' : ''}`}
      >
        {activeView !== 'staff_chat' && activeView !== 'chat_groups' && (
        <header className="topbar-neo">
          ...
          <div className="topbar-right">
            {safeOpen && activeView === 'roles_safe' && (
              <button className="btn-neo btn-neo-warning safe-exit-btn" type="button" onClick={handleSafeExit}>
                <i className="fas fa-lock-open" /> Seyfdan chiqish
              </button>
            )}
            ...
            <NotificationBellCluster
              ...
              setNotificationsOpen={setAdminNotificationsOpen}
              unreadCount={adminNotifications.filter((n) => !n.read_at).length}
              notifications={adminNotifications}
              formatDate={formatDate}
              busyId={adminNotifyApproveBusy}
              bellWrapClassName="admin-bell-wrap"
              bellButtonClassName="icon-btn strong notify"
              dotClassName="dot"
              ...
              onDismiss={async (n) => {
                if (!n.read_at) {
                  await request(`/admin/notifications/${n.id}/read`, { method: 'PATCH' });
                  await loadAdminNotifications();
                }
              }}
              renderActions={(n, { onClose, goNext }) => {
                const busy = adminNotifyApproveBusy === n.id;
                if (n.link_type === 'seller_product_approved' || n.link_type === 'seller_product_warehouse_kirim') {
                  return (
                    <button
                      type="button"
                      className="notif-inbox-btn notif-inbox-btn--primary"
                      onClick={() => {
                        void handleSellerProductApprovedBellOpen(n);
                        onClose();
                      }}
                    >
                ...
                if (n.link_type === 'seller_product_warehouse_chiqim') {
                  return (
                    <button
                      type="button"
                      className="notif-inbox-btn notif-inbox-btn--primary"
                    ...
                    >
                      Ombor kirim/chiqimda ochish
                    </button>
                  );
                }
                if (n.link_type === 'seller_product_ai_target' && n.link_id) {
                  return (
                    <button
                      type="button"
                      className="notif-inbox-btn notif-inbox-btn--primary"
                      disabled={busy}
                      onClick={() => {
                        void handleAdminAiTargetApprove(n).then(() => goNext());
                      }}
                    >
                      {busy ? '...' : 'Tasdiqlash'}
                    </button>
                ...
                if (n.link_type === 'payroll_advance' && n.link_id) {
                  return (
                    <button
                      type="button"
                      className="notif-inbox-btn notif-inbox-btn--primary"
                      disabled={busy}
                      onClick={() => {
                        void handleAdminPayrollAdvanceApprove(n).then(() => goNext());
                      }}
                    >
                      {busy ? '...' : 'Avansni tasdiqlash'}
                    </button>
                  );
                }
                if (n.link_type === 'withdrawal' && n.link_id) {
                  return (
                    <button
                      type="button"
                      className="notif-inbox-btn notif-inbox-btn--primary"
                      disabled={busy}
                      onClick={() => {
                        void handleAdminNotifyApprove(n).then(() => goNext());
                      }}
                    >
                      {busy ? '...' : 'Tasdiqlash'}
                    </button>
                ...
                if (n.link_type === 'withdrawal_payout' && n.link_id) {
                  return (
                    <>
                      <button
                        type="button"
                        ...
                        disabled={busy}
                        onClick={() => {
                          void handleAdminNotifyMarkPaid(n).then(() => goNext());
                        }}
                      >
                        {busy ? '...' : 'Pul berildi'}
                      </button>
                      <button
                        type="button"
                        className="notif-inbox-btn"
                        disabled={busy}
                        onClick={() => {
                          onClose();
                          setActiveViewWithUrl('accounting');
                        }}
                      >
                        Buxgalteriyada ochish
                      </button>
                    </>
                  );
                }
            ...
            {profileOpen &&
              createPortal(
                <>
                  <div
                    className="staff-profile-popover-backdrop"
                    role="presentation"
                    onClick={() => setProfileOpen(false)}
                    aria-hidden="true"
                  />
                  <div
                    className="profile-menu-center-shell"
                    role="presentation"
                    onClick={() => setProfileOpen(false)}
                  >
                    <div
                    ...
                    >
                      <i className="fas fa-home" /> Bosh sahifa
                    </button>
                    <button
                      type="button"
                      role="menuitem"
        ...
        {activeView === 'orders' && (
          <div className="topbar-rule topbar-rule--orders">
            <div className="topbar-rule__band topbar-rule__band--equal-split">
              <button
                type="button"
                className={`topbar-rule__split${ordersConfirmTab === 'confirmed' ? ' is-active' : ''}`}
                onClick={() => setOrdersConfirmTab('confirmed')}
              >
                Tasdiqlangan Zakazlar
              </button>
              <button
                type="button"
                className={`topbar-rule__split${ordersConfirmTab === 'unconfirmed' ? ' is-active' : ''}`}
                onClick={() => setOrdersConfirmTab('unconfirmed')}
              >
        ...
        <section
          className={`admin-content${activeView === 'staff_chat' ? ' admin-content--staff-chat' : ''}${activeView === 'chat_groups' ? ' admin-content--staff-chat' : ''}${accountingMoliyaHubActive ? ' admin-content--moliya-hub' : ''}`}
        >
          {recentUserDeleteNotice && (
            <div className="admin-alert admin-alert--delete-block">
              <span className="admin-alert--delete-block__text">{recentUserDeleteNotice.message}</span>
              <button
                type="button"
                className="admin-alert--delete-block__btn"
                onClick={() => void openDeletionInsights(recentUserDeleteNotice.user)}
              >
                Ma’lumotlarni ko‘rish
              </button>
              <button
                type="button"
                className="admin-alert--delete-block__close"
                aria-label="Yopish"
                onClick={() => setRecentUserDeleteNotice(null)}
              >
                <i className="fas fa-times" />
              </button>
            </div>
          )}
          {error && !recentUserDeleteNotice && <div className="admin-alert">{error}</div>}

          {activeView === 'staff_chat' ? (
            <AdminStaffChat onExitChat={() => setActiveViewWithUrl('dashboard')} />
          ) : activeView === 'chat_groups' ? (
            <ChatGroupsScreen onExit={() => setActiveViewWithUrl('dashboard')} />
          ) : loading ? (
            ...
            <>
              {activeView === 'dashboard' && (
                <>
                  <div className="stats-grid-neo">
                    ...
                    <StatCard
                      ...
                      tone="cyan"
                      label="Jami zakazlar"
                      value={new Intl.NumberFormat('uz-UZ').format(
                        dashboardOrderAgg.total || stats?.orders || 0,
                      )}
                    />
                    <StatCard
                      icon="fa-users"
                    ...
                    <StatCard
                      icon="fa-clock"
                      tone="orange"
                      label="Navbat / jarayon"
                      value={new Intl.NumberFormat('uz-UZ').format(dashboardOrderAgg.queue)}
                    />
                    <StatCard
                      icon="fa-sack-dollar"
                      tone="green"
                      label="Sof foyda"
                      value={formatCurrency(enterpriseKpis?.net_profit)}
                    />
                    <StatCard
                      icon="fa-warehouse"
                      tone="cyan"
                      label="Ombor qiymati"
                      value={formatCurrency(enterpriseKpis?.inventory_value)}
                    />
                    <StatCard
                      icon="fa-user-check"
                      tone="purple"
                      label="Aktiv mijozlar"
                      value={new Intl.NumberFormat('uz-UZ').format(enterpriseKpis?.active_customers || 0)}
                    />
                    <StatCard
                      icon="fa-bullhorn"
                      tone="orange"
                      label="ROAS"
                      value={
                        enterpriseKpis?.roas == null
                          ? '0.00x'
                          : `${new Intl.NumberFormat('uz-UZ', {
                              minimumFractionDigits: 2,
                              maximumFractionDigits: 2,
                            }).format(Number(enterpriseKpis.roas) || 0)}x`
                      }
                    />
                  </div>
                  ...
                  <div className="charts-row-neo">
                    <div className="chart-card-neo">
                      ...
                      <div className="chart-body">
                        <div className="sales-chart">
                          {salesChartLoading ? (
                            <div className="sales-chart-message">Yuklanmoqda...</div>
                          ) : salesChartDisplay.length === 0 ? (
                            <div className="sales-chart-message">Ma&apos;lumot yo&apos;q</div>
                          ) : (
                            salesChartDisplay.map((row) => (
                              <div className="sales-col" key={row.key}>
                                <div
                                  className="sales-bar"
                                  style={{ height: `${row.barPct}%` }}
                                  title={`${new Intl.NumberFormat('uz-UZ').format(Math.round(row.revenue))} so'm • ${row.order_count} ta buyurtma`}
                                />
                                <span>{row.label}</span>
                              </div>
                            ))
                    ...
                    <div className="mini-stats-col">
                      <button
                        type="button"
                        className="mini-stat-card mini-stat-card--drill"
                        onClick={() => openDashboardDrill('delivered')}
                        aria-label="Yetkazilgan buyurtmalarni ko‘rish"
                      >
                        <i className="fas fa-truck" />
                        <div>
                          <span>Yetkazilgan</span>
                          <strong>{new Intl.NumberFormat('uz-UZ').format(dashboardOrderAgg.done)}</strong>
                        </div>
                        <em>{dashboardOrderAgg.donePct}%</em>
                      </button>
                      <button
                        type="button"
                        className="mini-stat-card mini-stat-card--drill"
                        onClick={() => openDashboardDrill('shipping')}
                        aria-label="Yo‘ldagi buyurtmalarni ko‘rish"
                      >
                        <i className="fas fa-shipping-fast" />
                        <div>
                          <span>Yo&apos;lda</span>
                          <strong>{new Intl.NumberFormat('uz-UZ').format(dashboardOrderAgg.shipping)}</strong>
                        </div>
                        <em>buyurtma</em>
                      </button>
                      <button
                        type="button"
                        className="mini-stat-card mini-stat-card--drill"
                        onClick={() => openDashboardDrill('cancelled')}
                        aria-label="Bekor qilingan buyurtmalarni ko‘rish"
                      >
                        <i className="fas fa-times-circle" />
                        <div>
                          <span>Bekor qilingan</span>
                          <strong>{new Intl.NumberFormat('uz-UZ').format(dashboardOrderAgg.cancelled)}</strong>
                        </div>
                        <em>buyurtma</em>
                      </button>
                      <button
                        type="button"
                        className="mini-stat-card mini-stat-card--drill"
                        onClick={() => openDashboardDrill('avg_check')}
                        aria-label="Barcha buyurtmalar va o‘rtacha chek"
                      >
                        <i className="fas fa-percent" />
                        <div>
                          <span>O&apos;rtacha chek</span>
                          <strong>{formatCurrency(dashboardOrderAgg.avgCheck)}</strong>
                        </div>
                        <em>so&apos;m</em>
                      </button>
                    </div>
                    ...
                    <div className="panel-card">
                      <div className="panel-head">
                        ...
                        <button
                          ...
                          onClick={() => openDashboardDrill('activity')}
                        >
                          Barchasi
                        </button>
                      </div>
                    ...
                    <div className="panel-card">
                      <div className="panel-head">
                        ...
                        <button
                          type="button"
                          className="panel-head-link"
                          onClick={() => openDashboardDrill('products')}
                        >
                          Barchasi
                        </button>
                      </div>
                      ...
                      <div className="bubble-wrap">
                        <div className="bubble-item">
                          <div className="bubble big">
                            {staff.filter((row) => row.staff_type === 'operator').length}
                          </div>
                          <span>Operatorlar</span>
                        </div>
                        <div className="bubble-item">
                          <div className="bubble mid">{couriers.length}</div>
                          <span>Kuryerlar</span>
                        </div>
                        <div className="bubble-item">
                          <div className="bubble small">{stats?.superusers ?? 0}</div>
                          <span>Superuser</span>
                        </div>
                      </div>
                      <div className="progress-stack">
                        <div>
                          <div className="progress-head">
                            ...
                            <span>
                              {new Intl.NumberFormat('uz-UZ').format(dashboardOrderAgg.done)} /{' '}
                              {new Intl.NumberFormat('uz-UZ').format(dashboardOrderAgg.total)}
                            </span>
                          </div>
                          <div className="progress-line">
                            <span style={{ width: `${dashboardOrderAgg.donePct}%` }} />
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="roles-view">
                  <div className="text-center mb-4">
                    ...
                    <button className="btn-neo" type="button" style={{ marginLeft: '0.5rem' }} onClick={() => setActiveViewWithUrl('roles_archive')}>
                      <i className="fas fa-box-archive" /> Rollar arxivi ({trashRoles.length})
                    </button>
                  </div>
                        ...
                        <form className="role-create-modal-body" onSubmit={handleCreateRole}>
                          <section className="role-create-section">
                            ...
                            <div className="role-create-fields">
                                ...
                                <select
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setRoleForm((p) => ({
                                      ...p,
                                      portal_role: v,
                                      ...(!REGION_SCOPED_PORTAL_ROLES.has(v) ? { courier_viloyat_id: '', courier_tuman_ids: [] } : {}),
                                    }));
                                  }}
                                >
                              ...
                              {REGION_SCOPED_PORTAL_ROLES.has(roleForm.portal_role) ? (
                                <>
                                  <label className="role-create-label role-create-label-full">
                                    <span>{roleForm.portal_role === 'courier' ? 'Kuryer hududi — viloyat / shahar' : 'Ekspeditor hududi — viloyat / shahar (ixtiyoriy, bo\'sh qoldirilsa — butun O\'zbekiston)'}</span>
                                    <select
                                      className="role-create-input"
                                      ...
                                      required={roleForm.portal_role === 'courier'}
                                    >
                  ...
                  <div className="panel-card">
                    ...
                    <div className="table-wrap roles-table-wrap">
                      <table className="neo-table roles-table">
                        <thead>
                          <tr>
                            <th>ID</th>
                            <th>Rol</th>
                        ...
                        <tbody>
                          {filteredRoles.map((role) => {
                            const viloyatLabel =
                              ...
                              '—';
                            const tumanLabel =
                              role.courier_viloyat_id === 'toshkent_sh' &&
                              Array.isArray(role.courier_tuman_ids) &&
                              role.courier_tuman_ids.length > 0
                                ? role.courier_tuman_ids
                                    .map((id) => TOSHKENT_SH_TUMANS.find((t) => t.id === id)?.label || id)
                                    .join(', ')
                                : '';
                            const hududLabel = role.portal_role === 'expeditor' && !role.courier_viloyat_id
                              ? 'Butun O‘zbekiston'
                              : REGION_SCOPED_PORTAL_ROLES.has(role.portal_role) ? `${viloyatLabel}${tumanLabel ? ` · ${tumanLabel}` : ''}` : '—';
                            const portalLabel =
                              ROLE_PORTAL_OPTIONS.find((item) => item.value === role.portal_role)?.label ||
                              role.portal_role ||
                              'Avto';
                            return (
                              <tr key={role.id}>
                                <td>#{role.id}</td>
                                <td className="roles-table-name">{role.role_name || '—'}</td>
                                <td>{portalLabel}</td>
                                <td><code className="roles-table-code">{role.login || '—'}</code></td>
                                <td>{role.phone || '—'}</td>
                                <td>{role.email || '—'}</td>
                                <td className="roles-table-region">{hududLabel}</td>
                                <td>{formatCurrency(role.total_amount)}</td>
                                <td><span className={`status-pill ${role.status}`}>{role.status || '—'}</span></td>
                                <td>
                                  <div className="row-actions roles-table-actions">
                          ...
                          {filteredRoles.length === 0 ? (
                            <tr>
                              <td colSpan={10} className="roles-table-empty">Rol topilmadi</td>
                            </tr>
                          ) : null}
                        </tbody>
                      </table>
              ...
              {activeView === 'roles_archive' && (
                <div className="roles-archive-view">
                  <div className="panel-card roles-archive-panel">
                    <div className="roles-archive-head">
                      <div>
                        <h4>Rollar arxivi</h4>
                         <p className="muted roles-archive-hint">
                           O‘chirilgan ishchi rollar faqat shu yerda saqlanadi. Tiklash yoki butunlay o‘chirish mumkin.
                         </p>
                      </div>
                      {trashRoles.length > 0 && (
                        <button type="button" className="btn-neo btn-neo-danger" onClick={handleClearTrash} disabled={busyKey === 'clear-trash'}>
                          <i className="fas fa-trash-alt" /> Savatni tozala
                        </button>
                      )}
                      {isSuperuser && !safePasswordSet && (
                        <div className="roles-archive-safe-set">
                          <input
                            type="password"
                            className="neo-input"
                            placeholder="Seyf parolini kiriting (kamida 4 belgi)"
                            value={safePasswordInput}
                            onChange={(e) => setSafePasswordInput(e.target.value)}
                          />
                          <button type="button" className="btn-neo btn-neo-primary" onClick={handleSetSafePassword} disabled={busyKey === 'set-safe-password'}>
                            <i className="fas fa-key" /> O‘rnatish
                          </button>
                        </div>
                      )}
                      {isSuperuser && safePasswordSet && (
                        <button type="button" className="btn-neo btn-neo-primary" onClick={() => setSafeModalOpen(true)} disabled={busyKey === `safe-open`}>
                          <i className="fas fa-lock" /> Seyf
                        </button>
                      )}
                    </div>
                    {trashRoles.length === 0 ? (
                      <p className="muted roles-archive-empty">Arxiv bo‘sh — o‘chirilgan rol yo‘q.</p>
                    ) : (
                      <div className="roles-archive-list">
                        {trashRoles.map((row) => (
                          <div key={row.id} className="role-card roles-archive-card">
                            <div className="role-card-head">
                              <div>
                                <strong>{row.role_name || '—'}</strong>
                                <span className="roles-archive-id"> #{row.id}</span>
              ...
              {safeModalOpen && (
                <div className="modal-overlay-neo" onClick={() => closeSafeModal()}>
                  <div className="modal-panel modal-sm" onClick={(e) => e.stopPropagation()}>
                    <div className="modal-header-neo">
                      <h4><i className="fas fa-lock" /> Seyf — parolni kiriting</h4>
                      <button type="button" className="icon-btn" onClick={() => closeSafeModal()} aria-label="Yopish"><i className="fas fa-times" /></button>
                    </div>
                    <form onSubmit={handleSafePasswordSubmit}>
                      <div className="role-create-fields">
                        <label className="role-create-label">
                          <span>Parol</span>
                          <input
                            className="role-create-input"
                            type="password"
                            placeholder="Seyf parolini kiriting"
                            value={safePasswordInput}
                            onChange={(e) => setSafePasswordInput(e.target.value)}
                            required
                          />
                      ...
                      <div className="role-create-modal-footer">
                        <button type="button" className="btn-neo" onClick={() => closeSafeModal()}>Bekor qilish</button>
                        <button type="submit" className="btn-neo btn-neo-primary" disabled={busyKey === 'safe-open'}>
                          {busyKey === 'safe-open' ? '...' : 'Ochish'}
                        </button>
                      </div>
                    </form>
                  </div>
              ...
              {activeView === 'roles_safe' && (
                <div className="roles-safe-layout">
                  <aside className="roles-safe-sidebar">
                    <h5 className="roles-safe-sidebar-title"><i className="fas fa-lock" /> Seyf</h5>
                    {SAFE_MENU_GROUPS.map((group) => (
                      <div key={group.title} className="roles-safe-nav-group">
                        <p className="roles-safe-nav-group-title">{group.title}</p>
                        <nav className="roles-safe-nav">
                          {group.items.map((item) => (
                            <button
                              key={item.key}
                              type="button"
                              className={`roles-safe-nav-item${safeSubView === item.key ? ' active' : ''}`}
                              onClick={() => setSafeSubView(item.key)}
                            >
                              <i className={`fas ${item.icon}`} />
                              <span>{item.label}</span>
                            </button>
                  ...
                  <div className="roles-safe-content">
                    {safeSubView === 'umumiy' && (
                      <div className="panel-card">
                        <div className="panel-head"><h4>Seyf — Umumiy</h4></div>
                        <p className="muted">Seyf ochilgan. Chapdagi menyudan kerakli bo‘limni tanlang.</p>
                      </div>
                    )}
                    {safeSubView === 'balans' && (
                      <div className="roles-safe-balans-layout">
                        <aside className="roles-safe-balans-sidebar">
                          <h5 className="roles-safe-balans-title"><i className="fas fa-wallet" /> Rollar balansi</h5>
                          <input
                            className="neo-input roles-safe-balans-filter"
                            type="search"
                            placeholder="Rol yoki login bo‘yicha filtr..."
                            value={safeBalansFilter}
                            onChange={(e) => setSafeBalansFilter(e.target.value)}
                          />
                          <nav className="roles-safe-balans-nav">
                            {SAFE_BALANS_TYPES.map((t) => {
                              const count = workRoles.filter((r) => String(r.portal_role || '').trim().toLowerCase() === t.key).length;
                              return (
                                <button
                                  key={t.key}
                                  type="button"
                                  className={`roles-safe-balans-item${safeBalansType === t.key ? ' active' : ''}`}
                                  onClick={() => {
                                    setSafeBalansType(t.key);
                                    setSafeBalansRoleId(null);
                                    setSafeRoleFinance(null);
                                    loadSafeTypeAggregate(t.key);
                                  }}
                                >
                                  <span className="roles-safe-balans-item-left">
                                    <i className={`fas ${t.icon}`} />
                                    <span className="roles-safe-balans-item-name">{t.label}</span>
                                  </span>
                                  <span className="roles-safe-balans-item-count">{count}</span>
                                </button>
                              );
                            })}
                          </nav>
                        </aside>
                        <div className="roles-safe-balans-content">
                          {safeBalansType == null ? (
                            <div className="panel-card">
                              <div className="panel-head"><h4>Balans</h4></div>
                              <p className="muted">Chapdagi ro‘yxatdan rol turini tanlang — o‘sha turga tegishli rollar ro‘yxati ochiladi.</p>
                            </div>
                          ) : safeBalansRoleId == null ? (
                            <>
                              {safeBalansType && (
                                <div className="panel-card safe-ombor-overview">
                                  <div className="panel-head">
                                    <h4>{SAFE_BALANS_TYPES.find((t) => t.key === safeBalansType)?.label || 'Rollar'} — umumiy ko‘rinish</h4>
                                  </div>
                                  {safeTypeAggLoading ? (
                                    <p className="muted">Yuklanmoqda…</p>
                                  ) : safeTypeAgg ? (
                                    <>
                                      <div className="safe-stat-grid">
                                        <div className="safe-stat-card"><span>Umumiy balans</span><strong>{formatCurrency(safeTypeAgg.totalBalance)}</strong></div>
                                        <div className="safe-stat-card"><span>Jami kirim</span><strong className="pos">+{formatCurrency(safeTypeAgg.totalKirim)}</strong></div>
                                        <div className="safe-stat-card"><span>Jami chiqim</span><strong className="neg">−{formatCurrency(safeTypeAgg.totalChiqim)}</strong></div>
                                      </div>
                                      <h5 className="safe-activity-title">Kunlik o‘zgarishlar</h5>
                                      {groupActivityByDay(safeTypeAgg.activity).length === 0 ? (
                                        <p className="muted">Faoliyat yo‘q.</p>
                                      ) : (
                                        <div className="safe-daily-list">
                                          {groupActivityByDay(safeTypeAgg.activity).map((d) => (
                                            <div key={d.day} className="safe-daily-group">
                                              <div className="safe-daily-head">
                                                <strong>{formatDate(d.day)}</strong>
                                                <span className="safe-daily-sums">
                                                  <span className="pos">+{formatCurrency(d.kirim)}</span>
                                                  <span className="neg">−{formatCurrency(d.chiqim)}</span>
                                                </span>
                                              </div>
                                              <SafeActivityTable list={d.items} withDate={false} />
                                            </div>
                                          ))}
                                        </div>
                                      )}
                              ...
                              <div className="panel-card">
                                <div className="panel-head">
                                  <h4>{SAFE_BALANS_TYPES.find((t) => t.key === safeBalansType)?.label || 'Rollar'}</h4>
                                  <button type="button" className="btn-neo" onClick={() => setSafeBalansType(null)}>
                                    <i className="fas fa-arrow-left" /> Turlar
                                  </button>
                                </div>
                                {safeBalansTypeRoles.length === 0 ? (
                                  <p className="muted">Bu turga tegishli rollar topilmadi.</p>
                                ) : (
                                  <div className="safe-roles-grid">
                                  {safeBalansTypeRoles.map((r) => {
                                    const isArchived = safeArchivedRoles.some((a) => a.id === r.id);
                                    return (
                                      <button
                                        key={r.id}
                                        type="button"
                                        className="safe-role-card"
                                        onClick={() => {
                                          setSafeBalansRoleId(r.id);
                                          loadSafeRoleFinance(r.id);
                                        }}
                                      >
                                        <span className="safe-role-card-name">{r.role_name || '—'}</span>
                                        <span className="safe-role-card-login muted">{r.login || ''}</span>
                                        {isArchived && <span className="safe-role-card-badge">Seyfda</span>}
                                        <span className="safe-role-card-balance">{formatCurrency(Number(r.total_amount) || 0)}</span>
                                      </button>
                                    );
                                  })}
                                  </div>
                            ...
                            <div className="panel-card">
                              <div className="panel-head">
                                <h4>{safeRoleFinance?.role?.role_name || 'Rol balansi'}</h4>
                                <button
                                  type="button"
                                  className="btn-neo"
                                  onClick={() => {
                                ...
                                >
                                  <i className="fas fa-arrow-left" /> Ro‘yxatga
                                </button>
                              </div>
                              <div className="safe-role-detail">
                                {safeRoleFinanceLoading ? (
                                  <p className="muted">Yuklanmoqda…</p>
                                ) : safeRoleFinance ? (
                                  <>
                                    <div className="safe-role-detail-head">
                                      <div>
                                        <strong>{safeRoleFinance.role?.role_name || '—'}</strong>
                                        <span className="muted"> · {safeRoleFinance.role?.login || ''}</span>
                                      </div>
                                      <span className="status-pill">{safeRoleFinance.role?.portal_role || ''}</span>
                                      {safeArchivedRoles.some((a) => a.id === safeRoleFinance?.role?.id) && (
                                        <span className="status-pill blocked">Seyfda saqlangan</span>
                                      )}
                                    </div>
                                    <div className="safe-stat-grid">
                                      <div className="safe-stat-card"><span>Umumiy balans</span><strong>{formatCurrency(safeRoleFinance.balance)}</strong></div>
                                      <div className="safe-stat-card"><span>Kirim</span><strong className="pos">+{formatCurrency(safeRoleFinance.kirim)}</strong></div>
                                      <div className="safe-stat-card"><span>Chiqim</span><strong className="neg">−{formatCurrency(safeRoleFinance.chiqim)}</strong></div>
                                    </div>
                                    <h5 className="safe-activity-title">Saytdagi faoliyati</h5>
                                    <SafeActivityTable list={safeRoleFinance.activity} />
                                  </>
                                ) : null}
                              </div>
                            </div>
                    ...
                    {safeSubView === 'kirim' && (
                      <div className="panel-card">
                        <div className="panel-head"><h4>Kirim</h4></div>
                        <p className="muted">Seyfga kirim amallari hozircha mavjud emas.</p>
                      </div>
                    )}
                    {safeSubView === 'chiqim' && (
                      <div className="panel-card">
                        <div className="panel-head"><h4>Chiqim</h4></div>
                        <p className="muted">Seyfdan chiqim amallari hozircha mavjud emas.</p>
                      </div>
                    )}
                    {safeSubView === 'tarix' && (
                      <div className="panel-card">
                        <div className="panel-head"><h4>Tarix</h4></div>
                        <p className="muted">Seyf amallari tarixi hozircha bo‘sh.</p>
                      </div>
                    )}
                    {safeSubView === 'sozlamalar' && (
                      <div className="panel-card">
                        <div className="panel-head">
                          <h4>Sozlamalar</h4>
                          {!safePasswordSet && (
                            <button type="button" className="btn-neo btn-neo-primary" onClick={handleSetSafePassword} disabled={busyKey === 'set-safe-password'}>
                              <i className="fas fa-key" /> Seyf parolini o‘rnatish
                            </button>
                          )}
                        </div>
                        <p className="muted">
                          {safePasswordSet ? 'Seyf paroli o‘rnatilgan.' : 'Seyf paroli hali o‘rnatilmagan.'}
                        </p>
                      </div>
                    )}
                    {(safeSubView || '').startsWith('archive_') && (() => {
                      const item = SAFE_MENU_FLAT.find((i) => i.key === safeSubView);
                      if (!item || !item.portalRole) return null;
                      const list = trashRoles.filter(
                        (r) => String(r.portal_role || '').trim().toLowerCase() === item.portalRole,
                      );
                      return (
                        <div className="panel-card">
                          <div className="panel-head">
                            <h4>{item.label} — arxiv</h4>
                            <span className="roles-safe-archive-count muted">{list.length} ta</span>
                          </div>
                          {list.length === 0 ? (
                            <p className="muted roles-archive-empty">Bu rolda o‘chirilgan (arxivdagi) yozuvlar yo‘q.</p>
                          ) : (
                            <div className="roles-archive-list">{list.map(renderSafeArchiveList)}</div>
                          )}
                        </div>
                      );
                    })()}
              ...
              {activeView === 'couriers' && (
                <div className="panel-card admin-couriers-panel">
                            ...
                            <td>{row.orders_handled ?? 0}</td>
                              ...
                              {courierEditId === row.id ? (
                                ...
                                <>
                                  ...
                                  <button type="button" className="btn-neo btn-neo-success" style={{ marginLeft: '8px' }} onClick={() => openAdminCourierMoliya(row)}>Moliyani ko'rish</button>
                                </>
                              )}
                            </td>
                          </tr>
                  ...
                  {adminCourierMoliyaOpen && (
                    <ModalWrapper open={adminCourierMoliyaOpen} onClose={closeAdminCourierMoliya} title="Kuryer moliyasi" maxWidth="800px">
                      {adminCourierMoliyaLoading ? (
                        <div style={{ padding: '40px', textAlign: 'center' }}>
                          <i className="fas fa-circle-notch fa-spin fa-2x muted" />
                        </div>
                      ) : adminCourierMoliyaData ? (
                        <div style={{ padding: '20px' }}>
                          <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', marginBottom: '20px' }}>
                            <div className="panel-card" style={{ flex: 1, minWidth: '200px', backgroundColor: '#ecfdf5', borderColor: '#a7f3d0' }}>
                              ...
                              <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#065f46' }}>
                                {(adminCourierMoliyaData.balance || 0).toLocaleString('uz-UZ')} so'm
                              </div>
                            </div>
                          </div>
                          
                          <h4 style={{ marginBottom: '10px' }}>Kunlik Buyurtmalar Statistikasi</h4>
                          <div className="table-wrap" style={{ marginBottom: '20px' }}>
                            <table className="neo-table">
                              ...
                              <tbody>
                                {adminCourierMoliyaData.stats.length === 0 ? (
                                  <tr><td colSpan="4" className="muted" style={{ textAlign: 'center' }}>Ma'lumot yo'q</td></tr>
                                ) : adminCourierMoliyaData.stats.map((st, idx) => (
                                  <tr key={idx}>
                                    <td>{st.status}</td>
                                    <td>{st.count}</td>
                                    <td>{(st.total_amount || 0).toLocaleString('uz-UZ')}</td>
                                    <td>{(st.total_courier_fee || 0).toLocaleString('uz-UZ')}</td>
                                  </tr>
                                ))}
                              </tbody>
                            </table>
                          ...
                          <h4 style={{ marginBottom: '10px' }}>Pul Yechish Tarixi</h4>
                          <div className="table-wrap">
                            <table className="neo-table">
                              ...
                              <tbody>
                                {adminCourierMoliyaData.withdrawals.length === 0 ? (
                                  <tr><td colSpan="4" className="muted" style={{ textAlign: 'center' }}>Tranzaksiyalar yo'q</td></tr>
                                ) : adminCourierMoliyaData.withdrawals.map((w) => (
                                  <tr key={w.id}>
                                    <td>{new Date(w.created_at).toLocaleString('uz-UZ')}</td>
                                    <td><strong>{(w.amount || 0).toLocaleString('uz-UZ')}</strong></td>
                        ...
                        <p className="muted" style={{ padding: '20px', textAlign: 'center' }}>Xatolik yuz berdi.</p>
                      )}
                    </ModalWrapper>
                  )}
                </div>
                ...
                <div className="panel-card admin-staff-simple-panel">
                  ...
                  <form className="admin-staff-inline-create" onSubmit={(e) => handleCreateStaff('packer', e)}>
                    <div className="admin-staff-inline-create-grid">
                      <input
                        type="text"
                        className="neo-input"
                        placeholder="To‘liq ism"
                        aria-label="Yangi packer ismi"
                        value={staffForms.packer.full_name}
                        onChange={(e) =>
                          setStaffForms((p) => ({ ...p, packer: { ...p.packer, full_name: e.target.value } }))
                        }
                        required
                      />
                      <input
                        ...
                        className="neo-input"
                        placeholder="Telefon"
                        aria-label="Yangi packer telefoni"
                        value={staffForms.packer.phone}
                        onChange={(e) => setStaffForms((p) => ({ ...p, packer: { ...p.packer, phone: e.target.value } }))}
                      />
                      <select
                        className="neo-input"
                        aria-label="Status"
                        value={staffForms.packer.status}
                        onChange={(e) => setStaffForms((p) => ({ ...p, packer: { ...p.packer, status: e.target.value } }))}
                      >
                        <option value="active">Aktiv</option>
                        <option value="pending">Kutilmoqda</option>
                        <option value="blocked">Bloklangan</option>
                      </select>
                      <button type="submit" className="btn-neo btn-neo-primary" disabled={busyKey === 'staff-create-packer'}>
                        Packer qo‘shish
                      </button>
                    </div>
                    <p className="muted admin-couriers-search-hint admin-couriers-search-hint-second">
                      Picker topshirishi uchun keyin «Rollar» / ishchi rollardan shu telefon va portal roli «Packer» bilan akkaunt yarating — birinchi kirish xodim kartochkasi bilan ulanadi.
                    </p>
                  </form>
                  <div className="admin-couriers-search-block">
                    <input
                      type="search"
                      className="neo-input admin-couriers-search-input"
                      placeholder="Ixtiyoriy: ID, ism, telefon yoki email bo‘yicha qidirish…"
                      value={packerSearchInput}
                      onChange={(e) => setPackerSearchInput(e.target.value)}
                      aria-label="Packer qidiruv"
                    />
                    <p className="muted admin-couriers-search-hint">
                      Barcha packerlar pastda. Qidiruv bo‘sh bo‘lsa ham ro‘yxat ko‘rinadi; yozishni to‘xtatgach qidiruv taxminan 0,3 s keyin yangilanadi.
                    </p>
                  </div>
                  {filteredPackers.length === 0 ? (
                    <p className="muted admin-couriers-muted-center">Hozircha packer yo‘q — yuqoridan qo‘shing yoki qidiruvni tekshiring.</p>
                  ) : (
                    <AdminStaffSimpleTable
                ...
                <div className="panel-card admin-staff-simple-panel">
                  ...
                  <form className="admin-staff-inline-create" onSubmit={(e) => handleCreateStaff('picker', e)}>
                    <div className="admin-staff-inline-create-grid">
                      <input
                        type="text"
                        className="neo-input"
                        placeholder="To‘liq ism"
                        aria-label="Yangi picker ismi"
                        value={staffForms.picker.full_name}
                        onChange={(e) =>
                          setStaffForms((p) => ({ ...p, picker: { ...p.picker, full_name: e.target.value } }))
                        }
                        required
                      />
                      <input
                        ...
                        className="neo-input"
                        placeholder="Telefon"
                        aria-label="Yangi picker telefoni"
                        value={staffForms.picker.phone}
                        onChange={(e) => setStaffForms((p) => ({ ...p, picker: { ...p.picker, phone: e.target.value } }))}
                      />
                      <select
                        className="neo-input"
                        aria-label="Status"
                        value={staffForms.picker.status}
                        onChange={(e) => setStaffForms((p) => ({ ...p, picker: { ...p.picker, status: e.target.value } }))}
                      >
                        <option value="active">Aktiv</option>
                        <option value="pending">Kutilmoqda</option>
                        <option value="blocked">Bloklangan</option>
                      </select>
                      <button type="submit" className="btn-neo btn-neo-primary" disabled={busyKey === 'staff-create-picker'}>
                        Picker qo‘shish
                      </button>
                    </div>
                    <p className="muted admin-couriers-search-hint admin-couriers-search-hint-second">
                      «Rollar» bo‘limidan portal roli «Picker» va shu bilan bir xil telefon bilan ishchi yarating — kirish bilan bog‘lanadi.
                    </p>
                  </form>
                  <div className="admin-couriers-search-block">
                    <input
                    ...
                    />
                    <p className="muted admin-couriers-search-hint">
                      Barcha pickerlar pastda (qidiruv bo‘sh bo‘lsa ham). Qidiruv taxminan 0,3 s keyin yangilanadi.
                    </p>
                  </div>
                  {filteredPickers.length === 0 ? (
                    <p className="muted admin-couriers-muted-center">Hozircha picker yo‘q — yuqoridan qo‘shing yoki qidiruvni tekshiring.</p>
                  ) : (
                    <AdminStaffSimpleTable
              ...
              {activeView === 'arxiv' && (
                <div className="panel-card admin-orders-panel">
                  ...
                  <StaffArchivedOrdersTable request={request} canManage onError={setError} />
                </div>
              )}

              {['orders', 'hold', 'atkaz', 'delivery', 'on_the_way'].includes(activeView) && (() => {
                const map = { hold: 'hold', atkaz: 'cancelled', delivery: 'delivery', on_the_way: 'on_the_way' };
                const fixed = map[activeView] || null;
                const isOrdersMain = activeView === 'orders';
                ...
                const rows = orders.filter((row) => {
                  ...
                  if (isOrdersMain) {
                    ...
                    if (ordersConfirmTab === 'confirmed' && pending) return false;
                    if (ordersConfirmTab === 'unconfirmed' && !pending) return false;
                    if (orderFilter.status === 'delivered_done') {
                      const st = String(row.status || '').toLowerCase();
                      if (st !== 'delivered' && st !== 'completed') return false;
                    } else if (orderFilter.status !== 'all' && row.status !== orderFilter.status) return false;
                  } else if (!fixed && orderFilter.status !== 'all' && row.status !== orderFilter.status) return false;
                ...
                return (
                  <div className="panel-card admin-orders-panel">
                    ...
                    <div className={`admin-orders-toolbar${fixed || isOrdersMain ? ' admin-orders-toolbar--single' : ''}`}>
                      <input
                        type="text"
              ...
              {activeView === 'recent_users' && (
                <div className="panel-card admin-recent-users-panel">
                  <div className="panel-head admin-recent-users-head">
                    <button
                      type="button"
                      className="btn-neo seller-shop-back"
                      onClick={() => setActiveViewWithUrl('dashboard')}
                    >
                      <i className="fas fa-arrow-left" /> Dashboard
                    </button>
                    <h4>Tizimga qo‘shilgan foydalanuvchilar</h4>
                  </div>
                  <p className="muted admin-recent-users-note">
                    Ro‘yxatdan o‘tgan barcha foydalanuvchilar. Xabar, blok va o‘chirish amallari shu yerda.
                  </p>
                  <div className="admin-customers-toolbar">
                    <input
                      type="search"
                      className="neo-input admin-customers-search"
                      placeholder="Ism, email, login, rol bo‘yicha qidiring..."
                      value={recentUsersSearch}
                      onChange={(e) => setRecentUsersSearch(e.target.value)}
                      aria-label="Foydalanuvchilarni qidirish"
                    />
                  </div>
                  <div className="admin-recent-users-list">
                    {filteredRecentUsers.length === 0 ? (
                      <p className="muted admin-recent-users-empty">Foydalanuvchi topilmadi.</p>
                    ) : (
                      filteredRecentUsers.map((row) => {
                        const roleKey = String(row.role || '').toLowerCase();
                        const isSuper = roleKey === 'superuser';
                        const isSelf = Number(row.id) === Number(user?.id);
                        const isBlocked = String(row.status || '').toLowerCase() === 'blocked';
                        const canManage = !isSuper && !isSelf;
                        const displayName = row.full_name || row.email || `#${row.id}`;
                        const emailLabel = row.email || row.login || '—';
                        const timeFull = row.created_at ? formatDateTimeUz(row.created_at, { empty: '-' }) : '—';
                        const timeShort = (() => {
                          const d = parseServerDateTime(row.created_at);
                          if (!d) return timeFull;
                        ...
                        return (
                          <article
                            className={`aru-row${isBlocked ? ' is-blocked' : ''}`}
                            key={row.id}
                            title={`${displayName} · ${emailLabel}`}
                          >
                            <i
                              className={`aru-row__icon fas ${isBlocked ? 'fa-user-lock' : 'fa-user-circle'}`}
                              aria-hidden
                            />
                            <strong className="aru-row__name">{displayName}</strong>
                            <span className="aru-row__dot" aria-hidden>
                              ·
                            </span>
                            <span className="aru-row__email">{emailLabel}</span>
                            <span className={`aru-row__role role-${roleKey || 'unknown'}`}>{row.role || '—'}</span>
                            {isBlocked && <span className="aru-row__blocked">blok</span>}
                            {Number(row.orders_count) > 0 && (
                              <span className="aru-row__orders">{Number(row.orders_count)}</span>
                            )}
                            <time className="aru-row__time" dateTime={row.created_at || undefined} title={timeFull}>
                              {timeShort}
                            </time>
                            <div className="aru-row__actions">
                              <button
                                type="button"
                                className="aru-action aru-action--msg"
                                title="Xabarlar"
                                aria-label={`${displayName} — xabarlar`}
                                onClick={() => openRecentUserMessages(row)}
                              >
                                <i className="fas fa-envelope" />
                              </button>
                              <button
                                type="button"
                                className={`aru-action aru-action--block${isBlocked ? ' is-active' : ''}`}
                                title={isBlocked ? 'Aktiv qilish' : 'Bloklash'}
                                aria-label={`${displayName} — ${isBlocked ? 'aktiv qilish' : 'bloklash'}`}
                                disabled={!canManage || busyKey === `recent-user-status-${row.id}`}
                                onClick={() => void handleRecentUserStatusToggle(row)}
                              >
                                {busyKey === `recent-user-status-${row.id}` ? (
                                  <i className="fas fa-spinner fa-spin" />
                                ) : (
                                  <i className={`fas ${isBlocked ? 'fa-unlock' : 'fa-ban'}`} />
                                )}
                              </button>
                              <button
                                type="button"
                                className="aru-action aru-action--delete"
                                title="O‘chirish"
                                aria-label={`${displayName} — o‘chirish`}
                                disabled={!canManage || busyKey === `recent-user-delete-${row.id}`}
                                onClick={() => void handleDeleteRecentUser(row)}
                              >
                                {busyKey === `recent-user-delete-${row.id}` ? (
                                  <i className="fas fa-spinner fa-spin" />
                                ) : (
              ...
              {deletionInsightsOpen && (
                <ModalWrapper
                  open={deletionInsightsOpen}
                  onClose={closeDeletionInsights}
                  title={
                    deletionInsightsUser
                      ? `O‘chirish ma’lumotlari — ${deletionInsightsUser.full_name || deletionInsightsUser.email || `#${deletionInsightsUser.id}`}`
                      : 'O‘chirish ma’lumotlari'
                  }
                  maxWidth="820px"
                >
                  <div className="aru-deletion-insights">
                    {deletionInsightsLoading ? (
                      <p className="muted aru-deletion-insights__loading">
                        <i className="fas fa-spinner fa-spin" /> Yuklanmoqda...
                      </p>
                    ) : deletionInsightsData?.blocks?.length ? (
                      <>
                        <p className="aru-deletion-insights__lead muted">
                          {deletionInsightsData.user?.role
                            ? `Rol: ${deletionInsightsData.user.role}. `
                            : ''}
                          O‘chirishdan oldin bog‘liq ma’lumotlar:
                        </p>
                        <div className="aru-deletion-insights__grid">
                          {deletionInsightsData.blocks.map((block) => {
                            const canDrill = Boolean(block.drillable || Number(block.value) > 0);
                            return (
                              <article
                                className={`aru-deletion-insights__card${canDrill ? ' is-drillable' : ''}`}
                                key={block.key}
                                role={canDrill ? 'button' : undefined}
                                tabIndex={canDrill ? 0 : undefined}
                                title={canDrill ? 'Batafsil ko‘rish' : undefined}
                                onClick={canDrill ? () => void openInsightDetail(block) : undefined}
                                onKeyDown={
                                  canDrill
                                    ? (e) => {
                                        if (e.key === 'Enter' || e.key === ' ') {
                                          e.preventDefault();
                                          void openInsightDetail(block);
                                        }
                                      }
                                    : undefined
                                }
                              >
                                <span className="aru-deletion-insights__label">{block.label}</span>
                                <strong className="aru-deletion-insights__value">
                                  {formatInsightValue(block)}
                                </strong>
                                {block.unit && !String(block.unit).toLowerCase().includes('so') && (
                                  <em className="aru-deletion-insights__unit">{block.unit}</em>
                                )}
                                {canDrill && <i className="fas fa-chevron-right aru-deletion-insights__chev" aria-hidden />}
                              </article>
                            );
                          })}
                        </div>
              ...
              {insightDetailOpen && (
                <ModalWrapper
                  ...
                  title={
                    insightDetailBlock?.label
                      ? `${insightDetailBlock.label} — tafsilot`
                      : 'Tafsilot'
                  }
                  maxWidth="560px"
                >
                  <div className="aru-insight-detail">
                    {insightDetailLoading ? (
                      <p className="muted aru-insight-detail__loading">
                        <i className="fas fa-spinner fa-spin" /> Yuklanmoqda...
                      </p>
                    ) : insightDetailData?.items?.length ? (
                      <ul className="aru-insight-detail__list">
                        {insightDetailData.items.map((item) => (
                          <li className="aru-insight-detail__row" key={item.id}>
                            <div className="aru-insight-detail__body">
                              <div className="aru-insight-detail__main">
                                <strong>{item.primary}</strong>
                                <span>{item.secondary}</span>
                              </div>
                              {Array.isArray(item.products) && item.products.length > 0 && (
                                <div className="aru-insight-detail__products">
                                  {item.products.map((prod, idx) => (
                                    <div className="aru-insight-detail__product" key={`${item.id}-p-${idx}`}>
                                      <div className="aru-insight-detail__product-media">
                                        {prod.image_url ? (
                                          <img
                                            src={prod.image_url}
                                            alt=""
                                            className="aru-insight-detail__product-thumb"
                                            loading="lazy"
                                          />
                                        ) : (
                                          <span
                                            className="aru-insight-detail__product-thumb aru-insight-detail__product-thumb--empty"
                                            aria-hidden
                                          >
                                            <i className="fas fa-image" />
                                          </span>
                                        )}
                                        <span className="aru-insight-detail__product-name">{prod.name}</span>
                                      </div>
                                      <span className="aru-insight-detail__product-qty">
                                        {new Intl.NumberFormat('uz-UZ').format(Number(prod.quantity) || 0)} dona
                                      </span>
                                    </div>
                                  ))}
                                </div>
                              )}
                              <div className="aru-insight-detail__meta">
                                {item.detail && <em>{item.detail}</em>}
                                {item.date && (
                                  <time>{formatDateTimeUz(item.date, { empty: '-' })}</time>
                                )}
                              </div>
                            </div>
                            {item.deletable !== false && (
                              <div className="aru-insight-detail__foot">
                                <button
                                  type="button"
                                  className="aru-insight-detail__delete-btn"
                                  disabled={insightDeleteBusy}
                                  onClick={() => setInsightDeleteConfirm(item)}
                                >
                                  <i className="fas fa-trash-alt" /> O‘chirish
                                </button>
                              </div>
                            )}
                          </li>
                      ...
                      <p className="muted">Bu bo‘limda yozuv topilmadi.</p>
                    )}

                    {insightDeleteConfirm && (
                      <div className="aru-insight-confirm" role="dialog" aria-modal="true">
                        <div className="aru-insight-confirm__box">
                          <p>Siz haqiqatdan ham ushbu ma’lumotni o‘chirmoqchimisiz?</p>
                          <div className="aru-insight-confirm__actions">
                            <button
                              type="button"
                              className="btn-neo btn-neo-danger btn-sm"
                              disabled={insightDeleteBusy}
                              onClick={() => void confirmDeleteInsightItem()}
                            >
                              {insightDeleteBusy ? (
                                <>
                                  <i className="fas fa-spinner fa-spin" /> Kutilmoqda...
                                </>
                              ) : (
                                'Ha'
                              )}
                            ...
                            <button
                              ...
                              disabled={insightDeleteBusy}
                              onClick={() => setInsightDeleteConfirm(null)}
                            >
                              Yo‘q
                            </button>
                          </div>
              ...
              {recentUsersMsgUser && (
                <ModalWrapper
                  open={Boolean(recentUsersMsgUser)}
                  onClose={closeRecentUserMessages}
                  title={`Xabarlar — ${recentUsersMsgUser.full_name || recentUsersMsgUser.email || `#${recentUsersMsgUser.id}`}`}
                  maxWidth="640px"
                >
                  <div className="admin-recent-users-msg-modal">
                    <div className="admin-recent-users-msg-list">
                      {recentUsersMsgLoading ? (
                        <p className="muted">
                          <i className="fas fa-spinner fa-spin" /> Yuklanmoqda...
                        </p>
                        ...
                        recentUsersMessages.map((msg) => (
                          <article className="admin-recent-users-msg-card" key={msg.id}>
                            <header>
                              <strong>{msg.title || 'Xabar'}</strong>
                              <time>{msg.created_at ? formatDateTimeUz(msg.created_at, { empty: '-' }) : '—'}</time>
                            </header>
                            <p>{msg.body}</p>
                            {msg.read_at ? (
                              <small className="admin-recent-users-msg-read">
                                <i className="fas fa-check-double" /> O‘qilgan: {formatDateTimeUz(msg.read_at, { empty: '-' })}
                              </small>
                            ) : (
                              <small className="muted">Hali o‘qilmagan</small>
                            )}
                          </article>
                        ))
                      )}
                    </div>
                    <div className="admin-recent-users-msg-compose">
                      <h5>Yangi xabar yozish</h5>
                      <input
                        type="text"
                        className="neo-input"
                        placeholder="Sarlavha (ixtiyoriy)"
                        value={recentUsersMsgTitle}
                        onChange={(e) => setRecentUsersMsgTitle(e.target.value)}
                        maxLength={200}
                      />
                      <textarea
                        className="neo-input admin-recent-users-msg-textarea"
                        placeholder="Xabar matni..."
                        rows={4}
                        value={recentUsersMsgBody}
                        onChange={(e) => setRecentUsersMsgBody(e.target.value)}
                        maxLength={4000}
                      />
                      <button
                        type="button"
                        className="btn-neo btn-neo-primary admin-recent-users-msg-send"
                        disabled={busyKey === `recent-user-msg-${recentUsersMsgUser.id}` || !recentUsersMsgBody.trim()}
                        onClick={() => void handleSendRecentUserMessage()}
                      >
                        {busyKey === `recent-user-msg-${recentUsersMsgUser.id}` ? (
                          <>
                            <i className="fas fa-spinner fa-spin" /> Yuborilmoqda...
                          </>
                        ) : (
                          <>
                            <i className="fas fa-paper-plane" /> Yuborish
                          </>
                        )}
                      </button>
                    </div>
              ...
              {activeView === 'activity_log' && (() => {
                const AUDIT_PAGE_SIZE = 35;
                const totalPages = Math.max(1, Math.ceil((Number(auditLogTotal) || 0) / AUDIT_PAGE_SIZE));
                return (
                <div className="panel-card">
                  <div className="panel-head">
                    <h4><i className="fas fa-history" /> Loyiha jurnali</h4>
                  </div>
                  <p className="muted" style={{ marginBottom: '0.5rem', fontSize: '0.86rem' }}>
                    To‘g‘ridan-to‘g‘ri havola:{' '}
                    <code>/admin?view=activity_log</code>{' '}yoki qisqa{' '}
                    <code>/admin/jurnal</code>
                    {' '}— sahifa menyudan ham ochiladi («Loyiha jurnali»).
                  </p>
                  <p className="muted" style={{ marginBottom: '1rem' }}>
                    Bearer token bilan yozuvchilarning muvaffaqiyatli o‘zgartirish so‘rovlari (POST/PUT/PATCH/DELETE, 2xx) avtomatik yoziladi — chat va webhook yo‘nalishlarida hajm kamaytirish uchun yozilmaydi.
                    Superuser yozuv mazmunini, izohni tahrirlashi yoki jurnaldan yashirib qo‘yishi mumkin.
                  </p>
                  ...
                  {auditEditId !== null ? (
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.65rem', gap: '0.5rem', flexWrap: 'wrap' }}>
                        <strong>Tahrir — yozuv #{auditEditId}</strong>
                        <button type="button" className="btn-neo btn-neo-ghost btn-neo-xs" disabled={auditEditBusy} onClick={() => { setAuditEditId(null); setAuditEditBusy(false); }}>
                          Bekor
                        </button>
                      </div>
                      <label className="admin-form-label-stack" style={{ marginBottom: '0.65rem', display: 'block' }}>
                        <span>Yozuv (ko‘rinishdagi asosiy matn)</span>
                        <textarea className="neo-input" rows={3} value={auditEditDraft.summary_edited} onChange={(e) => setAuditEditDraft((p) => ({ ...p, summary_edited: e.target.value }))} />
                      </label>
                      <label className="admin-form-label-stack" style={{ marginBottom: '0.65rem', display: 'block' }}>
                        <span>Superuser izohi</span>
                        <textarea className="neo-input" rows={3} value={auditEditDraft.note_superuser} onChange={(e) => setAuditEditDraft((p) => ({ ...p, note_superuser: e.target.value }))} />
                      </label>
                      <label className="admin-form-label-stack" style={{ marginBottom: '0.65rem', display: 'block' }}>
                        <span>Qo‘shimcha yuklamalar (JSON yoki tekst, ixt.)</span>
                        <textarea className="neo-input" rows={2} value={auditEditDraft.payload_edited} onChange={(e) => setAuditEditDraft((p) => ({ ...p, payload_edited: e.target.value }))} />
                      </label>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem', fontSize: '0.9rem' }}>
                        <input type="checkbox" checked={auditEditDraft.hidden} onChange={(e) => setAuditEditDraft((p) => ({ ...p, hidden: e.target.checked }))} />
                        Yashirib qo‘yish (standart ro‘yxatdan)
                      </label>
                      <button
                        type="button"
                        className="btn-neo btn-neo-primary"
                        disabled={auditEditBusy}
                        onClick={async () => {
                          if (auditEditId == null) return;
                          setAuditEditBusy(true);
                          setError('');
                          try {
                            const res = await request(`/admin/audit-log/${auditEditId}`, {
                              method: 'PATCH',
                              body: JSON.stringify({
                                summary_edited: auditEditDraft.summary_edited.trim() ? auditEditDraft.summary_edited.trim() : null,
                                note_superuser: auditEditDraft.note_superuser.trim() ? auditEditDraft.note_superuser.trim() : null,
                                payload_edited: auditEditDraft.payload_edited.trim() ? auditEditDraft.payload_edited.trim() : null,
                                hidden: auditEditDraft.hidden ? 1 : 0,
                              }),
                            });
                            await ensureOk(res, 'Saqlanmadi');
                            setAuditEditId(null);
                            await loadAuditLog();
                          } catch (err) {
                            setError(String(err.message || err));
                          } finally {
                            setAuditEditBusy(false);
                          }
                        }}
                      >
                        {auditEditBusy ? 'Saqlanyapti…' : 'Saqlash'}
                      </button>
                    </div>
                  ) : null}

                  <div className="neo-card" style={{ padding: '1rem', marginBottom: '1rem', borderRadius: 12 }}>
                    <strong style={{ display: 'block', marginBottom: '0.5rem' }}>Qo‘lda eslatma</strong>
                    <label className="admin-form-label-stack" style={{ marginBottom: '0.65rem', display: 'block' }}>
                      <span>Qisqa mazmun</span>
                      <input className="neo-input" value={auditMemoSummary} onChange={(e) => setAuditMemoSummary(e.target.value)} placeholder="Masalan: depozitsiya qoidalari yangilandi" />
                    </label>
                    <label className="admin-form-label-stack" style={{ marginBottom: '0.65rem', display: 'block' }}>
                      <span>Batafsil (ixtiyoriy)</span>
                      <textarea className="neo-input" rows={2} value={auditMemoDetails} onChange={(e) => setAuditMemoDetails(e.target.value)} placeholder="Izoh va havolalar..." />
                    </label>
                    <button
                      type="button"
                      className="btn-neo btn-neo-primary"
                      disabled={auditMemoBusy || !auditMemoSummary.trim()}
                      onClick={async () => {
                        setAuditMemoBusy(true);
                        setError('');
                        try {
                          const res = await request('/admin/audit-log/manual', {
                            method: 'POST',
                            body: JSON.stringify({ summary: auditMemoSummary.trim(), details: auditMemoDetails.trim() }),
                          });
                          await ensureOk(res, 'Saqlanmadi');
                          setAuditMemoSummary('');
                          setAuditMemoDetails('');
                          ...
                          setAuditMemoBusy(false);
                        }
                      }}
                    >
                      {auditMemoBusy ? 'Qo‘shyapti…' : 'Journalga yozish'}
                    </button>
                  </div>

                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'flex-end', marginBottom: '1rem' }}>
                    <label style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', minWidth: 220, flex: '1 1 220px' }}>
                      <span className="muted" style={{ fontSize: '0.85rem' }}>Qidiruv</span>
                      <input
                        className="neo-input"
                        value={auditLogQueryInput}
                        onChange={(e) => setAuditLogQueryInput(e.target.value)}
                        placeholder="Yo‘l, rol, yozuv..."
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            setAuditLogPage(1);
                            setAuditLogAppliedQuery(auditLogQueryInput);
                          }
                        }}
                      />
                    </label>
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.45rem', fontSize: '0.9rem' }}>
                      <input
                        type="checkbox"
                        checked={auditLogShowHidden}
                        onChange={(e) => {
                          setAuditLogShowHidden(e.target.checked);
                          setAuditLogPage(1);
                        }}
                      />
                      Yashirib qo‘yilganlari ham
                    </label>
                  </div>

                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '0.75rem' }}>
                    <span className="muted" style={{ fontSize: '0.88rem' }}>
                      Jami {auditLogTotal} ta · sahifa {auditLogPage}/{totalPages}
                    </span>
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      <button type="button" className="btn-neo btn-neo-ghost" disabled={auditLogPage <= 1 || auditLogLoading} onClick={() => setAuditLogPage((p) => Math.max(1, p - 1))}>
                        Oldingi
                      </button>
                      <button type="button" className="btn-neo btn-neo-ghost" disabled={auditLogPage >= totalPages || auditLogLoading} onClick={() => setAuditLogPage((p) => p + 1)}>
                        Keyingi
                      </button>
                    </div>
                  </div>
                      ...
                      <table className="neo-table">
                        ...
                        <tbody>
                          {auditLogEntries.map((row) => {
                            const displaySummary = row.summary_edited || row.summary_original || '—';
                            const src = row.source === 'manual' ? 'Qo‘lda' : 'HTTP';
                            return (
                              <tr key={row.id}>
                                <td>{row.id}</td>
                                <td>{row.created_at ? formatDateTimeUz(row.created_at) : '—'}</td>
                                <td><span className={`menu-badge ${row.source === 'manual' ? 'warning' : 'default'}`}>{src}</span></td>
                                <td>
                                  <div style={{ fontSize: '0.82rem', lineHeight: 1.35 }}>
                                    <strong>{customerCellValue(row.actor_label, row.actor_user_id ? `#${row.actor_user_id}` : '—')}</strong>
                                    <div className="muted">{customerCellValue(row.actor_role)}</div>
                                    <div className="muted" title={`${String(row.method || '')} ${String(row.path || '')}`}>
                                      {customerCellValue(row.method)} {String(row.path || '').length > 48 ? `${String(row.path || '').slice(0, 48)}…` : String(row.path || '')}
                                    </div>
                                  </div>
                                </td>
                                <td style={{ maxWidth: 320 }}>
                                  <div style={{ fontSize: '0.88rem', whiteSpace: 'pre-wrap' }}>{displaySummary}</div>
                                  {row.note_superuser ? (
                                    <div style={{ marginTop: '0.35rem', fontSize: '0.8rem', color: 'var(--accent)' }}>{row.note_superuser}</div>
                                  ) : null}
                                  {(row.summary_edited && row.summary_edited !== row.summary_original) ? (
                                    <div className="muted" style={{ marginTop: '0.25rem', fontSize: '0.75rem' }}>Original: {String(row.summary_original || '').slice(0, 200)}{String(row.summary_original || '').length > 200 ? '…' : ''}</div>
                                  ) : null}
                                </td>
                                <td>
                                  <button
                                    type="button"
                                    className="btn-neo btn-neo-ghost btn-neo-xs"
                                    disabled={auditEditBusy}
                                    onClick={() => {
                                      setAuditEditId(row.id);
                                      setAuditEditDraft({
                                        summary_edited: row.summary_edited != null ? String(row.summary_edited) : '',
                                        note_superuser: row.note_superuser != null ? String(row.note_superuser) : '',
                                        payload_edited: row.payload_edited != null ? String(row.payload_edited) : '',
                  ...
                  {auditLogEntries.length === 0 && !auditLogLoading ? <p className="muted">Yozuvlar yo‘q.</p> : null}
                </div>
                );
              })()}

              {activeView === 'accounting' && accountingMoliyaHubOpen ? (
                <AdminAccountingMoliyaHub onBack={() => setAccountingMoliyaHubOpen(false)} />
              ) : null}
              ...
              {activeView === 'accounting' && !accountingMoliyaHubOpen && (
                <div className="panel-card admin-accounting-panel">
                  <div className="panel-head panel-head--accounting">
                    <h4>Buxgalteriya</h4>
                    <div className="admin-accounting-head-tools">
                      <button
                        type="button"
                        className="btn-neo btn-neo-primary admin-accounting-moliya-btn"
                        onClick={() => setAccountingMoliyaHubOpen(true)}
                        title="Buxgalteriya moliya bo‘limlarini ochish"
                      >
                        <i className="fas fa-wallet" aria-hidden /> Moliya
                      </button>
                      <input
                        type="search"
                  ...
                  <section className="admin-accounting-payout admin-withdrawals-panel" aria-labelledby="admin-payout-queue-title">
                    <h5 id="admin-payout-queue-title" style={{ margin: '0 0 8px', fontSize: '1.05rem', fontWeight: 600 }}>
                      <i className="fas fa-hand-holding-usd" aria-hidden /> To‘lov navbati
                    </h5>
                    <p className="admin-withdrawals-hint muted">
                      Superuser pul yechishni tasdiqlagan so‘rovlar. Naqd/card berilgandan keyin «Pul berildi» — boshqa superuserlarga tasdiq xabari ketadi.
                    </p>
                    {withdrawalPayoutLoading ? (
                      <p className="admin-withdrawals-empty muted">Yuklanmoqda...</p>
                    ) : withdrawalPayoutQueue.length === 0 ? (
                      <p className="admin-withdrawals-empty muted">Berish uchun navbat yo‘q.</p>
                    ) : (
                      <div className="admin-withdrawals-grid">
                        {withdrawalPayoutQueue.map((row, queueIndex) => (
                          <article key={`payout-${row.id}`} className="admin-withdrawal-card">
                            <div className="admin-withdrawal-card__top-row">
                              ...
                              <header className="admin-withdrawal-card__head">
                                <span className="admin-withdrawal-card__role" title="">
                                  {row.seller_id != null ? `Seller · ${row.seller_name || `#${row.seller_id}`}` : row.role_name || 'Ishchi rol'}
                                </span>
                                <span className="admin-withdrawal-card__ids">
                                  So‘rov #{row.id}
                                  {row.work_role_id != null ? (
                                    <span className="admin-withdrawal-card__wrid">WR #{row.work_role_id}</span>
                                  ) : row.seller_id != null ? (
                                    <span className="admin-withdrawal-card__wrid">S #{row.seller_id}</span>
                                  ) : null}
                                </span>
                              </header>
                            </div>
                            <div className="admin-withdrawal-card__login">
                              {row.seller_id != null ? row.seller_name || 'Seller' : row.work_role_login || '—'}
                            </div>
                              ...
                              {row.seller_id != null ? (
                                <>
                                  {row.seller_phone ? (
                                    <li>
                                      ...
                                      <span>{row.seller_phone}</span>
                                    </li>
                                  ) : null}
                                  {row.seller_email ? (
                                    <li>
                                      <i className="fas fa-envelope" aria-hidden />
                                      <span>{row.seller_email}</span>
                                    </li>
                                  ) : null}
                                ...
                                <>
                                  {row.work_role_phone ? (
                                    <li>
                                      <i className="fas fa-phone" aria-hidden />
                                      <span>{row.work_role_phone}</span>
                                    </li>
                                  ) : null}
                                  {row.work_role_email ? (
                                    <li>
                                      <i className="fas fa-envelope" aria-hidden />
                                      <span>{row.work_role_email}</span>
                                    </li>
                                  ) : null}
                                  {!row.work_role_phone && !row.work_role_email ? (
                                    <li className="admin-withdrawal-card__contacts--empty">Telefon / email yo‘q</li>
                                  ) : null}
                                </>
                              )}
                            </ul>
                            <div className="admin-withdrawal-card__amount">{formatCurrency(row.amount)}</div>
                            <div className="admin-withdrawal-card__payout muted">
                              {row.payout_method === 'card' ? 'Karta' : 'Naqd'}
                            </div>
                            <div className="admin-withdrawal-card__actions">
                              <button
                                type="button"
                                className="btn-neo btn-neo-success"
                                disabled={withdrawalMarkPaidBusy === row.id}
                                onClick={async () => {
                                  if (!window.confirm(`Pul berilganini qayd etasizmi? So‘rov #${row.id} · ${formatCurrency(row.amount)}`)) return;
                                  setWithdrawalMarkPaidBusy(row.id);
                                  try {
                                    const res = await request(`/admin/portal/withdrawal-requests/${row.id}/mark-paid`, {
                                      method: 'PATCH',
                                      body: JSON.stringify({}),
                                    });
                                    if (res.ok) await loadWithdrawalPayoutQueue();
                                  } finally {
                                    setWithdrawalMarkPaidBusy(null);
                                  }
                                }}
                              >
                                {withdrawalMarkPaidBusy === row.id ? '...' : 'Pul berildi'}
                              </button>
                            </div>
                          </article>
                        ))}
                    ...
                    <div className="admin-accounting-result" role="region" aria-label="Qidiruv natijasi">
                      ...
                      {accounting.filter.type === 'work_role' && (
                        <p className="admin-accounting-result-extra muted">
                          ...
                          {' · '}
                          Balans: <strong>{formatCurrency(accounting.summary?.sellerBalance || 0)}</strong>
                        </p>
                      )}
                  ...
                  <AdminPayrollOversight request={request} />
                </div>
              )}
                      ...
                      {withdrawalRequests.map((wr, queueIndex) => (
                        <article key={wr.id} className="admin-withdrawal-card">
                          <div className="admin-withdrawal-card__top-row">
                            ...
                            <header className="admin-withdrawal-card__head">
                              <span
                                className="admin-withdrawal-card__role"
                                title={wr.seller_id != null ? String(wr.seller_name || '') : wr.role_name || ''}
                              >
                                {wr.seller_id != null ? `Seller · ${wr.seller_name || `#${wr.seller_id}`}` : wr.role_name || 'Ishchi rol'}
                              </span>
                              <span className="admin-withdrawal-card__ids">
                                So‘rov #{wr.id}
                                {wr.work_role_id != null ? (
                                  ...
                                  <span className="admin-withdrawal-card__wrid">S #{wr.seller_id}</span>
                                ) : null}
                              </span>
                            </header>
                          </div>
                          <div className="admin-withdrawal-card__login">
                            {wr.seller_id != null ? wr.seller_name || 'Seller' : wr.work_role_login || '—'}
                          </div>
                          <ul className="admin-withdrawal-card__contacts">
                            {wr.seller_id != null ? (
                              <>
                                {wr.seller_phone ? (
                                  <li>
                                    <i className="fas fa-phone" aria-hidden />
                                    <span>{wr.seller_phone}</span>
                                  </li>
                                ) : null}
                                {wr.seller_email ? (
                                  <li>
                                    <i className="fas fa-envelope" aria-hidden />
                                    <span>{wr.seller_email}</span>
                                  </li>
                                ) : null}
                                {!wr.seller_phone && !wr.seller_email ? (
                                  <li className="admin-withdrawal-card__contacts--empty">Telefon / email yo‘q</li>
                                ) : null}
                              </>
                            ) : (
              ...
              {activeView === 'ai_calls' && (
                <>
                  {aiOperatorExpanded === 'sms' && (
                    <AdminTwilioOperatorPanel
                      settings={twilioSettings}
                      authDraft={twilioAuthDraft}
                      onAuthDraftChange={setTwilioAuthDraft}
                      onChangeField={(key, val) => {
                        setTwilioSettings((p) => ({ ...p, [key]: val }));
                      }}
                      saving={twilioSaving}
                      message={twilioPanelMsg}
                      sendTo={twilioSendTo}
                      sendBody={twilioSendBody}
                      onSendToChange={setTwilioSendTo}
                      onSendBodyChange={setTwilioSendBody}
                      sendBusy={twilioSendBusy}
                      onSave={() => void saveTwilioOperatorSettings()}
                      onSend={() => void sendTwilioOperatorSms()}
                    />
                  )}
                  ...
                  {aiOperatorExpanded === 'sms' && aiOperatorSmsSubTab === 'mijoz' && (
                    <div className="panel-card">
                      ...
                      <p className="muted" style={{ marginTop: 0, marginBottom: '1rem', fontSize: '0.92rem' }}>
                        Mijoz chatlari va <strong>Twilio</strong> orqali kelgan kiruvchi SMS lar. Yuqoridagi Twilio blokida webhook ni ulang.
                      </p>
                      <AdminAiOperatorFeedTable
                  ...
                  {aiOperatorExpanded === 'sms' && aiOperatorSmsSubTab === 'rollar' && (
                    <div className="panel-card">
                      ...
                      <p className="muted" style={{ marginTop: 0, marginBottom: '1rem', fontSize: '0.92rem' }}>
                        Kuryer yozuvlari, lead izohlari, hodim chatlari va <strong>Twilio</strong> orqali operator yuborgan chiquvchi SMS lar.
                      </p>
                      <AdminAiOperatorFeedTable
              ...
              {activeView === 'admin_profile' && (
                <div className="panel-card admin-profile-panel staff-account-touch">
                  <div className="panel-head">
                          ...
                          <td>
                            {adminProfileEditing ? (
                              <input
                                type="text"
                                className="neo-input"
                                value={adminProfileDraft.full_name}
                                onChange={(e) =>
                                  setAdminProfileDraft((d) => ({ ...d, full_name: e.target.value }))
                                }
                                autoComplete="name"
                                aria-label="Ism"
                              />
                          ...
                          <td>
                            {adminProfileEditing ? (
                              <input
                                ...
                                onChange={(e) =>
                                  setAdminProfileDraft((d) => ({ ...d, email: e.target.value }))
                                }
                                autoComplete="email"
                                aria-label="Email"
                              />
                          ...
                          <td>
                            {adminProfileEditing ? (
                              <input
                                type="text"
                                className="neo-input"
                                value={adminProfileDraft.login}
                                onChange={(e) =>
                                  setAdminProfileDraft((d) => ({
                                    ...d,
                  ...
                  {adminProfileErr ? (
                    <p className="admin-profile-save-err" role="alert">{adminProfileErr}</p>
                  ) : null}
                  <div className="admin-profile-card-footer">
                    <button
                      type="button"
                      className={`btn-neo admin-profile-footer-action-btn ${
                        adminProfileEditing ? 'admin-profile-footer-action-btn--save' : 'admin-profile-footer-action-btn--edit'
                      }`}
                      disabled={adminProfileSaveBusy}
                      onClick={async () => {
                        ...
                        setAdminProfileSaveBusy(true);
                        setAdminProfileErr('');
                        try {
                          await updateProfile({
                            full_name: adminProfileDraft.full_name.trim(),
                            email: adminProfileDraft.email.trim(),
                            login: adminProfileDraft.login.trim(),
                          });
                          setAdminProfileEditing(false);
                        } catch (err) {
                          setAdminProfileErr(err?.message || 'Saqlanmadi.');
                        } finally {
                          setAdminProfileSaveBusy(false);
                        }
                      }}
                    >
                      {adminProfileSaveBusy ? 'Saqlanmoqda…' : adminProfileEditing ? 'Saqlash' : 'Tahrirlash'}
                    </button>
                  </div>
                </div>
              )}
              ...
              {activeView === 'admin_settings' && (
                <div className="panel-card admin-settings-panel">
                  ...
                  <div className="admin-settings-body">
                    ...
                    {settingsMsg.text ? (
                      <div
                        className={settingsMsg.type === 'ok' ? 'admin-alert admin-alert--ok' : 'admin-alert'}
                        style={{ marginTop: 12 }}
                      >
                        {settingsMsg.text}
                      </div>
                    ...
                    <div className="admin-settings-appearance-card" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 12 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div className="admin-settings-appearance-icon" aria-hidden="true">
                          <i className="fas fa-phone" />
                        </div>
                        <div className="admin-settings-appearance-text">
                          <span className="admin-settings-appearance-title">Telefon raqami</span>
                          <span className="admin-settings-appearance-desc">Profilingizga telefon raqamini kiriting yoki yangilang</span>
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                        <input
                          ...
                          style={{ flex: '1 1 220px', minWidth: 0, height: 36, padding: '0 12px', borderRadius: 8, border: '1px solid var(--border, #e5e7eb)', fontSize: 14, lineHeight: '36px', boxSizing: 'border-box' }}
                          autoComplete="tel"
                        />
                        <button
                          type="button"
                          ...
                          style={{ height: 36, padding: '0 16px', borderRadius: 8, fontSize: 14, lineHeight: '36px', boxSizing: 'border-box' }}
                        >
                          {settingsBusy ? 'Saqlanmoqda…' : 'Saqlash'}
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="admin-settings-appearance-card" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 12 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <div className="admin-settings-appearance-text">
                          <span className="admin-settings-appearance-title">Parolni almashtirish</span>
                          <span className="admin-settings-appearance-desc">Avval joriy parol, so‘ng yangi parolni ikki marta kiriting</span>
                        </div>
                        <label style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13 }}>
                          <input
                            type="checkbox"
                            checked={settingsShowPwd}
                            onChange={(e) => setSettingsShowPwd(e.target.checked)}
                          />
                          Ko‘rsat
                        </label>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 8 }}>
                        <input
                          type={settingsShowPwd ? 'text' : 'password'}
                          className="form-control"
                          placeholder="Joriy parol"
                          value={settingsCurrentPwd}
                          onChange={(e) => setSettingsCurrentPwd(e.target.value)}
                          autoComplete="current-password"
                          style={{ height: 36, padding: '0 12px', borderRadius: 8, border: '1px solid var(--border, #e5e7eb)', fontSize: 14, lineHeight: '36px', boxSizing: 'border-box' }}
                        />
                        <input
                          type={settingsShowPwd ? 'text' : 'password'}
                          className="form-control"
                        ...
                        <input
                          ...
                          value={settingsNewPwd2}
                          onChange={(e) => setSettingsNewPwd2(e.target.value)}
                          autoComplete="new-password"
                          style={{ height: 36, padding: '0 12px', borderRadius: 8, border: '1px solid var(--border, #e5e7eb)', fontSize: 14, lineHeight: '36px', boxSizing: 'border-box' }}
                        />
                      </div>
                      <div>
                        <button
                          ...
                          style={{ height: 36, padding: '0 16px', borderRadius: 8, fontSize: 14, lineHeight: '36px', boxSizing: 'border-box' }}
                        >
                          {settingsBusy ? 'Almashtirilmoqda…' : 'Parolni almashtirish'}
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="admin-settings-appearance-card" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 12 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <div className="admin-settings-appearance-text">
                          <span className="admin-settings-appearance-title">Ma’lumot zaxirasi (backup)</span>
                          <span className="admin-settings-appearance-desc">
                            Zip ichida SQLite bazaning konsistent nusxasi (<code className="admin-ai-target-code">myshop.db</code>) va{' '}
                            <code className="admin-ai-target-code">uploads/</code> papkasi (mahsulot rasmlari, bannerlar, chat media va h.k.).
                            Buyurtmalar, foydalanuvchilar, audit yozuvlari bazada — ular shu zip orqali tiklanadi. Server{' '}
                            <code className="admin-ai-target-code">.env</code> dagi maxfiy kalitlar zipda yo‘q.
                          </span>
                        </div>
                      </div>
                      <p className="muted" style={{ margin: 0, fontSize: 13 }}>
                        Google Drive avtomatik yuklash:{' '}
                        {driveBackupStatus?.uploadConfigured ? (
                          <strong style={{ color: '#16a34a' }}>yoqilgan</strong>
                        ) : (
                          <span>o‘chiq (Render da env qo‘ying — pastdagi qo‘llanma).</span>
                        )}
                      </p>
                      <details className="admin-backup-drive-hint" style={{ fontSize: 13 }}>
                        <summary style={{ cursor: 'pointer', color: 'var(--text-muted, #64748b)' }}>
                          Google Drive ni ulash (service account)
                        </summary>
                        <ol style={{ margin: '8px 0 0', paddingLeft: 18, lineHeight: 1.5 }}>
                          <li>Google Cloud → APIs → Drive API ni yoqing.</li>
                          <li>Service Account yarating, JSON kalitni yuklab oling.</li>
                          <li>Drive da papka oching → papkani service account email bilan «Tahrirlovchi» sifatida ulashing.</li>
                          <li>
                            Render → Environment:{' '}
                            <code className="admin-ai-target-code">MYSHOP_GOOGLE_DRIVE_FOLDER_ID</code> (papka URL dan), va{' '}
                            <code className="admin-ai-target-code">MYSHOP_GOOGLE_SERVICE_ACCOUNT_JSON</code> yoki{' '}
                            <code className="admin-ai-target-code">MYSHOP_GOOGLE_SERVICE_ACCOUNT_JSON_BASE64</code>.
                          </li>
                        </ol>
                      </details>
                      {backupPanelMsg.text ? (
                        <div
                          className={backupPanelMsg.type === 'ok' ? 'admin-alert admin-alert--ok' : 'admin-alert'}
                          style={{ margin: 0 }}
                        >
                          {backupPanelMsg.text}
                        </div>
                      ...
                      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                        <button
                          type="button"
                          className="btn btn-primary"
                          disabled={backupPanelBusy}
                          onClick={() => void handleBackupRun()}
                          style={{ height: 36, padding: '0 16px', borderRadius: 8, fontSize: 14, lineHeight: '36px', boxSizing: 'border-box' }}
                        >
                          {backupPanelBusy ? 'Yaratilmoqda…' : 'Backup yaratish'}
                        </button>
                        <button
                          type="button"
                          className="btn-neo"
                          disabled={backupListLoading || driveRemoteLoading || backupPanelBusy}
                          onClick={() => void refreshBackupSection()}
                          style={{ height: 36, padding: '0 14px', borderRadius: 8, fontSize: 14 }}
                        >
                          Ro‘yxatni yangilash
                        </button>
                      </div>
                      <p className="muted" style={{ margin: 0, fontSize: 13 }}>
                        Bir nusxa Render persistent diskda, Drive sozlangan bo‘lsa ikkinchi nusxa bulutda. Zip ichidagi ma’lumotlarni kompyuteringizga yuklab, ochib ko‘rishingiz yoki SQLite ni tiklash uchun qayta joylashtirishingiz mumkin.
                      </p>
                      <h5 style={{ margin: '1rem 0 0.35rem', fontSize: '0.95rem', fontWeight: 600 }}>Render disk</h5>
                      {backupListLoading ? (
                        <p className="muted" style={{ margin: 0 }}>Backup ro‘yxati yuklanmoqda…</p>
                      ) : backupList.length === 0 ? (
                        <p className="muted" style={{ margin: 0 }}>Hozircha backup fayllari yo‘q.</p>
                      ) : (
                        <div className="table-wrap compact">
                          <table className="neo-table compact">
                            <thead>
                              ...
                              {backupList.map((b) => {
                                ...
                                return (
                                  <tr key={b.name}>
                                    ...
                                    <td>{sizeLabel}</td>
                                    <td>{b.mtime ? formatDateTimeUz(b.mtime, { empty: '—' }) : '—'}</td>
                                  </tr>
                                );
                              })}
                            </tbody>
                      ...
                      <h5 style={{ margin: '1.25rem 0 0.35rem', fontSize: '0.95rem', fontWeight: 600 }}>Google Drive (bulut)</h5>
                      {driveRemoteLoading ? (
                        <p className="muted" style={{ margin: 0 }}>Drive ro‘yxati yuklanmoqda…</p>
                      ) : !driveBackupStatus?.uploadConfigured ? (
                        <p className="muted" style={{ margin: 0 }}>
                          Drive ulanmagan — yuqoridagi env lar bilan yoqing; keyin zip lar avtomatik shu yerga ham tushadi.
                        </p>
                      ) : driveRemoteFiles.length === 0 ? (
                        <p className="muted" style={{ margin: 0 }}>Drive papkasida zip hali yo‘q.</p>
                      ) : (
                        <div className="table-wrap compact">
                          <table className="neo-table compact">
                            <thead>
                              ...
                              {driveRemoteFiles.map((f) => {
                                ...
                                return (
                                  <tr key={f.id}>
                                    ...
                                    <td>{sizeLabel}</td>
                                    <td>{f.createdTime ? formatDateTimeUz(f.createdTime, { empty: '—' }) : '—'}</td>
                                    <td style={{ whiteSpace: 'nowrap' }}>
                                      ...
                                      <button
                                        type="button"
                                        className="btn-neo btn-neo-danger"
                                        disabled={driveDeleteBusyId === f.id}
                                        onClick={() => void handleDriveBackupDelete(f.id)}
                                        style={{ padding: '4px 10px', fontSize: 12 }}
                                      >
                                        {driveDeleteBusyId === f.id ? '…' : 'O‘chirish'}
                                      </button>
                                    </td>
                                  </tr>
                                );
              ...
              {(activeView === 'atkaz_products' || activeView === 'brak_products' || activeView === 'warehouse_kirim_chiqim') &&
                (() => {
                  if (activeView === 'warehouse_kirim_chiqim') {
                    const baseList = warehouseProductsOverview || [];
                    const noopReload = async () => {};
                    const readOnlyBusy = {
                      approvingId: null,
                      confirmingChiqimId: null,
                      confirmingAtkazId: null,
                      revokingKirimId: null,
                      delistingId: null,
                      deletingProductId: null,
                    };
                    return (
                      <div className="panel-card admin-warehouse-kirim-chiqim-card">
                        <div className="panel-head">
                          <h4>
                            <i className="fas fa-right-left" /> Ombor kirim va chiqim{' '}
                            <span className="muted" style={{ fontWeight: 400 }}>
                              ({baseList.length})
                            </span>
                          </h4>
                        </div>
                        <div className="warehouse-admin-sheet-unified admin-warehouse-kirim-chiqim-sheet">
                          <div className="warehouse-admin-sheet-unified-inner">
                            ...
                            <div className="warehouse-admin-grid-rows warehouse-admin-grid-rows--sheet" role="grid">
                              ...
                              {!warehouseProductsLoading && baseList.length === 0 && (
                                <p className="warehouse-admin-grid-row-empty">Mahsulot yo&apos;q.</p>
                              )}
                              {!warehouseProductsLoading &&
                                baseList.map((p) => (
                                  <div key={p.id} className="warehouse-admin-sheet-row-group">
                                    <div
                                      className={`warehouse-admin-grid-row warehouse-admin-grid-row--ledger${warehouseKirimChiqimSheetMainRowClass('kirim_chiqim', p)}`}
                                      role="row"
                  ...
                  const cfg =
                    activeView === 'atkaz_products'
                      ? {
                          icon: 'fa-ban',
                          title: 'Atkaz mahsulot',
                          intro:
                            'Ombor admini atkaz sonini kiritib tasdiqlagandan keyin bu yerda ko‘rinadi (buyurtmalar bo‘yicha ko‘rsatkich alohida).',
                          emptyText: 'Tasdiqlangan atkaz yozuvi yo‘q.',
                          tone: '#dc2626',
                        }
                      : {
                          icon: 'fa-triangle-exclamation',
                          title: 'Brak mahsulot',
                          intro: 'Brak soni tasdiqlangach bu ro‘yxatga chiqadi.',
                          emptyText: 'Tasdiqlangan brak yozuvi yo‘q.',
                          tone: '#d97706',
                        };
                  const baseList = warehouseProductsOverview || [];
                  const list =
                    activeView === 'atkaz_products'
                      ? baseList.filter(
                          (p) =>
                            Boolean(p.warehouse_atkaz_confirmed_at) && Number(p.warehouse_atkaz_qty) > 0,
                        )
                      : baseList.filter(
                          (p) => Boolean(p.warehouse_brak_confirmed_at) && Number(p.brak_qty) > 0,
                        );
                  const displayQty = (p) => {
                    if (activeView === 'atkaz_products') return Number(p.warehouse_atkaz_qty) || 0;
                    return Number(p.brak_qty) || 0;
                  };
                  return (
                    <div className="panel-card">
                      <div className="panel-head">
                      ...
                      <p className="muted" style={{ padding: '0.25rem 0 0.75rem' }}>
                        {cfg.intro}
                      </p>
                      {warehouseProductsLoading && <p className="muted">Yuklanmoqda…</p>}
                      {!warehouseProductsLoading && list.length === 0 && <p className="muted">{cfg.emptyText}</p>}
                      {!warehouseProductsLoading && list.length > 0 && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                          {list.map((p) => (
                            ...
                            >
                              {p.image_url ? (
                                <img src={p.image_url} alt="" style={{ width: 44, height: 44, objectFit: 'cover', borderRadius: 6 }} />
                              ) : (
                                <span style={{ width: 44, height: 44, borderRadius: 6, background: '#f1f5f9', display: 'inline-block' }} />
                              )}
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <div style={{ fontSize: 12, opacity: 0.7 }}>
                                  Sotuvchi: {p.seller_name || '—'} · Ombor: {Number(p.stock) || 0}
                                </div>
                              </div>
                              <div style={{ fontWeight: 700, color: cfg.tone, minWidth: 48, textAlign: 'right' }}>
                                {displayQty(p)}
                              </div>
                            </div>
                          ))}
              ...
              {activeView === 'reklama' && (
                ...
                <div className="panel-card admin-reklama-panel">
                  ...
                  <p className="admin-reklama-lead muted">
                    Bosh sahifadagi slayder (1–5). Desktop uchun + rasm (JPEG/PNG/WebP; tavsiya ~1920px). Mobil uchun ixtiyoriy alohida rasm (tor/portret — telefon ekranida crop kamroq). Film — MP4 yoki WebM; YouTube havolasi mumkin. Yuklangan fayllar bazaga yoziladi; «Saqlash» bilan tasdiqlang. Video hajmi: 80 MB gacha.
                  </p>
                    ...
                    onSubmit={async (e) => {
                      ...
                      try {
                        if (adSlideFormDetached) {
                          ...
                          const imgMob0 = adSlideRemoveImageMobile ? '' : adSlideForm.image_url_mobile.trim();
                        ...
                        const nextImageMobileUrl = adSlideRemoveImageMobile ? '' : adSlideForm.image_url_mobile.trim();
                        ...
                        if (selectedIdxs.length === 1) {
                          ...
                          if (slide?.id) {
                            const patchBody = {
                              ...
                              image_url_mobile: nextImageMobileUrl,
                            ...
                            if (nextImageMobileUrl) postBody.image_url_mobile = nextImageMobileUrl;
                  ...
                  >
                    <div className="admin-reklama-form-grid">
                      ...
                      <label className="admin-reklama-span-2">
                        <span>Mobil banner rasmi (ixtiyoriy, telefon ≤768px): portret/vertical — desktop rasmdan alohida</span>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
                          <input
                            type="url"
                            className="neo-input"
                            style={{ flex: '1 1 240px', minWidth: 0 }}
                            value={adSlideForm.image_url_mobile}
                            onChange={(ev) => setAdSlideForm((p) => ({ ...p, image_url_mobile: ev.target.value }))}
                            placeholder="/api/uploads/ad-slides/... yoki tashqi https://"
                            maxLength={800}
                          />
                          <button
                            type="button"
                            className="btn-neo btn-sm"
                            disabled={Boolean(adSlideBusy)}
                            onClick={() => document.getElementById('ad-reklama-mobile-file')?.click()}
                          >
                            Yuklash
                          </button>
                          <input
                            id="ad-reklama-mobile-file"
                            type="file"
                            ...
                            onChange={async (ev) => {
                              ...
                              try {
                                ...
                                const ud = up.ok ? await up.json().catch(() => ({})) : {};
                                if (!up.ok) throw new Error(ud.error || 'Rasm yuklanmadi');
                                const url = String(ud.url || '').trim();
                                if (!url) throw new Error('URL olinmadi');
                                setAdSlideForm((p) => ({ ...p, image_url_mobile: url }));
                                setAdSlideRemoveImageMobile(false);
                              } catch (err) {
                        ...
                        {adSlidePreviewMobile ? (
                          <div className="admin-reklama-form-preview" style={{ maxWidth: 220, marginTop: 8 }}>
                            <img src={adSlidePreviewMobile} alt="" />
                          </div>
                        ) : null}
                        {adSlidePreviewMobile && adSlideEditingId && (adSlideFormDetached || adSlideSelectedIndices.length === 1) ? (
                          <button
                            type="button"
                            className="btn-neo btn-sm admin-reklama-remove-img"
                            onClick={() => {
                              setAdSlideRemoveImageMobile(true);
                              setAdSlideForm((p) => ({ ...p, image_url_mobile: '' }));
                            }}
                          >
                            Mobil rasmini olib tashlash
                          </button>
                        ) : null}
                      </label>
                      <label className="admin-reklama-span-2">
                      ...
                      {adSlides.map((s, idx) => (
                        <div key={s.id} className={`admin-reklama-card ${s.active ? '' : 'is-inactive'}`}>
                          <div className="admin-reklama-card-main">
                            {s.image_url || s.image_url_mobile ? (
                              <div className="admin-reklama-thumb">
                                <img src={s.image_url || s.image_url_mobile} alt="" onError={(ev) => { ev.target.style.display = 'none'; }} />
                ...
                <div className="panel-card admin-footer-strip-panel">
                  <div className="panel-head">
                    <h4><i className="fas fa-window-maximize" /> Footer — yuqori lenta</h4>
                  </div>
                  <p className="admin-reklama-lead muted">
                    Barcha mijoz sahifalaridagi footer ustidagi ko‘k chiziqli blok (reklama matni + telefon).
                    <strong> Saqlash</strong> faqat qoralamani bazaga yozadi; <strong>E&apos;lon qilish</strong> qoralamani jonli saytga chiqaradi.
                  </p>
                  {footerStripNote ? (
                    <p className="admin-footer-strip-note">{footerStripNote}</p>
                  ) : null}
                  {footerStripLoading ? (
                    <p className="muted">Yuklanmoqda…</p>
                  ) : (
                    <div className="admin-footer-strip-grid">
                      <label className="admin-footer-strip-span">
                        <span>Qoralama — reklama matni</span>
                        <textarea
                          className="neo-input admin-footer-strip-textarea"
                          rows={3}
                          maxLength={500}
                          value={footerDraftText}
                          onChange={(ev) => setFooterDraftText(ev.target.value)}
                          placeholder="Masalan: Bepul yetkazib berish — …"
                        />
                      </label>
                      <label className="admin-footer-strip-span">
                        <span>Qoralama — telefon</span>
                        <input
                          type="text"
                          className="neo-input"
                          maxLength={80}
                          value={footerDraftPhone}
                          onChange={(ev) => setFooterDraftPhone(ev.target.value)}
                          placeholder="+998 …"
                        />
                      </label>
                      <div className="admin-footer-strip-span admin-footer-strip-live muted">
                        <strong style={{ color: 'var(--text, #111)' }}>Saytda hozir (jonli):</strong>
                        <div style={{ marginTop: 6 }}>{footerLive.promo_text || '—'}</div>
                        <div style={{ marginTop: 4, fontWeight: 600 }}>{footerLive.phone || '—'}</div>
                      </div>
                      <div className="admin-footer-strip-actions">
                        <button
                          type="button"
                          className="btn-neo btn-neo-primary"
                          disabled={Boolean(footerStripBusy)}
                          onClick={async () => {
                            setFooterStripBusy('save');
                            setError('');
                            setFooterStripNote('');
                            try {
                              const res = await request('/admin/portal/footer-strip/draft', {
                                method: 'PATCH',
                                body: JSON.stringify({
                                  promo_text: footerDraftText,
                                  phone: footerDraftPhone,
                                }),
                              });
                              const d = res.ok ? await res.json().catch(() => ({})) : {};
                              if (!res.ok) throw new Error(d.error || 'Saqlanmadi');
                              ...
                              setFooterDraftText(String(draft.promo_text ?? ''));
                              setFooterDraftPhone(String(draft.phone ?? ''));
                              setFooterStripNote('Qoralama saqlandi. Saytga chiqarish uchun «Eʼlon qilish»ni bosing.');
                            } catch (err) {
                              setError(err.message || 'Saqlanmadi');
                            } finally {
                              setFooterStripBusy(null);
                            }
                          }}
                        >
                          Saqlash (qoralama)
                        ...
                        <button
                          ...
                          disabled={Boolean(footerStripBusy)}
                          onClick={async () => {
                            setFooterStripBusy('publish');
                            setError('');
                            setFooterStripNote('');
                            try {
                              const res = await request('/admin/portal/footer-strip/publish', { method: 'POST' });
                              const d = res.ok ? await res.json().catch(() => ({})) : {};
                              if (!res.ok) throw new Error(d.error || 'Eʼlon qilinmadi');
                              const live = d.live || {};
                              const draft = d.draft || {};
                              setFooterLive({
                                promo_text: String(live.promo_text || ''),
                              ...
                              setFooterDraftText(String(draft.promo_text ?? ''));
                              setFooterDraftPhone(String(draft.phone ?? ''));
                              setFooterStripNote('Jonli sayt yangilandi. Bosh sahifani yangilab tekshiring.');
                            } catch (err) {
                              setError(err.message || 'Eʼlon qilinmadi');
                            } finally {
                              setFooterStripBusy(null);
                            }
                          }}
                ...
                <div className="panel-card admin-home-benefits-panel">
                  <div className="panel-head">
                    <h4><i className="fas fa-th-large" /> MyShop qulayliklari</h4>
                  </div>
                  <p className="admin-reklama-lead muted">
                    Bosh sahifaning pastki qismidagi 4 ta kartochka va ularning ustidagi sarlavha.
                    Ikonlar va ranglar dasturda qoladi — faqat matnlarni tahrirlaysiz.
                    <strong> Saqlash</strong> qoralama; <strong>E&apos;lon qilish</strong> jonli saytga chiqaradi.
                  </p>
                  {hbNote ? <p className="admin-footer-strip-note">{hbNote}</p> : null}
                  {hbLoading ? (
                    <p className="muted">Yuklanmoqda…</p>
                  ) : (
                    <div className="admin-home-benefits-inner">
                      <label className="admin-home-benefits-span">
                        <span>Qoralama — bo‘lim sarlavhasi</span>
                        <input
                          type="text"
                          className="neo-input"
                          maxLength={200}
                          value={hbDraftSectionTitle}
                          onChange={(ev) => setHbDraftSectionTitle(ev.target.value)}
                          placeholder="MyShop qulayliklari"
                        />
                      </label>
                      {['Yetkazib berish', 'To‘lov', 'Qo‘llab-quvvatlash', 'Rag‘batlantirish'].map((hint, idx) => (
                        <div key={`hb-slot-${hint}`} className="admin-home-benefits-band">
                          <div className="admin-home-benefits-band-head muted">{idx + 1}-kartochka — {hint}</div>
                          <label className="admin-home-benefits-span">
                            <span>Sarlavha</span>
                            <input
                              type="text"
                              className="neo-input"
                              maxLength={400}
                              value={hbDraftItems[idx]?.title ?? ''}
                              onChange={(ev) =>
                                setHbDraftItems((prev) => {
                                  const next = [...prev];
                          ...
                          <label className="admin-home-benefits-span">
                            ...
                            <textarea
                              ...
                              rows={3}
                              maxLength={1200}
                              value={hbDraftItems[idx]?.text ?? ''}
                              onChange={(ev) =>
                                setHbDraftItems((prev) => {
                                  const next = [...prev];
                                  next[idx] = { ...next[idx], text: ev.target.value };
                                  return next;
                                })
                              }
                            />
                      ...
                      <div className="admin-home-benefits-live muted">
                        <strong style={{ color: 'var(--text, #111)' }}>Saytda hozir (jonli):</strong>
                        <div style={{ marginTop: 8, fontWeight: 700 }}>{hbLive.section_title || '—'}</div>
                        <ul className="admin-home-benefits-live-list">
                          {(hbLive.items || []).map((it, i) => (
                            <li key={`hbl-${i}`}>
                              <span className="admin-home-benefits-live-i">{i + 1}.</span>{' '}
                      ...
                      <div className="admin-footer-strip-actions">
                        <button
                          type="button"
                          className="btn-neo btn-neo-primary"
                          disabled={Boolean(hbBusy)}
                          onClick={async () => {
                            setHbBusy(true);
                            setError('');
                            setHbNote('');
                            try {
                              const res = await request('/admin/portal/home-benefits/draft', {
                                method: 'PATCH',
                                body: JSON.stringify({
                                  section_title: hbDraftSectionTitle,
                                  items: hbDraftItems,
                                }),
                              });
                              const d = res.ok ? await res.json().catch(() => ({})) : {};
                              ...
                              setHbNote('Qoralama saqlandi. Saytga chiqarish uchun «Eʼlon qilish»ni bosing.');
                            } catch (err) {
                              setError(err.message || 'Saqlanmadi');
                            } finally {
                              setHbBusy(false);
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            setHbNote('');
                            try {
                              const res = await request('/admin/portal/home-benefits/publish', { method: 'POST' });
                              const d = res.ok ? await res.json().catch(() => ({})) : {};
                              if (!res.ok) throw new Error(d.error || 'Eʼlon qilinmadi');
                              const live = d.live || {};
                              const draft = d.draft || {};
                              ...
                              setHbNote('Jonli sayt yangilandi. Bosh sahifani yangilab tekshiring.');
                            } catch (err) {
                              setError(err.message || 'Eʼlon qilinmadi');
                            } finally {
                              setHbBusy(false);
            ...
            <div className="modal-body-neo">
              ...
              <div className="role-modal-portal-panel" style={{ marginTop: '1rem' }}>
                ...
                <div className="role-create-fields">
                  <label className="role-create-label role-create-label-full">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (!REGION_SCOPED_PORTAL_ROLES.has(v)) {
                  ...
                  {REGION_SCOPED_PORTAL_ROLES.has(rolePortalDraft) ? (
                    <>
                      <label className="role-create-label role-create-label-full">
                        <span>{rolePortalDraft === 'courier' ? 'Viloyat / shahar' : 'Ekspeditor hududi — viloyat / shahar (ixtiyoriy, bo\'sh qoldirilsa — butun O\'zbekiston)'}</span>
                        <select
                          className="role-create-input"

...
const HIDDEN_VIEW_KEYS = ['seller_products', 'customer_detail', 'admin_profile', 'admin_settings', 'recent_users'];
...
export default function AdminDashboard() {
  ...
  const handleCreateRole = async (e) => {
    ...
    if (roleForm.portal_role === 'expeditor' && !String(roleForm.courier_viloyat_id || '').trim()) {
      setError('Ekspeditor uchun hudud (viloyat) tanlang.');
      return;
    }
  ...
  const handleSaveRolePortal = async () => {
    ...
    if (rolePortalDraft === 'expeditor' && !String(roleCourierViloyatDraft || '').trim()) {
      window.alert('Ekspeditor uchun hudud (viloyat) tanlang.');
      return;
    }
                          ...
                          <section className="role-create-section">
                            ...
                            <div className="role-create-fields">
                              ...
                              {REGION_SCOPED_PORTAL_ROLES.has(roleForm.portal_role) ? (
                                <>
                                  <label className="role-create-label role-create-label-full">
                                    <span>{roleForm.portal_role === 'courier' ? 'Kuryer hududi — viloyat / shahar' : 'Ekspeditor hududi — viloyat / shahar'}</span>
                                    <select
                                      className="role-create-input"
                            ...
                            const tumanLabel =
                              ...
                              role.courier_tuman_ids.length > 0
                                ? role.courier_tuman_ids
                                    ...
                                    .join(', ')
                                : '';
                            const hududLabel = REGION_SCOPED_PORTAL_ROLES.has(role.portal_role) ? `${viloyatLabel}${tumanLabel ? ` · ${tumanLabel}` : ''}` : '—';
                            const portalLabel =
                    ...
                    <div className="roles-archive-head">
                      <div>
                        ...
                        <p className="muted roles-archive-hint">
                          O‘chirilgan ishchi rollar faqat shu yerda saqlanadi. Tiklash yoki moliya tarixi bo‘lmasa butunlay o‘chirish mumkin.
                        </p>
                      </div>
                      {trashRoles.length > 0 && (
              ...
              <div className="role-modal-portal-panel" style={{ marginTop: '1rem' }}>
                ...
                <div className="role-create-fields">
                  ...
                  {REGION_SCOPED_PORTAL_ROLES.has(rolePortalDraft) ? (
                    <>
                      <label className="role-create-label role-create-label-full">
                        <span>{rolePortalDraft === 'courier' ? 'Viloyat / shahar' : 'Ekspeditor hududi — viloyat / shahar'}</span>
                        <select
                          className="role-create-input"

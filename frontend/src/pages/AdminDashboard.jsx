...
const MENU_GROUPS = [
  {
    ...
    items: [
      ...
      { key: 'warehouse_kirim', icon: 'fa-truck-ramp-box', label: 'Omborga kirim' },
      { key: 'warehouse_chiqim', icon: 'fa-truck-arrow-right', label: 'Ombordan chiqim' },
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    if (!['atkaz_products', 'brak_products', 'warehouse_kirim', 'warehouse_chiqim'].includes(activeView)) return;
    let cancelled = false;
    setWarehouseProductsLoading(true);
    request('/warehouse-admin/products-overview')
      .then((res) => (res.ok ? res.json() : { products: [] }))
      .then((data) => {
        if (cancelled) return;
        ...
        <section className={`admin-content${activeView === 'staff_chat' ? ' admin-content--staff-chat' : ''}`}>
          ...
          {activeView === 'staff_chat' ? (
            ...
            <>
              ...
              {(activeView === 'atkaz_products' || activeView === 'brak_products'
                || activeView === 'warehouse_kirim' || activeView === 'warehouse_chiqim') && (() => {
                const cfg = {
                  atkaz_products: {
                    icon: 'fa-ban', title: 'Atkaz mahsulot',
                    intro: 'Atkaz qilingan (mijoz qabul qilmagan / qaytarilgan) mahsulotlar — buyurtmalar holati asosida.',
                    field: 'atkaz_soni', emptyText: 'Atkaz qilingan mahsulot yo‘q.', tone: '#dc2626',
                  },
                  brak_products: {
                    icon: 'fa-triangle-exclamation', title: 'Brak mahsulot',
                    intro: 'Brak (yaroqsiz / shikastlangan) sifatida belgilangan mahsulotlar.',
                    field: 'brak_qty', emptyText: 'Brak mahsulot yo‘q.', tone: '#d97706',
                  },
                  warehouse_kirim: {
                    icon: 'fa-truck-ramp-box', title: 'Omborga kirim',
                    intro: 'Skladga kelgan mahsulotlar bo‘yicha kirim yozuvlari (hozircha mahsulot bo‘yicha jami kirim soni).',
                    field: 'kirim_soni', emptyText: 'Kirim yozuvlari yo‘q.', tone: '#16a34a',
                  },
                  warehouse_chiqim: {
                    icon: 'fa-truck-arrow-right', title: 'Ombordan chiqim',
                    intro: 'Ombordan chiqarilgan mahsulotlar — buyurtmalar bo‘yicha chiqim hisoblari.',
                    field: 'chiqim_soni', emptyText: 'Chiqim yozuvlari yo‘q.', tone: '#2563eb',
                  },
                }[activeView];
                const list = (warehouseProductsOverview || []).filter((p) => Number(p[cfg.field]) > 0);
                return (
                  <div className="panel-card">
                    <div className="panel-head">
                      <h4><i className={`fas ${cfg.icon}`} /> {cfg.title} <span className="muted" style={{ fontWeight: 400 }}>({list.length})</span></h4>
                    </div>
                    <p className="muted" style={{ padding: '0.25rem 0 0.75rem' }}>{cfg.intro}</p>
                    {warehouseProductsLoading && <p className="muted">Yuklanmoqda…</p>}
                    {!warehouseProductsLoading && list.length === 0 && (
                      <p className="muted">{cfg.emptyText}</p>
                    )}
                    {!warehouseProductsLoading && list.length > 0 && (

      ...
      <form onSubmit={handleSubmit} className="card checkout-form">
        ...
        <div className="form-group">
          ...
          <div className="checkout-payment-methods">
            {[
              { id: 'cash', label: 'Naqd pul', icon: 'fa-money-bill-wave' },
              { id: 'payme', label: 'Payme', icon: 'fa-credit-card' },
              { id: 'click', label: 'Click', icon: 'fa-mobile-alt' },
            ].map((m) => (
              <button
                key={m.id}
                type="button"
                className={`checkout-payment-option${paymentMethod === m.id ? ' is-active' : ''}`}
                onClick={() => setPaymentMethod(m.id)}
              >
                <i className={`fas ${m.icon}`} aria-hidden />
                <span>{m.label}</span>
                {m.id !== 'cash' && (
                  <span className="checkout-payment-badge">Tez orada</span>
                )}
              </button>
            ))}
          </div>
          {paymentMethod !== 'cash' && (
            <p className="checkout-payment-note">
              <i className="fas fa-info-circle" /> {paymentMethod === 'payme' ? 'Payme' : 'Click'} to'lovi tez orada ulash rejalashtirilgan. Hozircha buyurtma berish mumkin — operator siz bilan bog'lanadi.
            </p>
          )}
        </div>

...
import { getApiPrefix, safeResponseJson } from '../utils/apiBase.js';
...
const FALLBACK_AD_SLIDES = [
  { id: 1, title: 'Yangi kelganlar', text: 'Eng so\'nggi mahsulotlar do\'konimizda' },
  { id: 2, title: 'Chegirmalar', text: 'Aksiyali narxlardan bahramand bo\'ling' },
  { id: 3, title: 'Bepul yetkazib berish', text: '500 000 so\'mdan ortiq buyurtmalarda' },
  { id: 4, title: 'Tez yetkazib berish', text: 'Buyurtmangiz 1–3 kun ichida' },
  { id: 5, title: 'Kafolat', text: 'Sifat kafolati va qaytarish imkoniyati' },
  { id: 6, title: 'MyShop', text: 'Xavfsiz va qulay onlayn do\'kon' },
];
...
export default function Home() {
      ...
      .then(async (r) => {
        if (!r.ok) {
          if (import.meta.env.DEV) {
            console.warn('[MyShop] /products', r.status, String(API).slice(0, 80));
          }
          return { products: [] };
        }
        try {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`${API}/ad-slides`)
      .then((r) => safeResponseJson(r, { slides: [] }))
      .then((d) => {
        if (cancelled) return;
        setAdSlidesRemote(Array.isArray(d?.slides) ? d.slides : []);
      })
      .catch(() => {
        if (!cancelled) setAdSlidesRemote(null);
      });
    return () => {
      cancelled = true;
    };

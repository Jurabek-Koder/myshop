import React, { useState, useEffect, useMemo, useRef } from 'react';
import { Link } from 'react-router-dom';
import { useCart } from '../context/CartContext';
import { API_PREFIX, parseApiJsonText } from '../lib/apiBase';
/** Kartochka rangi va ikonlari (matnlar API dan) */
const HOME_BENEFIT_SLOTS = [
  { accent: 'delivery', icon: 'fa-truck-fast' },
  { accent: 'payment', icon: 'fa-credit-card' },
  { accent: 'support', icon: 'fa-comments' },
  { accent: 'loyalty', icon: 'fa-gift' },
];

const HOME_BENEFITS_TEXT_FALLBACK = [
  {
    title: 'Tezkor yetkazib berish xizmati',
    text: "Buyurtmangiz O'zbekistonning ko'plab hududlariga 1–3 kun ichida yetkazib beriladi.",
  },
  {
    title: "To'lov istalgan usulda",
    text: "Buyurtmani oldindan Click, Payme orqali yoki buyurtmani qo'lingizga olganingizdan keyin amalga oshiring.",
  },
  {
    title: 'Qo‘llab-quvvatlash',
    text: "Savollar bo'yicha yordam: +998 71 123 45 67. Telegram orqali ham yozishingiz mumkin.",
  },
  {
    title: "Mijozlarni rag'batlantirish tizimi",
    text: 'Doimiy mijozlar uchun aksiyalar, chegirmalar va maxsus takliflar.',
  },
];

function buildBenefitsFallbackBlock() {
  return {
    section_title: 'MyShop qulayliklari',
    items: HOME_BENEFIT_SLOTS.map((slot, i) => ({
      accent: slot.accent,
      icon: slot.icon,
      title: HOME_BENEFITS_TEXT_FALLBACK[i].title,
      text: HOME_BENEFITS_TEXT_FALLBACK[i].text,
    })),
  };
}

/** `public/images` — `/images/...`; API ishlamaganda ham banner ko‘rinsin */
const FALLBACK_AD_SLIDES = [
  { id: 1, title: 'Yangi kelganlar', text: 'Eng so\'nggi mahsulotlar do\'konimizda', image: '/images/atir.webp' },
  { id: 2, title: 'Chegirmalar', text: 'Aksiyali narxlardan bahramand bo\'ling', image: '/images/blender.webp' },
  { id: 3, title: 'Bepul yetkazib berish', text: '500 000 so\'mdan ortiq buyurtmalarda', image: '/images/espander-universalnyy-168033-1.jpeg' },
  { id: 4, title: 'Tez yetkazib berish', text: 'Buyurtmangiz 1–3 kun ichida', image: '/images/photo_2026-03-22_18-25-13.jpg' },
  { id: 5, title: 'Kafolat', text: 'Sifat kafolati va qaytarish imkoniyati', image: '/images/photo_2026-03-22_18-26-16.jpg' },
  { id: 6, title: 'MyShop', text: 'Xavfsiz va qulay onlayn do\'kon', image: '/images/photo_2026-03-22_18-26-37.jpg' },
];

function youtubeVideoId(url) {
  const s = String(url || '');
  const m = s.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([^&\s?/#]+)/i);
  return m ? m[1] : null;
}

function resolveAdSlideAssetUrl(raw) {
  const s = String(raw || '').trim();
  if (!s) return '';
  if (/^https?:\/\//i.test(s) || s.startsWith('data:')) return s;
  if (s.startsWith('/api/')) {
    const base = (import.meta.env.VITE_API_BASE_URL || '').replace(/\/$/, '');
    return base ? `${base}${s}` : s;
  }
  if (s.startsWith('/')) return s;
  return s;
}

function AdSliderSlide({ slide, isActive }) {
  const videoRef = useRef(null);
  const v = resolveAdSlideAssetUrl(slide.video_url);
  const img = resolveAdSlideAssetUrl(slide.image_url);
  const imgMob = resolveAdSlideAssetUrl(slide.image_url_mobile);
  const desktopImg = img || imgMob;
  const yid = v ? youtubeVideoId(v) : null;
  const showVideo = Boolean(v);
  const showImage = Boolean(desktopImg) && !showVideo;
  const hasMedia = showVideo || Boolean(desktopImg);

  useEffect(() => {
    const el = videoRef.current;
    if (!el || yid) return;
    if (isActive) {
      el.play().catch(() => {});
    } else {
      el.pause();
      try {
        el.currentTime = 0;
      } catch {
        /* ignore */
      }
    }
  }, [isActive, yid, v, slide.id]);

  const caption = (
    <div className="ad-slider-slide-caption">
      <h3>{slide.title}</h3>
      {slide.text ? <p>{slide.text}</p> : null}
    </div>
  );

  const mediaBlock = (
    <div className="ad-slider-slide-media" aria-hidden={!hasMedia}>
      {showVideo && yid ? (
        <iframe
          key={`${slide.id}-${isActive ? 'on' : 'off'}`}
          className="ad-slider-slide-iframe"
          src={
            isActive
              ? `https://www.youtube.com/embed/${yid}?autoplay=1&mute=1&playsinline=1&rel=0`
              : `https://www.youtube.com/embed/${yid}?mute=1&playsinline=1&rel=0`
          }
          title={slide.title || 'Reklama videosi'}
          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
          allowFullScreen
        />
      ) : null}
      {showVideo && !yid ? (
        <video
          ref={videoRef}
          className="ad-slider-slide-video"
          src={v}
          poster={img || undefined}
          muted
          playsInline
          loop
          controls
          preload="auto"
        />
      ) : null}
      {showImage ? (
        img && imgMob && img !== imgMob ? (
          <picture className="ad-slider-slide-picture">
            <source media="(max-width: 767px)" srcSet={imgMob} />
            <img
              src={img}
              alt=""
              className="ad-slider-slide-img--fill"
              decoding="async"
              fetchPriority={isActive ? 'high' : 'low'}
            />
          </picture>
        ) : (
          <img
            src={desktopImg}
            alt=""
            className="ad-slider-slide-img--fill"
            decoding="async"
            fetchPriority={isActive ? 'high' : 'low'}
          />
        )
      ) : null}
    </div>
  );

  const textOnlyInner = (
    <>
      <div className="ad-slider-slide-fallback" />
      <div className="ad-slider-slide-textonly">
        <h3>{slide.title}</h3>
        {slide.text ? <p style={{ margin: 0, color: 'var(--text-muted)' }}>{slide.text}</p> : null}
      </div>
    </>
  );

  const u = (slide.link_url || '').trim();

  if (hasMedia) {
    const captionHit =
      /^https?:\/\//i.test(u) ? (
        <a href={u} target="_blank" rel="noopener noreferrer" className="ad-slider-slide-caption-hit">
          {caption}
        </a>
      ) : u.startsWith('/') ? (
        <Link to={u} className="ad-slider-slide-caption-hit">
          {caption}
        </Link>
      ) : (
        caption
      );

    return (
      <div className="ad-slider-slide-inner ad-slider-slide-inner--media">
        {mediaBlock}
        {captionHit}
      </div>
    );
  }

  if (/^https?:\/\//i.test(u)) {
    return (
      <a href={u} target="_blank" rel="noopener noreferrer" className="ad-slider-slide-link">
        {textOnlyInner}
      </a>
    );
  }
  if (u.startsWith('/')) {
    return <Link to={u} className="ad-slider-slide-link">{textOnlyInner}</Link>;
  }
  return <div className="ad-slider-slide-inner">{textOnlyInner}</div>;
}

export default function Home() {
  const { add } = useCart();

  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);
  const [benefitsBlock, setBenefitsBlock] = useState(buildBenefitsFallbackBlock);
  const [adIndex, setAdIndex] = useState(0);
  const [adSlidesRemote, setAdSlidesRemote] = useState(null);
  const adTimerRef = useRef(null);
  const adSwipeRef = useRef(null);

  const adSlidesDisplay = useMemo(() => {
    const localFallbacks = FALLBACK_AD_SLIDES;
    const withFallbackImage = (s, i) => {
      const fromApi = resolveAdSlideAssetUrl(s?.image_url);
      if (fromApi) return fromApi;
      return String(localFallbacks[i % localFallbacks.length]?.image || '').trim();
    };
    if (adSlidesRemote === null) {
      return localFallbacks.map((s) => ({
        id: s.id,
        title: s.title,
        text: s.text,
        link_url: '',
        image_url: s.image || '',
        image_url_mobile: '',
        video_url: '',
      }));
    }
    if (adSlidesRemote.length > 0) {
      return adSlidesRemote.map((s, i) => ({
        id: s.id,
        title: s.title,
        text: s.subtitle || '',
        link_url: s.link_url || '',
        image_url: withFallbackImage(s, i),
        image_url_mobile: resolveAdSlideAssetUrl(s.image_url_mobile),
        video_url: resolveAdSlideAssetUrl(s.video_url),
      }));
    }
    return localFallbacks.map((s) => ({
      id: s.id,
      title: s.title,
      text: s.text,
      link_url: '',
      image_url: s.image || '',
      image_url_mobile: '',
      video_url: '',
    }));
  }, [adSlidesRemote]);

  useEffect(() => {
    fetch(`${API_PREFIX}/products`)
      .then(async (r) => {
        const text = await r.text();
        const d = parseApiJsonText(text);
        if (d && typeof d === 'object' && !Array.isArray(d)) setProducts(Array.isArray(d.products) ? d.products : []);
        else setProducts([]);
      })
      .catch(() => setProducts([]))
      .finally(() => setLoading(false));
  }, []);

  useEffect(() => {
    let cancelled = false;
    fetch(`${API_PREFIX}/ad-slides`, { cache: 'no-store' })
      .then((r) => {
        if (!r.ok) throw new Error('ad-slides');
        return r.json();
      })
      .then((d) => {
        if (cancelled) return;
        setAdSlidesRemote(Array.isArray(d.slides) ? d.slides : []);
      })
      .catch(() => {
        if (!cancelled) setAdSlidesRemote([]);
      });
    return () => {
      cancelled = true;
    };
  }, []);

  useEffect(() => {
    let cancelled = false;
    fetch(`${API_PREFIX}/home-benefits`, { cache: 'no-store' })
      .then(async (r) => {
        const text = await r.text();
        return parseApiJsonText(text);
      })
      .then((d) => {
        if (cancelled || !d || !Array.isArray(d.items)) return;
        setBenefitsBlock({
          section_title: d.section_title || 'MyShop qulayliklari',
          items: d.items.map((it, i) => ({
            accent: HOME_BENEFIT_SLOTS[i]?.accent || 'delivery',
            icon: HOME_BENEFIT_SLOTS[i]?.icon || 'fa-circle',
            title: String(it.title ?? ''),
            text: String(it.text ?? ''),
          })),
        });
      })
      .catch(() => {});
    return () => {
      cancelled = true;
    };
  }, []);

  useEffect(() => {
    setAdIndex((i) => {
      const n = adSlidesDisplay.length;
      if (n === 0) return 0;
      return i % n;
    });
  }, [adSlidesDisplay.length]);

  useEffect(() => {
    const n = adSlidesDisplay.length;
    if (n === 0) return undefined;
    adTimerRef.current = setInterval(() => {
      setAdIndex((i) => (i + 1) % n);
    }, 5000);
    return () => clearInterval(adTimerRef.current);
  }, [adSlidesDisplay.length]);

  const handleAdSliderTouchStart = (e) => {
    if (!e.touches?.[0]) return;
    adSwipeRef.current = { x0: e.touches[0].clientX };
  };
  const handleAdSliderTouchEnd = (e) => {
    const start = adSwipeRef.current;
    adSwipeRef.current = null;
    if (!start || !e.changedTouches?.[0]) return;
    const n = adSlidesDisplay.length;
    if (n < 2) return;
    const dx = e.changedTouches[0].clientX - start.x0;
    if (Math.abs(dx) < 56) return;
    if (dx < 0) setAdIndex((i) => (i + 1) % n);
    else setAdIndex((i) => (i - 1 + n) % n);
  };
  const handleAdSliderTouchCancel = () => {
    adSwipeRef.current = null;
  };

  const formatPrice = (n) => new Intl.NumberFormat('uz-UZ').format(n) + ' so\'m';

  const activeProducts = useMemo(
    () =>
      products
        .filter((p) => (p.status === 'active' || !p.status) && p.stock > 0)
        .map((p) => ({ ...p, units_sold: Number(p.units_sold) || 0 })),
    [products],
  );

  /** Yangilar: created_at bo‘yicha eng yangi 15 ta */
  const newestProducts = useMemo(() => {
    const sorted = [...activeProducts].sort((a, b) =>
      String(b.created_at || '').localeCompare(String(a.created_at || '')),
    );
    return sorted.slice(0, 15);
  }, [activeProducts]);

  /** Cheksiz lentada tiklash uchun ikki marta */
  const newestStrip = useMemo(() => [...newestProducts, ...newestProducts], [newestProducts]);

  /**
   * Eng ommabop: faqat haqiqiy sotuv (buyurtma yakunlangan) bo‘lgan mahsulotlar.
   * `units_sold` 0 bo‘lsa — bo‘lim butunlay yashirin (sarlavha ham, «yo‘q» matni ham).
   */
  const ommabop = useMemo(() => {
    const withSales = activeProducts.filter((p) => (Number(p.units_sold) || 0) >= 1);
    return [...withSales].sort((a, b) => (b.units_sold || 0) - (a.units_sold || 0)).slice(0, 12);
  }, [activeProducts]);

  const ommabopStrip = useMemo(() => [...ommabop, ...ommabop], [ommabop]);

  /** Barcha mahsulotlar: avval sotuvi pastlari (ko‘rinadi), keyin qolganlar */
  const gridProducts = useMemo(() => {
    return [...activeProducts].sort((a, b) => {
      const d = (a.units_sold || 0) - (b.units_sold || 0);
      if (d !== 0) return d;
      return String(b.created_at || '').localeCompare(String(a.created_at || ''));
    });
  }, [activeProducts]);
  const disabledProducts = products.filter((p) => (p.status === 'active' || !p.status) && p.stock <= 0);

  function ProductCard({ p, className = '', showAdd = true }) {
    const isDisabled = (p.stock ?? 0) <= 0;
    return (
      <article className={`card product-card ${isDisabled ? 'disabled' : ''} ${className}`}>
        <Link to={`/products/${p.id}`} className="product-card-link">
          <div className="product-image">
            {p.image_url ? <img src={p.image_url} alt={p.name_uz} /> : <div className="product-placeholder" />}
          </div>
          <div className="product-card-body">
            <h3>{p.name_uz}</h3>
            <p className="product-price">{formatPrice(p.sale_price ?? p.price)}</p>
          </div>
        </Link>
        {showAdd && (
          <div className="product-card-actions">
            <button type="button" className="btn btn-primary" onClick={() => add(p)} disabled={isDisabled}>Savatga</button>
          </div>
        )}
      </article>
    );
  }

  return (
    <div className="home-page">
      {adSlidesDisplay.length > 0 ? (
        <section className="ad-slider-section ad-slider-section--fullwidth" aria-label="Asosiy banner">
          <div
            className="ad-slider ad-slider--touch"
            onTouchStart={handleAdSliderTouchStart}
            onTouchEnd={handleAdSliderTouchEnd}
            onTouchCancel={handleAdSliderTouchCancel}
          >
            <div className="ad-slider-track" style={{ transform: `translateX(-${adIndex * 100}%)` }}>
              {adSlidesDisplay.map((slide, i) => {
                const img = String(slide.image_url || '').trim();
                const imgMob = String(slide.image_url_mobile || '').trim();
                const hasBannerMedia = Boolean(
                  String(slide.video_url || '').trim() || img || imgMob,
                );
                return (
                  <div key={`ad-${i}-${slide.id}`} className={`ad-slider-slide${hasBannerMedia ? ' ad-slider-slide--media' : ''}`}>
                    <AdSliderSlide slide={slide} isActive={i === adIndex} />
                  </div>
                );
              })}
            </div>
          </div>
        </section>
      ) : null}

      <div className="container">
        <section className="home-new-products" aria-labelledby="home-new-products-heading">
          <div className="home-section-heading">
            <h2 id="home-new-products-heading" className="home-section-title">
              Yangi qo‘shilgan mahsulotlar
            </h2>
          </div>
          {loading ? (
            <p style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>Yuklanmoqda...</p>
          ) : newestProducts.length > 0 ? (
            <div className="home-ommabop-wrap home-ommabop-wrap--fullbleed home-new-products-strip">
              <div className="home-ommabop-track home-new-products-track">
                {newestStrip.map((p, idx) => (
                  <ProductCard key={`new-${p.id}-${idx}`} p={p} />
                ))}
              </div>
            </div>
          ) : (
            <p style={{ color: 'var(--text-muted)', marginTop: '0.75rem' }}>
              Hozircha yangi mahsulot yo&apos;q.
            </p>
          )}
        </section>

        {!loading && ommabop.length > 0 ? (
          <section className="home-products" aria-labelledby="home-ommabop-heading">
            <div className="home-products-heading-row">
              <div className="home-section-heading">
                <h2 id="home-ommabop-heading" className="home-section-title">
                  Eng ommabop mahsulotlar
                </h2>
              </div>
            </div>
            <div className="home-ommabop-wrap home-ommabop-wrap--fullbleed">
              <div className="home-ommabop-track home-ommabop-track--popular">
                {ommabopStrip.map((p, idx) => (
                  <ProductCard key={`pop-${p.id}-${idx}`} p={p} />
                ))}
              </div>
            </div>
          </section>
        ) : null}

        <section className="home-products home-products-grid">
          <div className="home-section-heading">
            <h2 className="home-section-title">Barcha mahsulotlar</h2>
          </div>
          {loading ? (
            <p style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>Yuklanmoqda...</p>
          ) : (
            <div className="product-grid">
              {gridProducts.map((p) => (
                <ProductCard key={p.id} p={p} />
              ))}
              {disabledProducts.map((p) => (
                <ProductCard key={p.id} p={p} />
              ))}
            </div>
          )}
          {!loading && products.length > 0 && (
            <div style={{ textAlign: 'center', marginTop: '2rem' }}>
              <Link to="/products" className="catalog-cat-chip catalog-cat-chip--cta home-all-products-cta">
                Barcha mahsulotlar
                <i className="fas fa-arrow-right home-all-products-cta-icon" aria-hidden />
              </Link>
            </div>
          )}
        </section>
      </div>

      <section className="home-benefits" aria-labelledby="home-benefits-heading">
        <div className="container">
          <h2 id="home-benefits-heading" className="home-benefits-title">
            {benefitsBlock.section_title}
          </h2>
          <div className="home-benefits-grid">
            {benefitsBlock.items.map((item, idx) => (
              <div
                key={`home-benefit-${idx}`}
                className={`home-benefits-card home-benefits-card--${item.accent}`}
              >
                <div className="home-benefits-icon-wrap" aria-hidden>
                  <i className={`fas ${item.icon} home-benefits-icon`} />
                </div>
                <h3 className="home-benefits-card-title">{item.title}</h3>
                <p className="home-benefits-card-text">{item.text}</p>
              </div>
            ))}
          </div>
        </div>
      </section>
    </div>
  );
}

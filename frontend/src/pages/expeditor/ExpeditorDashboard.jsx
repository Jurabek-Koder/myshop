...
function ExpeditorCourierHandoffDrawer({
  ...
  const loadClosedBatches = useCallback(async () => {
  ...
  }, [open, loadClosedBatches, regionFilterId, selectedDate, listsRefreshNonce]);

  useEffect(() => {
    if (!open) return;
    setSelectedBatchId(null);
    setSelectedBatchOrders([]);
  }, [selectedViloyatId, selectedDate, open]);
  ...
  const batchesForDate = useMemo(() => {
    ...
    return closedBatches.filter((b) => {
      ...
      return parts?.date === selectedDate;
    });
  }, [closedBatches, selectedDate]);
  ...
  useEffect(() => {
    if (!open) return;
    if (!selectedDate && closedBatches.length > 0) {
      const parts = getDateTimePartsInUzbekistan(closedBatches[0].closed_at);
      if (parts?.date) setSelectedDate(parts.date);
    }
  }, [open, selectedDate, closedBatches]);
      ...
      <div className="expeditor-handoff-drawer-panel">
          ...
          <div className="expeditor-handoff-filterbar">
            <label className="expeditor-handoff-filterfield">
              ...
              <StaffChipDropdown
                ...
                onChange={setSelectedViloyatId}
                options={PACKER_UZ_VILOYATLAR.map((v) => ({ value: v.id, label: v.name }))}
                emptyValue="all"
        ...
        <div className="expeditor-handoff-drawer-body">
          {!courierStaffId ? (
            ...
            <p className="expeditor-queue-hint">Viloyat tanlang, keyin sana maydoni chiqadi.</p>
          ) : !selectedDate ? (
            <p className="expeditor-queue-hint">Sanani tanlang — keyin kunlik listlar chiqadi.</p>
          ) : loadingBatches ? (
                ...
                {batchesForDate.map((batch) => {
                  ...
                  return (
                    <button
                      ...
                      aria-expanded={false}
                    >
                      <span className="expeditor-handoff-result-item-title">{stamp}</span>
            ...
            <>
              <p className="expeditor-handoff-result-summary">
                {selectedBatch ? formatHandoffBatchStamp(selectedBatch.closed_at, selectedBatchOrders.length) : 'List'}
...
export default function ExpeditorDashboard() {
  ...
  const openCameraScanner = useCallback(() => {
    ...
    openScan();
  }, [scanDuplicateModal, listCloseReminderOpen, courierStaffId, scanBlockedByBalance, isHome, navigate]);

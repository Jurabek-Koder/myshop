...
import { PACKER_UZ_VILOYATLAR, orderMatchesViloyatEntry } from '../../constants/uzViloyatlarPacker';
...
import StaffTopbarBalanceDropdown from '../../components/staff/StaffTopbarBalanceDropdown.jsx';
import ExpeditorRegionCourierSelect from '../../components/expeditor/ExpeditorRegionCourierSelect.jsx';
...
import { speakScannedProductCount, warmUpScanSpeech } from '../../utils/scanProductSpeech.js';
...
/** Kuryerning sklad viloyati matnini (region_name) viloyat slug id ga moslaydi. */
function viloyatIdForCourierRegion(regionName) {
  const s = String(regionName || '').toLowerCase().trim();
  if (!s) return '';
  const hit = PACKER_UZ_VILOYATLAR.find((entry) => orderMatchesViloyatEntry(s, entry));
  return hit?.id || '';
}
...
function openUnsoldProductsReceipt({ orders, courierLabel, regionLabel, autoPrint = true }) {
    ...
    <section className="order-receiver-atkaz" aria-labelledby="expeditor-unsold-title">
      ...
      <div className="order-receiver-atkaz__actions">
        <button
          ...
          onClick={() => openUnsoldProductsReceipt({ orders, courierLabel, regionLabel, autoPrint: true })}
        >
...
function ExpeditorMoliyaPanel({ request }) {
  ...
  const load = useCallback(async () => {
    ...
    setNoWorkRole(false);
    try {
      const [bRes, fRes] = await Promise.all([request('/expeditor/work-role/balance'), request('/expeditor/finance')]);
      ...
      if (bRes.status === 404 && bData.code === 'no_work_role') {
        ...
        if (fRes.ok) {
          ...
          setTransactions(Array.isArray(fData.transactions) ? fData.transactions : []);
          setList(Array.isArray(fData.withdrawals) ? fData.withdrawals : []);
        } else {
          const wRes = await request('/expeditor/withdrawals');
  ...
  return (
    <div className="packer-page">
      ...
      <StaffTransactionTimeline transactions={transactions} />
      {list.length > 0 && transactions.length === 0 ? (
...
export default function ExpeditorDashboard() {
  ...
  useEffect(() => {
    ...
    const update = () => setIsNarrowTopbar(mq.matches);
    update();
    if (mq.addEventListener) {
      mq.addEventListener('change', update);
      return () => mq.removeEventListener('change', update);
  ...
  const loadCouriers = useCallback(async () => {
    ...
    try {
      // Barcha kuryerlar — viloyat bo‘yicha filtrlash client-side (birlashgan dropdown uchun ham kerak).
      const res = await request('/expeditor/couriers', { cache: 'no-store' });
      const data = await res.json().catch(() => ({}));
  ...
  useEffect(() => {
    ...
    const courier = couriers.find((c) => String(c.id) === String(courierStaffId));
    if (!courier) {
      // O‘chirilgan / mavjud bo‘lmagan kuryer
      writeExpeditorCourierFilter('');
      setCourierStaffId('');
      return;
    }
    if (regionFilterId) {
      const entry = PACKER_UZ_VILOYATLAR.find((v) => v.id === regionFilterId);
      if (entry && !orderMatchesViloyatEntry(String(courier.region_name || ''), entry)) {
        writeExpeditorCourierFilter('');
        setCourierStaffId('');
      }
    }
  }, [courierStaffId, couriers, regionFilterId]);
  ...
  useEffect(() => {
    ...
    warmUpScanSpeech();
    const input = document.getElementById('expeditor-order-scan');
    if (!input) return undefined;
    const onActivate = () => warmUpScanSpeech();
    input.addEventListener('focus', onActivate);
    input.addEventListener('pointerdown', onActivate);
    return () => {
      input.removeEventListener('focus', onActivate);
      input.removeEventListener('pointerdown', onActivate);
    };
  }, [isHome]);

  /** Tanlangan viloyat bo‘yicha kuryerlar (desktopdagi alohida kuryer dropdowni uchun) — client-side. */
  const regionCouriers = useMemo(() => {
    if (!regionFilterId) return couriers;
    const entry = PACKER_UZ_VILOYATLAR.find((v) => v.id === regionFilterId);
    if (!entry) return couriers;
    return couriers.filter((c) => orderMatchesViloyatEntry(String(c.region_name || ''), entry));
  }, [couriers, regionFilterId]);
  ...
  /** Mobil birlashgan tanlov: viloyat id -> shu viloyat kuryerlari (2-bosqich modali uchun). */
  const couriersByVid = useMemo(() => {
    const map = {};
    for (const c of couriers) {
      const vid = viloyatIdForCourierRegion(c.region_name) || '__other__';
      (map[vid] ||= []).push(c);
    }
  ...
  /** 1-bosqich modali: barcha viloyatlar + (mavjud bo‘lsa) boshqa hududlar, kuryerlar soni bilan. */
  const regionListForSelect = useMemo(() => {
    const list = PACKER_UZ_VILOYATLAR.map((v) => ({
      id: v.id,
      name: getExpeditorViloyatDisplayName(v.id),
      count: (couriersByVid[v.id] || []).length,
    }));
    if ((couriersByVid.__other__ || []).length) {
      list.push({ id: '__other__', name: 'Boshqa hududlar', count: couriersByVid.__other__.length });
    }
    return list;
  }, [couriersByVid]);

  const applyRegionFilter = useCallback((vid) => {
    setRegionFilterId(vid);
    writeExpeditorViloyatFilter(vid);
    if (vid) setMainHandoffListClosed(true);
  }, []);

  const handleCombinedRegionCourierChange = useCallback(
    (val) => {
      const v = String(val || '');
      if (!v) {
        applyRegionFilter('');
      ...
      if (v.startsWith('c:')) {
        const id = v.slice(2);
        const courier = couriers.find((c) => String(c.id) === id) || null;
        const vid = courier ? viloyatIdForCourierRegion(courier.region_name) : '';
        applyRegionFilter(vid || '');
        setCourierStaffId(id);
        writeExpeditorCourierFilter(id);
      }
    },
    [applyRegionFilter, couriers],
  );

  const combinedRegionCourierLabel = useMemo(() => {
    if (courierStaffId) {
      const c = couriers.find((x) => String(x.id) === String(courierStaffId));
      if (c) {
        const vid = viloyatIdForCourierRegion(c.region_name);
        const vName = vid ? getExpeditorViloyatDisplayName(vid) : '';
        return `${vName ? `${vName} · ` : ''}${c.full_name || `Kuryer #${c.id}`}`;
      }
    }
    if (regionFilterId) return getExpeditorViloyatDisplayName(regionFilterId);
    return 'Viloyat / kuryer';
  }, [couriers, courierStaffId, regionFilterId]);
  ...
  const topbarClockOrList = showCloseHandoffListBtn ? (
    ...
    >
      ...
      <span className="expeditor-list-btn-label">Ro‘yxat</span>
    </button>
  ) : (
    <time
  ...
  return (
    <div className="packer-app packer-app--compact-cards">
      <aside id="expeditor-sidebar-nav" className="packer-sidebar packer-sidebar--rail" aria-label="Ekspeditor navigatsiyasi">
        <div className="packer-sidebar-inner">
          ...
          <nav className="packer-nav packer-nav--rail" aria-label="Ekspeditor bo‘limlari">
            ...
            <NavLink to="/expeditor/guruh" className={navLinkClassRail} title="Guruh">
              <RailIconNav>
                <path
                  {...RAIL_IC}
            ...
            {isNarrowTopbar && (showOpenHandoffListBtn || showCloseHandoffListBtn) ? (
              <button
                type="button"
                className={`packer-nav-link packer-nav-link--rail expeditor-rail-list${showCloseHandoffListBtn ? ' expeditor-rail-list--close' : ''}`}
                title={showCloseHandoffListBtn ? 'Listni yopish' : 'Tanlangan viloyat zakazlari ro‘yxatini ochish'}
                aria-label={showCloseHandoffListBtn ? 'Listni yopish' : 'Ro‘yxat'}
                onClick={() => {
                  if (showCloseHandoffListBtn) {
                    void closeHandoffList();
                    return;
                  }
                  setMainHandoffListClosed(false);
                  setHomePaneTab('kuryer');
              ...
              >
                <RailIconNav>
                  <path {...RAIL_IC} d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01" />
                </RailIconNav>
                <span className="packer-sr-only">Ro‘yxat</span>
              </button>
            ) : null}
            {isHome ? (
              <button
      ...
      <div className="packer-workspace">
        <header className={`packer-sticky-topbar packer-sticky-topbar--expeditor${isNarrowTopbar ? ' packer-sticky-topbar--expeditor-mobile' : ''}${hideHomeTopbarDetails ? ' expeditor-topbar--compact' : ''}`}>
          {isNarrowTopbar ? (
            <>
              ...
              {!hideHomeTopbarDetails ? (
                <div className="expeditor-mtop">
                  <ExpeditorRegionCourierSelect
                    triggerLabel={combinedRegionCourierLabel}
                    hasFilter={Boolean(courierStaffId || regionFilterId)}
                    regions={regionListForSelect}
                    couriersByVid={couriersByVid}
                    disabled={couriersLoading && couriers.length === 0}
                    title="Viloyat va kuryer"
                    instanceId="expeditor-region-courier"
                    className="expeditor-mtop-rc"
                    onSelectAll={() => handleCombinedRegionCourierChange('')}
                    onSelectRegion={(vid) => handleCombinedRegionCourierChange(`r:${vid}`)}
                    onSelectCourier={(id) => handleCombinedRegionCourierChange(`c:${id}`)}
                  />
                  {isHome ? (
                    <StaffTopbarBalanceDropdown
                      triggerLabel="Balans"
                      alert={Boolean(showCourierBalance && courierBalanceNegative)}
                      rows={[
                        {
                          key: 'mahsulot',
                          label: 'Sklad mahsulotlari balansi',
                          value: formatExpeditorMoney(expeditorBalance ?? 0, 'UZS'),
                        },
                        ...(showCourierBalance
                          ? [
                              {
                                key: 'kuryer',
                                label: 'Kuryer balansi',
                                value: formatExpeditorMoney(Number(selectedCourier.balance ?? 0), 'UZS'),
                                danger: courierBalanceNegative,
                              },
                            ]
                          : []),
                      ]}
                      title="Balanslar"
                      className="expeditor-mtop-balance"
                    />
                  ) : null}
                  {isHome ? <div className="expeditor-mtop-scan">{orderScanForm}</div> : null}
                </div>
              ) : null}
            </>
          ) : (
        ...
        >
          ...
          <Routes>
            ...
            <Route path="guruh" element={<ChatGroupsScreen onExit={() => navigate('/expeditor')} />} />

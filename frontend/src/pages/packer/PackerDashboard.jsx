    ...
    <div className="packer-app packer-app--v2-queue">
      ...
      <div className="packer-workspace">
        <header className="packer-sticky-topbar">
          ...
          <div className="packer-staff-topbar-actions">
            ...
            <StaffTopbarProfileMenu
              ...
              onHome={() => navigate('/')}
              onProfile={() => navigate('/packer/profile')}
              onSettings={() => navigate('/packer/profile')}
              onLogout={() => {
                logout();
                navigate('/login');
              }}
              ...
              element={
                <div className="packer-v2-queue">
                  {loading && orders.length === 0 ? (
                    ...
                    <div className="packer-v2-product-grid">
                      {queueProducts.map((row) => (
                        <article key={row.product_id} className="packer-v2-card">
                          <div className="packer-v2-card__hit">
                            <button
                              type="button"
                              className="packer-v2-card__thumb"
                              onClick={() => openQueueProductModal(row)}
                              title="Zakazlar va chek"
                              aria-label={`${row.name_uz} — zakazlar va chek`}
                            >
                              {row.image_url ? (
                                <img src={resolvePackerProductImageUrl(row.image_url)} alt="" decoding="async" />
                              ) : (
                                <div className="packer-v2-card__thumb-placeholder" aria-hidden />
                              )}
                            </button>
                            {row.showHoldCta ? (
                              <div className="packer-v2-card__hold-strip">
                                <button
                                  type="button"
                                  className="packer-v2-card__hold-btn"
                                  onClick={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                            ...
                            <button
                              type="button"
                              className="packer-v2-card__title-wrap"
                              onClick={() => openQueueProductModal(row)}
                            >
                              <h3 className="packer-v2-card__title">{row.name_uz}</h3>
                              {row.units_in_queue > 1 ? (
                                <span className="packer-v2-card__sub">Navbatda {row.units_in_queue} dona</span>
                              ) : null}
                            </button>
                          </div>
                          <div className="packer-v2-card__metrics" aria-label="Mahsulot bo‘yicha zakazlar va sklad soni">
                            <div className="packer-v2-card__metric packer-v2-card__metric--orders">
                              <span className="packer-v2-card__metric-k">Zakazlar soni</span>
                              <span className="packer-v2-card__metric-num" tabIndex={0}>
                                {row.orders_count ?? 0}
                              </span>
                            </div>
                            <span className="packer-v2-card__dots" aria-hidden />
                            <div className="packer-v2-card__metric packer-v2-card__metric--warehouse">
                              <span className="packer-v2-card__metric-k">Sklad</span>
                              <span
                                className={`packer-v2-card__metric-num packer-v2-card__metric-num--stock${Number(row.stock) <= 0 ? ' packer-v2-card__metric-num--warn' : ''}`}
                                key={`${row.product_id}-${row.stock}`}
                              >
                                {Number(row.stock) || 0}
                              </span>
                            </div>

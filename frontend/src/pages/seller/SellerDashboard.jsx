...
export default function SellerDashboard() {
  ...
  const chartData = useMemo(() => {
    const base = [0, 1, 2, 3, 4, 5, 6].map((i) => {
      const src = products[i % Math.max(1, products.length)] || {};
      const pricePart = Math.round(Number(src.price || 0) / 100000);
      const stockPart = Math.round(Number(src.stock || 0) * 1.8);
      return Math.max(10, pricePart + stockPart + i * 6);
    });
    ...
    return base.map((v, i) => ({
      label: labels[i],
      value: v,
      height: 24 + Math.round((v / max) * 120),
    }));
  }, [products]);
              ...
              <div className="seller-charts-row">
                <article className="seller-panel-card">
                  <div className="seller-panel-head">
                    <h4>Oxirgi 7 kunlik sotuvlar</h4>
                    <select>
                  ...
                  <div className="seller-chart-box">
                    {chartData.map((row) => (
                      <div className="seller-bar-col" key={row.label}>
                        <div className="seller-bar" style={{ height: `${row.height}px` }} />
                        <span>{row.label}</span>
                      </div>
                    ))}
                  </div>

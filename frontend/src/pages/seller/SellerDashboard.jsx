...
const SELLER_POSITION_SIZE_MIN = 30;
const SELLER_POSITION_SIZE_MAX = 50;
const SELLER_POSITION_SIZE_OPTIONS = Array.from(
  { length: SELLER_POSITION_SIZE_MAX - SELLER_POSITION_SIZE_MIN + 1 },
  (_, i) => String(SELLER_POSITION_SIZE_MIN + i),
);

/** O‘lcham bo‘lmagan mahsulotlar uchun qo‘lda yozish */
const SELLER_POSITION_COLOR_PRESETS = [
  'Qora',
  'Oq',
  'Kulrang',
  'Jigarrang',
...
function newSellerPositionLineId() {
...
function makeProductForm() {
  return {
    ...
    position_kind: '',
    position_size: '',
    position_color_select: '',
    position_color_custom: '',
    position_custom_label: '',
    position_line_qty: '1',
    position_lines: [],
  };
}
...
export default function SellerDashboard() {
  ...
  const myShopChatNavItem = useMemo(
    () => ({ key: 'chat', label: pickerUiT.navMyShopChat }),
    [pickerUiT.navMyShopChat]
  );

  const guruhNavItem = useMemo(() => ({ key: 'guruh', label: 'Sellerlar guruhi' }), []);
  ...
  const pageTitle = useCallback(
    (view) => {
      ...
      if (view === 'chat') return pickerUiT.navMyShopChat;
      const found = mainMenuItems.find((item) => item.key === view);
      return found ? found.label : 'Dashboard';
    },
    [mainMenuItems, pickerUiT.navMyShopChat]
  );
  ...
  const sellerAppRef = useRef(null);
  const sellerTopbarRef = useRef(null);
  ...
  /** Mobil: yon panel topbar ostidan boshlansin — bo‘sh chiziq yo‘q; balandlikni real DOM bo‘yicha */
  useLayoutEffect(() => {
    const app = sellerAppRef.current;
    const topbar = sellerTopbarRef.current;
    if (!app) return undefined;

    if (typeof window === 'undefined' || isDesktop || activeView === 'chat' || activeView === 'guruh') {
      app.style.removeProperty('--seller-mobile-sidebar-top');
      return undefined;
    }
    ...
    const apply = () => {
      ...
      if (Number.isFinite(bottom) && bottom >= 0) {
        app.style.setProperty('--seller-mobile-sidebar-top', `${bottom}px`);
      }
    };

    apply();

    const ro = typeof ResizeObserver !== 'undefined' ? new ResizeObserver(() => requestAnimationFrame(apply)) : null;
    ro?.observe(topbar);

    window.addEventListener('resize', apply);
    const onOrient = () => requestAnimationFrame(apply);
  ...
  }, [isDesktop, activeView, darkMode, sidebarOpen]);
  ...
  const positionStockSum = useMemo(
    () =>
      form.position_lines.reduce((acc, row) => acc + Math.max(0, Math.round(Number(row.qty) || 0)), 0),
    [form.position_lines],
  );
  ...
  const productListSummaryNodes = useMemo(
    () =>
      filteredProducts.map((row) => {
        ...
        const title =
          String(row.name_uz || row.name_ru || '').trim() || `Mahsulot #${row.id}`;
        ...
        return (
          <div key={row.id} className="seller-product-summary-row" role="listitem" aria-label={title}>
            ...
            {archiveBrak ? (
              ...
              <div className="seller-product-summary-body">
                <strong className="seller-product-summary-name">{title}</strong>
  ...
  function appendProductPositionRow() {
    const kindRaw = String(form.position_kind || '').trim();
    if (!kindRaw) {
      setError('Pozitsiya turini tanlang.');
      return;
    }

    let label = '';
    if (kindRaw === 'size') {
      const s = String(form.position_size || '').trim();
      if (!s) {
        setError('O‘lchamni tanlang.');
        return;
      }
      label = s;
    } else if (kindRaw === 'color') {
      const custom = String(form.position_color_custom || '').trim();
      const preset = String(form.position_color_select || '').trim();
      label = custom || preset;
      if (!label) {
        setError('Rangni tanlang yoki yozing.');
        return;
      }
    } else if (kindRaw === 'custom') {
      label = String(form.position_custom_label || '').trim();
      if (!label) {
        setError('Pozitsiya yozuvini kiriting.');
        return;
      }
    } else {
      setError('Noto‘g‘ri pozitsiya turi.');
      return;
    }

    const qty = Math.round(Number.parseInt(String(form.position_line_qty), 10));
    if (!Number.isInteger(qty) || qty < 1) {
      setError('Miqdor 1 yoki undan katta bo‘lsin.');
      return;
    }

    setForm((prev) => ({
      ...prev,
      position_lines: [...prev.position_lines, { id: newSellerPositionLineId(), kind: kindRaw, label, qty }],
      position_kind: '',
      position_size: '',
      position_color_select: '',
      position_color_custom: '',
  ...
  function removeProductPositionRow(lineId) {
    setForm((prev) => ({
      ...prev,
      position_lines: prev.position_lines.filter((row) => row.id !== lineId),
    }));
  }
  ...
  const handleCreate = async (e) => {
    ...
    if (!formShare.valid) return setError("Admin ulushi va sayt foizi yig'indisi 100% dan oshmasligi kerak.");
    ...
    await runMutation('create-product', async () => {
      ...
      const sellerLines = form.position_lines
        .map((row) => ({
          kind: String(row.kind || '').trim(),
          label: String(row.label || '').trim(),
          qty: Math.max(0, Math.round(Number(row.qty) || 0)),
        }))
        .filter((row) => row.label && row.qty > 0 && ['size', 'color', 'custom'].includes(row.kind));

      const stockFromPositions = sellerLines.reduce((s, r) => s + r.qty, 0);
      const stockVal = sellerLines.length > 0 ? stockFromPositions : Number(form.stock) || 0;
      ...
      if (sellerLines.length > 0) {
        body.seller_positions_json = JSON.stringify(sellerLines);
      }
  ...
  const sellerInboxNotifications = useMemo(
    () =>
      notifications.map((item) => ({
        ...
        read_at: Number(item.is_read) ? item.read_at || item.created_at || '1' : null,
      })),
    [notifications],
  );
  ...
  const isMyShopChatView = activeView === 'chat' || activeView === 'guruh';

  return (
    <div
      ref={sellerAppRef}
      className={`seller-app seller-mobile-shell ${darkMode ? 'seller-dark' : ''}${isMyShopChatView ? ' seller-chat-mode' : ''}`}
    >
      ...
      <aside className={`seller-sidebar ${sidebarOpen ? 'open' : ''}`}>
        <div className="seller-sidebar-inner">
          ...
          <nav className="seller-menu-list" aria-label="Asosiy menyu">
          ...
          <div className="seller-menu-label seller-label-gap seller-menu-label-myshop">MYSHOP</div>
          <nav className="seller-menu-list seller-menu-list--myshop" aria-label={pickerUiT.navMyShopChat}>
            <button
              type="button"
              data-nav={myShopChatNavItem.key}
              className={`seller-menu-item ${activeView === myShopChatNavItem.key ? 'active' : ''}`}
              title={myShopChatNavItem.label}
              aria-label={myShopChatNavItem.label}
              onClick={() => {
                setActiveViewWithUrl(myShopChatNavItem.key);
                if (!isDesktop) setSidebarOpen(false);
              }}
            >
              <span className="seller-menu-item-leading">
              ...
              <span className="seller-menu-item-label">{myShopChatNavItem.label}</span>
              <span className="seller-menu-item-badge-wrap" />
            </button>
            <button
              type="button"
              data-nav={guruhNavItem.key}
              className={`seller-menu-item ${activeView === guruhNavItem.key ? 'active' : ''}`}
              title={guruhNavItem.label}
              aria-label={guruhNavItem.label}
              onClick={() => {
                setActiveViewWithUrl(guruhNavItem.key);
                if (!isDesktop) setSidebarOpen(false);
              }}
            >
              <span className="seller-menu-item-leading" aria-hidden="true">
                👥
              </span>
              <span className="seller-menu-item-label">{guruhNavItem.label}</span>
              <span className="seller-menu-item-badge-wrap" />
            </button>
          </nav>
      ...
      <main className={`seller-main${isMyShopChatView ? ' seller-main--chat' : ''}`}>
        ...
        {!isMyShopChatView && (
        <header ref={sellerTopbarRef} className="seller-topbar">
          <StaffTopbarCenterId />
          ...
          <div className="seller-topbar-right">
            ...
            <NotificationBellCluster
              ...
              notifications={sellerInboxNotifications}
              formatDate={formatDateTimeUz}
              bellWrapClassName="seller-notify-wrap"
              bellButtonClassName="ms-bell-btn ms-bell-btn--seller"
              dotClassName="ms-bell-dot"
              emptyMessage={
                notificationsLoading
                  ? 'Xabarlar yuklanmoqda...'
                  : "Tanlangan sana uchun xabar yo'q."
              }
              ...
              onDismiss={async (n) => {
                ...
                if (item && !Number(item.is_read)) await handleReadNotification(item.id);
              }}
                ...
                if (item.link_view) {
                  actions.push(
                    <button
                      ...
                      onClick={() => {
                        handleOpenFromNotification(item.link_view);
                        onClose();
                      }}
                    >
                      Ochish
                ...
                if (!Number(item.is_read)) {
                  actions.push(
                    <button
                      ...
                      onClick={() => {
                        void handleReadNotification(item.id).then(() => goNext());
                      }}
                    >
                      O&apos;qildi
                    </button>,
            ...
            <div className="seller-profile-wrap">
              <StaffTopbarProfileMenu
                ...
                triggerClassName="seller-profile-btn"
                onHome={() => navigate('/')}
                onProfile={() => setActiveViewWithUrl(PROFILE_VIEW_KEY)}
                onSettings={() => setActiveViewWithUrl('settings')}
                onLogout={() => {
                  logout();
                  navigate('/login');
                }}
        ...
        <section
          className={`seller-content${isMyShopChatView ? ' seller-content--chat' : ''}${
            activeView === 'products_print' || activeView === 'products_archive'
              ...
              : ''
          }${activeView === 'products_add' ? ' seller-content--product-add' : ''}`}
        >
            ...
            <>
              ...
              <article className="seller-panel-card">
                ...
                <form className="seller-product-form" onSubmit={handleCreate}>
                  ...
                  <div className="seller-field-block seller-video-block">
                    ...
                    {String(form.video_url || '').trim() ? (
                      <div className="seller-video-preview">
                        <video
                          className="seller-video-preview-el"
                          src={form.video_url}
                          controls
                          playsInline
                          preload="metadata"
                          aria-label="Tanlangan video oldindan ko‘rish"
                        >
                  ...
                  <div className="seller-field-block seller-field-wide seller-positions-block">
                    <span>Pozitsiyalar (o‘lcham / rang)</span>
                    <div className="seller-positions-grid">
                      <select
                        className="seller-field"
                        aria-label="Pozitsiya turi"
                        value={form.position_kind}
                        onChange={(e) =>
                          setForm((p) => ({
                            ...p,
                            position_kind: e.target.value,
                            position_size: '',
                            position_color_select: '',
                            position_color_custom: '',
                            position_custom_label: '',
                      ...
                      >
                        <option value="">Pozitsiya tanlang</option>
                        <option value="size">O‘lcham</option>
                        <option value="color">Rang</option>
                        <option value="custom">Yozma</option>
                      </select>
                      {form.position_kind === 'size' ? (
                        <select
                          className="seller-field"
                          aria-label="O‘lcham"
                        ...
                        >
                          ...
                          {SELLER_POSITION_SIZE_OPTIONS.map((sz) => (
                            <option key={sz} value={sz}>{sz}</option>
                          ))}
                        </select>
                      ) : null}
                      {form.position_kind === 'color' ? (
                        <>
                          <select
                            className="seller-field"
                            aria-label="Rang (tanlangan)"
                            value={form.position_color_select}
                            onChange={(e) => setForm((p) => ({ ...p, position_color_select: e.target.value }))}
                          >
                            <option value="">Rang tanlang</option>
                            {SELLER_POSITION_COLOR_PRESETS.map((c) => (
                              <option key={c} value={c}>{c}</option>
                            ))}
                          </select>
                          <input
                            className="seller-field"
                            placeholder="Yoki rangni yozing"
                            value={form.position_color_custom}
                            onChange={(e) => setForm((p) => ({ ...p, position_color_custom: e.target.value }))}
                            aria-label="Rang yozma"
                          />
                        </>
                      ) : null}
                      {form.position_kind === 'custom' ? (
                        <input
                          className="seller-field"
                          placeholder="Masalan: XL, materialli va h.k."
                          value={form.position_custom_label}
                          onChange={(e) => setForm((p) => ({ ...p, position_custom_label: e.target.value }))}
                          aria-label="Pozitsiya yozuvi"
                        />
                      ) : null}
                      <input
                        className="seller-field"
                        type="number"
                        min={1}
                        placeholder="Bu qator soni"
                        value={form.position_line_qty}
                        onChange={(e) => setForm((p) => ({ ...p, position_line_qty: e.target.value }))}
                        aria-label="Pozitsiya miqdori"
                      />
                      <button type="button" className="seller-video-upload-btn" onClick={appendProductPositionRow}>
                        Qator qo‘shish
                      </button>
                    </div>
                    <small className="seller-field-hint">
                      Oyoq kiyim uchun o‘lcham 30–50 orasidan tanlanadi; keragicha bir nechta qator qo‘shishingiz mumkin. Bo‘sh
                      qoldirsangiz, pastdagi umumiy stock ishlatiladi.
                    </small>
                    {form.position_lines.length > 0 ? (
                      <ul className="seller-positions-lines" aria-label="Qo‘shilgan pozitsiyalar">
                        {form.position_lines.map((row) => (
                          <li key={row.id} className="seller-positions-line">
                            <span>
                              <strong>
                                {row.kind === 'size' ? 'O‘lcham' : row.kind === 'color' ? 'Rang' : 'Yozma'}
                              </strong>
                              {': '}
                              {row.label}
                              {' — '}
                            ...
                            <button
                              type="button"
                              className="seller-positions-line-remove"
                              onClick={() => removeProductPositionRow(row.id)}
                              aria-label={`Olib tashlash: ${row.label}`}
                            >
                              O‘chirish
                            </button>
                          </li>
                  ...
                  <input
                    className={`seller-field${form.position_lines.length > 0 ? ' seller-field-readonly-stock' : ''}`}
                    type="number"
                    min="0"
                    placeholder="Soni (stock)"
                    readOnly={form.position_lines.length > 0}
                    title={form.position_lines.length > 0 ? 'Pozitsiyalar yig‘indisi — qatorlardan hisoblanadi' : ''}
                    value={form.position_lines.length > 0 ? String(positionStockSum) : form.stock}
                    onChange={(e) =>
                      form.position_lines.length === 0
                  ...
                  />
                  <input className="seller-field" type="number" min="0" max="100" placeholder="Admin ulushi %" value={form.operator_share_percent} onChange={(e) => setForm((p) => ({ ...p, operator_share_percent: e.target.value }))} />
                  ...
                  <div className="seller-calc-box seller-field-wide">
                    <div><span>Admin ulushi summasi</span><strong>{formatCurrency(formShare.operatorAmount)}</strong></div>
          ...
          {activeView === 'finance' && (
            <article className="seller-panel-card">
              ...
              <div className="seller-finance-grid">
                ...
                <div><span>Admin ulushi</span><strong>{formatCurrency(summary?.operator_share_total || 0)}</strong></div>
            ...
            <article className="seller-panel-card">
              ...
              <div className="seller-settings-box">
                ...
                {profileLoading ? (
                  ...
                  <form className="seller-profile-form staff-account-touch" onSubmit={handleProfileSave}>
                    <input
                      className="seller-field"
                      placeholder="Ism"
          ...
          <nav className="seller-mobile-dock seller-mobile-shell-dock" aria-label="Asosiy pastki menyu">
            <div className="seller-mobile-dock-inner">
              {[
                ...
                { key: 'products', icon: 'fa-box', label: 'Mahsulotlar' },
                { key: 'products_add', icon: 'fa-plus', label: 'Qo‘shish', fab: true, ariaLabel: 'Yangi mahsulot qo‘shish' },
                ...
                return (
                  ...
                  >
                    ...
                    {!d.fab ? <span className="seller-mobile-dock-label">{d.label}</span> : null}
                  </button>
                );
              })}
            </div>

            ...
            <section className="picker-withdrawal-card courier-moliya-withdraw" aria-labelledby="courier-withdraw-heading">
              ...
              {withdrawMsg ? (
                ...
                {withdrawals.length > 0 && (
                  <div className="picker-withdrawal-history">
                    <h4 style={{ margin: '16px 0 8px', fontSize: '0.95rem' }}>Tranzaksiyalar tarixi</h4>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {withdrawals.map((w) => (
                        <div key={w.id} style={{ padding: '8px', background: 'var(--bg-secondary, #f0f0f0)', borderRadius: '6px', fontSize: '0.85rem' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                            <strong style={{ color: w.status === 'paid' ? 'var(--color-success, green)' : w.status === 'rejected' ? 'var(--color-danger, red)' : 'var(--color-warning, orange)' }}>
                              {w.status === 'paid' ? 'To\'landi' : w.status === 'rejected' ? 'Rad etildi' : 'Kutilmoqda'}
                            </strong>
                            <span className="muted">{formatDateTime(w.created_at)}</span>
                          </div>
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <span>{formatCurrency(w.amount)}</span>
                            <span className="muted">{w.payout_method === 'card' ? 'Karta' : 'Naqd'}</span>
                          </div>
                        </div>
                      ))}
                    </div>

...
export default function CourierDashboard() {
  ...
  const sideNavItems = useMemo(
    () => [
      ...
      { id: 'guruh', label: 'Kuryerlar guruhi', icon: '👥' },
      {
        id: 'courier_calls',
  ...
  const mainTelegramLayout = filter === 'lichka' || filter === 'guruh';
  const hideTopbarForMessaging = filter === 'lichka' || filter === 'guruh';
    ...
    <div className="picker-app picker-mobile">
      ...
      >
        ...
        <main className={`picker-main${mainTelegramLayout ? ' picker-main--telegram' : ''}`}>
          <StaffAdvanceConfirm />
          {filter === 'guruh' ? (
            <ChatGroupsScreen onExit={() => goNav('lists')} />
          ) : filter === 'lichka' ? (
            <PickerLichka
              t={pickerUiT}
              request={request}

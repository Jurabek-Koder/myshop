...
export default function CourierDashboard() {
  ...
  /** Yangi zakazlar: ekspeditor partiyalari sukut bo‘yicha ochiq; birinchi bosish yopadi. Kuryer baza: eski toggle. */
  const isExpeditorBatchExpanded = (batchId) =>
    filter === 'new' ? openExpeditorBatches[batchId] !== false : !!openExpeditorBatches[batchId];

  const toggleExpeditorBatchAdaptive = useCallback(
    (batchId) => {
      if (filter === 'new') {
        setOpenExpeditorBatches((prev) => {
          const cur = prev[batchId];
          const nextExpanded = cur === undefined ? false : !cur;
          return { ...prev, [batchId]: nextExpanded };
        });
      } else {
  ...
  const isDailyNewBatchExpanded = (batchId) =>
    filter === 'new' ? openDailyNewBatches[batchId] !== false : !!openDailyNewBatches[batchId];

  const toggleDailyNewBatchAdaptive = useCallback(
    (batchId) => {
      if (filter === 'new') {
        setOpenDailyNewBatches((prev) => {
          const cur = prev[batchId];
          const nextExpanded = cur === undefined ? false : !cur;
          return { ...prev, [batchId]: nextExpanded };
        });
            ...
            <section className="picker-subpage">
              ...
              <div className="picker-profile-card">
                {courierProfileLoading ? (
                  ...
                  <form className="picker-profile-form" onSubmit={handleCourierProfileSave}>
                    {courierProfileError ? (
                    ...
                    <label className="picker-profile-field">
                      ...
                      <input type="email" className="picker-profile-input" value={courierProfileForm.email} readOnly />
                    </label>
                    <label className="picker-profile-field">
                      <span className="picker-profile-label">{pickerUiT.profileLogin}</span>
                      <input type="text" className="picker-profile-input" value={courierProfileForm.login} readOnly />
                    </label>
                    <label className="picker-profile-field">
                      ...
                      <section key={batch.id} className="courier-packer-batch">
                        ...
                        >
                          <div className="courier-packer-batch-meta">
                            <span className="courier-packer-batch-title">
                              {filter === 'new' ? 'Ekspeditor berdi (navbat)' : 'Kuryer baza listi'}
                            </span>
              ...
              {['new', 'courier_base'].includes(filter) && dailyNewBatches.length > 0 ? (
                <div className="courier-packer-batches" aria-label="Kunlik yangi zakazlar listlari">
                  {dailyNewBatches.map((batch) => {
                    ...
                    if (!batchOrdersRaw.length) return null;
                    const batchOrders = sortBatchOrdersByAssignedTimeAsc(batchOrdersRaw);
                    const open = isDailyNewBatchExpanded(batch.id);
                    ...
                    const activateDailyToolbar = () => {
                      if (filter === 'new') {
                        toggleDailyNewBatchAdaptive(batch.id);
                        return;
                      }
                    ...
                    return (
                      <section key={batch.id} className="courier-packer-batch">
                        <div
                          ...
                          tabIndex={0}
                          aria-expanded={open}
                          onClick={activateDailyToolbar}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter' || e.key === ' ') {
                              e.preventDefault();
                              activateDailyToolbar();
                            }
                          }}
                        >
                          <div className="courier-packer-batch-meta">

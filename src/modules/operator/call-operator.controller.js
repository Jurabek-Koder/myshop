...
import { TEST_CONVERSATION_VAPI_ID, TEST_DIALOGUE_LINES } from './ai-test-dialogue.js';
...
export function handleSeedTestAiConversation(req, res) {
  ...
  const tx = db.transaction(() => {
    db.prepare('DELETE FROM ai_call_transcripts WHERE vapi_call_id = ?').run(TEST_CONVERSATION_VAPI_ID);
    const ins = db.prepare(
      `INSERT INTO ai_call_transcripts (order_id, vapi_call_id, event_type, ciphertext, iv, auth_tag)
       VALUES (?, ?, ?, ?, ?, ?)`,
    );
    for (const line of TEST_DIALOGUE_LINES) {
      const enc = encryptTranscriptText(line.text);
      ins.run(orderId, TEST_CONVERSATION_VAPI_ID, line.event_type, enc.ciphertext, enc.iv, enc.auth_tag);
    }
  });
  tx();
  ...
  return res.json({
    ok: true,
    inserted: TEST_DIALOGUE_LINES.length,
    vapi_call_id: TEST_CONVERSATION_VAPI_ID,
    order_id: orderId,
  });
}
...
export async function handleTestDialogAudioClips(req, res) {
  ...
  try {
    for (let i = 0; i < TEST_DIALOGUE_LINES.length; i++) {
      const line = TEST_DIALOGUE_LINES[i];

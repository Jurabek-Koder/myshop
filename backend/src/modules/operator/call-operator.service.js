...
function ensureQueue() {
  ...
  try {
    queue = redisUrl
      ? new Bull('ai_call_operator', redisUrl)
      : new Bull('ai_call_operator', {
...
/** Render prod: Redis bo‘lmasa navbat o‘rniga to‘g‘ridan-to‘g‘ri Vapi chaqiruv. */
function shouldUseDirectAiCall() {
  const flag = String(process.env.MYSHOP_AI_CALL_DIRECT || '').trim();
  if (flag === '1' || flag.toLowerCase() === 'true') return true;
  if (flag === '0' || flag.toLowerCase() === 'false') return false;
  if (String(process.env.REDIS_URL || '').trim()) return false;
  return process.env.NODE_ENV === 'production' || String(process.env.RENDER || '').trim().toLowerCase() === 'true';
}

export function logAiOperatorEnvStatus() {
  const missing = [];
  if (!String(process.env.MYSHOP_VAPI_TOKEN || '').trim()) missing.push('MYSHOP_VAPI_TOKEN');
  if (!String(process.env.MYSHOP_VAPI_PHONE_NUMBER_ID || '').trim()) missing.push('MYSHOP_VAPI_PHONE_NUMBER_ID');
  if (!String(process.env.MYSHOP_VAPI_WEBHOOK_URL || '').trim()) missing.push('MYSHOP_VAPI_WEBHOOK_URL');
  if (!String(process.env.MYSHOP_APP_SECRET_KEY || '').trim()) missing.push('MYSHOP_APP_SECRET_KEY');
  if (!readEncryptionKey()) missing.push('MYSHOP_AI_TRANSCRIPTS_AES_KEY (32 byte hex yoki base64)');

  const provider = String(process.env.MYSHOP_VAPI_VOICE_PROVIDER || 'openai').trim().toLowerCase();
  if (
    (provider === '11labs' || provider === 'elevenlabs') &&
    !String(process.env.MYSHOP_VAPI_ELEVENLABS_VOICE_ID || '').trim()
  ) {
    missing.push('MYSHOP_VAPI_ELEVENLABS_VOICE_ID');
  }

  if (missing.length) {
    console.warn('[MyShop AI Call] Yetishmayotgan env:', missing.join(', '));
  } else {
    console.log('[MyShop AI Call] Asosiy Vapi env to‘ldirilgan.');
  }

  if (String(process.env.REDIS_URL || '').trim()) {
    console.log('[MyShop AI Call] Redis navbat: REDIS_URL');
  } else if (shouldUseDirectAiCall()) {
    console.log('[MyShop AI Call] Redis yo‘q — qo‘ng‘iroqlar to‘g‘ridan-to‘g‘ri Vapi orqali (navbatsiz).');
  } else {
    console.warn(
      '[MyShop AI Call] REDIS_URL yo‘q — Bull 127.0.0.1:6379 ga ulanadi; Render’da qo‘ng‘iroqlar ishlamasligi mumkin.',
    );
  }
}
...
export async function enqueueAiCallForPendingOrder({ orderId, operatorId } = {}) {
  if (shouldUseDirectAiCall()) {
    return startAiCallForOrderNow({ orderId, operatorId });
  }

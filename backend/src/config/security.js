...
export const security = {
  ...
  cors: {
    origins: (() => {
      ...
      const fromFrontend = process.env.FRONTEND_URL
        ...
        : [];
      /** Render Web Service HTTPS manzili — bitta servis (SPA+/api) da CORS bo‘sh bo‘lsa ham ishlaydi */
      const fromRender = process.env.RENDER_EXTERNAL_URL
        ? [String(process.env.RENDER_EXTERNAL_URL).replace(/\/$/, '').trim()].filter(Boolean)
        : [];
      const merged = [...new Set([...fromList, ...fromFrontend, ...fromRender])];
  ...
  bodyLimit: '30mb',
  // MyShop Shield: staged rejim oddiy foydalanuvchini asraydi, aniq hujumlarni bloklaydi.
  shield: {
    enabled: String(process.env.MYSHOP_SHIELD_ENABLED || '1').trim() !== '0',
    mode: ['monitor', 'staged', 'strict'].includes(String(process.env.MYSHOP_SHIELD_MODE || '').trim().toLowerCase())
      ? String(process.env.MYSHOP_SHIELD_MODE).trim().toLowerCase()
      : 'staged',
    windowMs: 60 * 1000,
    warnPerMinute: Number(process.env.MYSHOP_SHIELD_WARN_PER_MIN || 90),
    blockPerMinute: Number(process.env.MYSHOP_SHIELD_BLOCK_PER_MIN || 240),
    strictBlockPerMinute: Number(process.env.MYSHOP_SHIELD_STRICT_BLOCK_PER_MIN || 150),
  },
  // Parol talablari
  password: {
    minLength: 8,

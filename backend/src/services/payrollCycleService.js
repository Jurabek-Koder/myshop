...
export function upsertPayrollRoleDefault(payload, actorUserId) {
  ...
  if (existing) {
    ...
    return db.prepare(`SELECT * FROM payroll_role_defaults WHERE id = ?`).get(existing.id);
  }
  ...
  return db.prepare(`SELECT * FROM payroll_role_defaults WHERE id = ?`).get(r.lastInsertRowid);
}
...
export function upsertPayrollEmployee(payload, actorUserId) {
  ...
  if (userId) {
    ...
    if (existing) {
      ...
      return db.prepare(`SELECT * FROM payroll_employees WHERE user_id = ?`).get(userId);
    }
    ...
    return db.prepare(`SELECT * FROM payroll_employees WHERE id = ?`).get(r.lastInsertRowid);
  }
  ...
  if (existing) {
    ...
    return db.prepare(`SELECT * FROM payroll_employees WHERE work_role_id = ?`).get(workRoleId);
  }

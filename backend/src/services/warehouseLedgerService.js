...
/**
 * Kirim tasdiqlanganda stock oshirish (birinchi marta yoki qty o‘zgarganda delta).
 */
...
/**
 * Chiqim tasdiqlanganda stock kamaytirish.
 */
...
/** Kirim tasdiqini bekor qilganda stockni qaytarish. */
export function reverseKirimStock({
  ...
  const appliedRow = db
    .prepare('SELECT coalesce(warehouse_kirim_stock_applied, 0) AS applied FROM products WHERE id = ?')
    .get(productId);
  const q = Number(appliedRow?.applied) || 0;
  if (q < 1) return { applied: false, stock_before: null, stock_after: null, event_id: null };

  const stockBefore = Number(meta.stock) || 0;
  const stockAfter = Math.max(0, stockBefore - q);

  const eventId = db.transaction(() => {
    db.prepare(
      'UPDATE products SET stock = ?, warehouse_kirim_stock_applied = 0 WHERE id = ?',

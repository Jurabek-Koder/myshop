...
// DB_DIR env: Render Persistent Disk uchun /data, lokal uchun backend/data
const dataDir = process.env.DB_DIR || join(__dirname, '..', '..', 'data');
if (!existsSync(dataDir)) mkdirSync(dataDir, { recursive: true });

const dbPath = join(dataDir, 'myshop.db');
...
export function initDatabase() {
  ...
  if (adSlideCount === 0) {
    const ins = db.prepare(
      'INSERT INTO ad_slides (sort_order, title, subtitle, active) VALUES (?, ?, ?, 1)',
    );
    const defaults = [
      ['Yangi kelganlar', "Eng so'nggi mahsulotlar do'konimizda"],
      ['Chegirmalar', "Aksiyali narxlardan bahramand bo'ling"],
      ["Bepul yetkazib berish", "500 000 so'mdan ortiq buyurtmalarda"],
      ['Tez yetkazib berish', 'Buyurtmangiz 1–3 kun ichida'],
      ['Kafolat', 'Sifat kafolati va qaytarish imkoniyati'],
      ['MyShop', "Xavfsiz va qulay onlayn do'kon"],
    ];
    defaults.forEach((pair, i) => {
      ins.run(i, pair[0], pair[1]);
    });
  }
  ...
  /* password_plain olib tashlandi — parollar faqat bcrypt hash sifatida saqlanadi */

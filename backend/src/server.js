...
import { backfillStaffWorkRoleIds } from './lib/workRoleArchive.js';
...
import { registerCoreEventSubscribers } from './events/subscribers/coreSubscribers.js';
import { registerCrmEventSubscribers } from './events/subscribers/crmSubscribers.js';
import { initializeRealtimeServer } from './realtime/socketServer.js';
...
import chatGroupsRoutes from './routes/chatGroups.js';
import chatUnreadRoutes from './routes/chatUnread.js';
...
initDatabase();
registerCoreEventSubscribers();
registerCrmEventSubscribers();
try {
...
try {
  backfillStaffWorkRoleIds();
} catch (e) {
  console.warn('[MyShop] backfillStaffWorkRoleIds:', e?.message || e);
}
...
const productUploadsRoot = getUploadsRoot();
app.use('/uploads', express.static(productUploadsRoot, { maxAge: '7d' }));
app.use('/api/uploads', express.static(productUploadsRoot, { maxAge: '7d' }));
...
app.use('/api/chat', chatUnreadRoutes);

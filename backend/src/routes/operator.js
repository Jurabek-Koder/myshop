...
router.post('/orders/:orderId/create-lead', (req, res) => {
  ...
  if (pendingLead) {
    db.prepare(`
      ...
      SET status = 'contacted', operator_id = ?, product_id = ?, contact_phone = ?, contact_email = ?, full_name = ?
      WHERE id = ?
    `).run(req.user.id, item.product_id, contact_phone, contact_email, full_name, pendingLead.id);
    const lead = db.prepare(`
      SELECT l.*, p.name_uz as product_name, p.price as product_price
      FROM product_leads l JOIN products p ON p.id = l.product_id WHERE l.id = ?
    `).get(pendingLead.id);
  ...
  const result = db.prepare(`
    INSERT INTO product_leads (product_id, contact_phone, contact_email, full_name, status, operator_id, order_id)
    VALUES (?, ?, ?, ?, 'contacted', ?, ?)
  `).run(item.product_id, contact_phone, contact_email, full_name, req.user.id, orderId);

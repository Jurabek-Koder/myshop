...
import { notifyAllSuperusers } from '../lib/staffWithdrawalFlow.js';
...
function resolveWithdrawalWorkerUserId(workRole, sellerRow) {
...
router.patch('/withdrawal-requests/:id', (req, res) => {
  ...
  if (status !== 'approved' && status !== 'rejected')
    return res.status(400).json({ error: 'status: approved yoki rejected bo\'lishi kerak.' });
  ...
  if (!workRole && !sellerRow) return res.status(404).json({ error: 'Bog\'lanish topilmadi.' });
  ...
  if (status === 'approved') {
    ...
    if (notifyUserId) {
      db.prepare('INSERT INTO user_notifications (user_id, title, body) VALUES (?, ?, ?)').run(
        ...
        note || `So'rovingiz superuser tomonidan tasdiqlandi. ${amountStr}. Buxgalteriya pul berishini yozib oladi.`,
      );
  ...
  if (status === 'rejected') {
    if (notifyUserId) {
      ...
      `).run(sellerRow.id, 'Pul yechish rad etildi', note || "So'rovingiz rad etildi.");
    }
  }
  res.json({ ok: true });
});
...
router.patch('/withdrawal-requests/:id/mark-paid', (req, res) => {
  ...
  if (String(row.status) !== 'approved')
    return res.status(400).json({ error: 'Faqat tasdiqlangan so\'rov uchun.' });
  if (row.paid_out_at != null && String(row.paid_out_at).trim() !== '')
    return res.status(400).json({ error: 'Pul berilgani allaqachon qayd etilgan.' });

...
import { adSlidesUploadPath } from '../config/dataPaths.js';
import {
  parseHomeBenefitsJson,
  coerceHomeBenefitsShape,
...
import {
  archiveWorkRole,
  linkStaffMembersToWorkRole,
  restoreArchivedWorkRole,
  sqlStaffHasActiveWorkRole,
  workRoleHasArchiveHistory,
} from '../lib/workRoleArchive.js';
...
const STAFF_TYPES = new Set(['courier', 'operator', 'packer', 'picker', 'expeditor', 'order_receiver', 'warehouse_admin']);
const STAFF_STATUSES = new Set(['active', 'blocked', 'pending']);
const WORK_ROLE_STATUSES = new Set(['active', 'blocked', 'pending']);
const WORK_PORTAL_ROLES = new Set([
  'seller',
  'target',
  'courier',
  'operator',
...
/**
 * Hudud (viloyat) biriktiriladigan portal rollari.
 * Kuryerdan tashqari ekspeditor/order_receiver ham o'z hududi bo'yicha ishlaydi
 * (backend/src/routes/expeditor.js -> validateRegionAccess shu maydonga tayanadi).
 */
const REGION_SCOPED_PORTAL_ROLES = new Set(['courier', 'expeditor', 'order_receiver']);
...
const AD_SLIDES_UPLOAD_DIR = adSlidesUploadPath();
...
router.get('/staff', (req, res) => {
  ...
  let sql = `
    ...
    WHERE 1 = 1
      AND ${sqlStaffHasActiveWorkRole('s')}
  `;
  const params = [];
...
function getSettingRaw(key, fallback = '') {
  const row = db.prepare('SELECT value FROM app_settings WHERE key = ?').get(key);
  return row ? String(row.value ?? '') : fallback;
}

const FOOTER_STRIP_TEXT_MAX = 500;
const FOOTER_STRIP_PHONE_MAX = 80;
const FOOTER_STRIP_DEFAULT_TEXT = "Bepul yetkazib berish — 500 000 so'mdan ortiq buyurtmalarda";
const FOOTER_STRIP_DEFAULT_PHONE = '+998 71 123 45 67';

function footerStripAdminDto() {
  return {
    live: {
      promo_text: getSetting('footer_strip_text_live', FOOTER_STRIP_DEFAULT_TEXT),
      phone: getSetting('footer_strip_phone_live', FOOTER_STRIP_DEFAULT_PHONE),
    },
    draft: {
      promo_text: getSetting('footer_strip_text_draft', ''),
      phone: getSetting('footer_strip_phone_draft', ''),
    },
  };
}

router.get('/footer-strip', (_req, res) => {
  res.json(footerStripAdminDto());
});

router.patch('/footer-strip/draft', (req, res) => {
  const promo_text = String(req.body?.promo_text ?? '').trim().slice(0, FOOTER_STRIP_TEXT_MAX);
  const phone = String(req.body?.phone ?? '').trim().slice(0, FOOTER_STRIP_PHONE_MAX);
  setSetting('footer_strip_text_draft', promo_text);
  setSetting('footer_strip_phone_draft', phone);
  res.json({ ok: true, draft: { promo_text, phone }, live: footerStripAdminDto().live });
});

router.post('/footer-strip/publish', (_req, res) => {
  let t = String(getSetting('footer_strip_text_draft', '')).trim().slice(0, FOOTER_STRIP_TEXT_MAX);
  let p = String(getSetting('footer_strip_phone_draft', '')).trim().slice(0, FOOTER_STRIP_PHONE_MAX);
  if (!t) t = FOOTER_STRIP_DEFAULT_TEXT;
  if (!p) p = FOOTER_STRIP_DEFAULT_PHONE;
  setSetting('footer_strip_text_live', t);
  setSetting('footer_strip_phone_live', p);
  res.json({ ok: true, ...footerStripAdminDto() });
});

function homeBenefitsAdminDto() {
  const liveStr = getSettingRaw('home_benefits_live', '');
  const draftStr = getSettingRaw('home_benefits_draft', '');
  const liveParsed = parseHomeBenefitsJson(liveStr.trim() ? liveStr : '');
  const draftParsed = draftStr.trim() ? parseHomeBenefitsJson(draftStr) : null;
  const live = coerceHomeBenefitsShape(liveParsed || HOME_BENEFITS_DEFAULT);
  const draft = coerceHomeBenefitsShape(draftParsed != null ? draftParsed : liveParsed || HOME_BENEFITS_DEFAULT);
  return { live, draft };
}

router.get('/home-benefits', (_req, res) => {
  res.json(homeBenefitsAdminDto());
});

router.patch('/home-benefits/draft', (req, res) => {
  const coerced = coerceHomeBenefitsShape(req.body);
  setSetting('home_benefits_draft', JSON.stringify(coerced));
  res.json({ ok: true, ...homeBenefitsAdminDto() });
});

router.post('/home-benefits/publish', (_req, res) => {
  const draftStr = getSettingRaw('home_benefits_draft', '');
  const liveStr = getSettingRaw('home_benefits_live', '');
  const parsed =
    parseHomeBenefitsJson(draftStr.trim() ? draftStr : '') ||
    parseHomeBenefitsJson(liveStr.trim() ? liveStr : '');
  const coerced = coerceHomeBenefitsShape(parsed || HOME_BENEFITS_DEFAULT);
  setSetting('home_benefits_live', JSON.stringify(coerced));
  res.json({ ok: true, ...homeBenefitsAdminDto() });
});
...
router.get('/accounting', (req, res) => {
  ...
  if (q) {
    ...
    if (ent.kind === 'work_role') {
      ...
      const financePayload = buildWorkRoleFinancePayload(wr, { limit: 200 });
      return res.json({
        ...
        finance: {
          ...
          timeline: financePayload.timeline,
          fines: financePayload.fines,
          rewards: financePayload.rewards,
          withdrawals: financePayload.withdrawals,
        },
        daily: [],
        statusBreakdown: [],
        sellers: emptySellers,
...
/** Buxgalteriya qidiruv natijasi — to‘liq moliya (tranzaksiyalar, yechishlar). */
router.get('/accounting/moliya', (req, res) => {
  const q = req.query.q ? String(req.query.q).trim() : '';
  if (!q) return res.status(400).json({ error: 'Qidiruv matni kerak.' });

  const ent = resolveAccountingEntity(q);
  if (!ent) {
    return res.status(404).json({ error: 'Topilmadi.', filter: { type: 'none', label: q } });
  }

  if (ent.kind === 'work_role') {
    const wr = db.prepare('SELECT * FROM work_roles WHERE id = ? AND deleted_at IS NULL').get(ent.id);
    if (!wr) return res.status(404).json({ error: 'Ish roli topilmadi.' });

    const options = { limit: 500 };
    const portalRole = String(wr.portal_role || '').toLowerCase();
    if (portalRole === 'operator' && wr.login) {
      const opUser = db
        .prepare(`SELECT id FROM users WHERE lower(trim(login)) = lower(trim(?)) AND lower(trim(role)) = 'operator'`)
        .get(wr.login);
      if (opUser) options.operatorId = opUser.id;
    }

    const payload = buildWorkRoleFinancePayload(wr, options);
    return res.json({
      filter: { type: 'work_role', id: wr.id, label: wr.role_name || ent.label },
  ...
  if (ent.kind === 'staff') {
    const sm = db.prepare('SELECT * FROM staff_members WHERE id = ?').get(ent.id);
    if (!sm) return res.status(404).json({ error: 'Xodim topilmadi.' });

    let user = null;
    if (sm.user_id != null && Number(sm.user_id) > 0) {
      user = db.prepare('SELECT id, login, role FROM users WHERE id = ?').get(sm.user_id);
    }
    if (!user) {
      user = db.prepare('SELECT id, login, role FROM users WHERE staff_member_id = ? ORDER BY id DESC LIMIT 1').get(sm.id);
    }

    if (user && String(user.role || '').toLowerCase() === 'courier') {
      const wr = db
        .prepare(`SELECT * FROM work_roles WHERE login = ? AND lower(trim(portal_role)) = 'courier' AND deleted_at IS NULL`)
        .get(user.login);
      if (wr) {
        const payload = buildWorkRoleFinancePayload(wr, { limit: 500 });
        return res.json({
          filter: { type: 'staff', id: sm.id, label: sm.full_name || ent.label, user_id: user.id },
          balance: payload.summary.balance,
          summary: payload.summary,
          transactions: payload.transactions,
          timeline: payload.timeline,
    ...
    if (user?.login) {
      ...
      const wr = db
        .prepare(`SELECT * FROM work_roles WHERE login = ? AND lower(trim(portal_role)) = ? AND deleted_at IS NULL`)
        .get(user.login, roleLower);
      if (wr) {
        const options = { limit: 500 };
        if (roleLower === 'operator') options.operatorId = user.id;
        const payload = buildWorkRoleFinancePayload(wr, options);
        return res.json({
          filter: { type: 'staff', id: sm.id, label: sm.full_name || ent.label, user_id: user.id },
          balance: payload.summary.balance,
    ...
    return res.json({
      filter: { type: 'staff', id: sm.id, label: sm.full_name || ent.label },
      balance: Number(sm.balance) || 0,
      summary: { balance: Number(sm.balance) || 0, reward_amount: 0, fine_amount: 0, fines_count: 0 },
      transactions: [],
      timeline: [],
      fines: [],
      rewards: [],
  ...
  if (ent.kind === 'seller') {
    const seller = db.prepare('SELECT id, name, balance FROM sellers WHERE id = ?').get(ent.id);
    if (!seller) return res.status(404).json({ error: 'Seller topilmadi.' });
    return res.json({
      filter: { type: 'seller', id: seller.id, label: seller.name || ent.label },
      balance: Number(seller.balance) || 0,
      summary: { balance: Number(seller.balance) || 0, reward_amount: 0, fine_amount: 0, fines_count: 0 },
      transactions: [],
      timeline: [],
      fines: [],
      rewards: [],
  ...
  return res.json({
    filter: { type: ent.kind, id: ent.id, label: ent.label },
    balance: 0,
    summary: { balance: 0, reward_amount: 0, fine_amount: 0, fines_count: 0 },
    transactions: [],
    timeline: [],
    fines: [],
    rewards: [],
...
router.get('/work-roles/trash', (_req, res) => {
  const rows = db
    .prepare("SELECT * FROM work_roles WHERE deleted_at IS NOT NULL AND COALESCE(safe_archived,0)=0 ORDER BY deleted_at DESC")
    .all();
  res.json({ roles: rows.map(roleRowToDto) });
});

router.get('/work-roles/safe-archived', (_req, res) => {
  const rows = db.prepare('SELECT * FROM work_roles WHERE COALESCE(safe_archived,0)=1 ORDER BY id DESC').all();
  res.json({ roles: rows.map(roleRowToDto) });
});

/** Savatni to'zala — moliya tarixsiz arxiv rollarni butunlay o'chirish */
router.delete('/work-roles/trash', (_req, res) => {
  const trashed = db.prepare('SELECT id FROM work_roles WHERE deleted_at IS NOT NULL').all();
  let deleted = 0;
  let skipped = 0;
  for (const row of trashed) {
    if (workRoleHasArchiveHistory(row.id)) {
      skipped += 1;
      continue;
    }
    db.prepare('DELETE FROM work_roles WHERE id = ?').run(row.id);
    deleted += 1;
  }
  res.json({
    ok: true,
    ...
    message:
      skipped > 0
        ? `${skipped} ta rol moliya/zakaz tarixi tufayli arxivda qoldi.`
        : undefined,
  });
});
...
router.post('/work-roles', (req, res) => {
  ...
  // Eslatma: ekspeditor uchun hudud tanlash ixtiyoriy — bo'sh qoldirilsa,
  // bu ekspeditor butun O'zbekiston bo'yicha barcha kuryer/zakazlarni ko'radi.
  ...
  `).run(
    ...
    status,
    portal_role,
    REGION_SCOPED_PORTAL_ROLES.has(portal_role) ? courier_viloyat_id : null,
    REGION_SCOPED_PORTAL_ROLES.has(portal_role) ? courier_tuman_ids_json : '[]',
  );
  ...
  try {
    syncPayrollRoleDefaults();
  } catch (e) {
    console.warn('[portal] payroll role defaults sync:', e?.message || e);
  }
...
router.post('/work-roles/:id/restore', (req, res) => {
  ...
  restoreArchivedWorkRole(existing);
...
router.delete('/work-roles/:id/permanent', (req, res) => {
  ...
  if (row.deleted_at == null) {
    return res.status(400).json({ error: 'Faqat arxivdagi (o\'chirilgan) rolni butunlay o\'chirish mumkin.' });
  }
  denyPortalAccessForWorkRole(row);
  const hasHistory = db.prepare('SELECT 1 FROM work_role_ledger_entries WHERE work_role_id = ? LIMIT 1').get(id);
  if (hasHistory) {
    db.prepare("UPDATE work_roles SET safe_archived = 1 WHERE id = ?").run(id);
    res.json({ ok: true, safe_archived: true });
  } else {
...
router.get('/work-roles/safe/status', (_req, res) => {
  const row = db.prepare("SELECT value FROM app_settings WHERE key = 'archive_safe_password_hash'").get();
  res.json({ set: Boolean(row?.value) });
});

router.post('/work-roles/safe/password', (req, res) => {
  const password = String(req.body?.password || '').trim();
  if (!password || password.length < 4) {
    return res.status(400).json({ error: 'Parol kamida 4 belgidan iborat bo‘lishi kerak.' });
  }
  const hash = bcrypt.hashSync(password, 12);
  db.prepare('INSERT OR REPLACE INTO app_settings (key, value) VALUES (?, ?)').run('archive_safe_password_hash', hash);
  res.json({ ok: true });
});

router.post('/work-roles/safe/open', (req, res) => {
  const password = String(req.body?.password || '').trim();
  const hashRow = db.prepare("SELECT value FROM app_settings WHERE key = 'archive_safe_password_hash'").get();
  const hash = String(hashRow?.value || '').trim();
  if (!hash) {
    return res.status(400).json({ error: 'Seyf paroli o‘rnatilmagan. Superuser sozlamalaridan o‘rnating.' });
  }
  try {
    const match = bcrypt.compareSync(password, hash);
    if (!match) return res.status(401).json({ error: 'Parol noto‘g‘ri.' });
  } catch (e) {
    return res.status(500).json({ error: 'Parol tekshirishda xato.' });
  }
  res.json({ ok: true });
});
...
router.get('/work-roles/:id/finance', (req, res) => {
  const id = Number.parseInt(req.params.id, 10);
  if (Number.isNaN(id) || id < 1) return res.status(400).json({ error: 'Noto\'g\'ri ID.' });
  const row = db.prepare('SELECT * FROM work_roles WHERE id = ?').get(id);
  if (!row) return res.status(404).json({ error: 'Rol topilmadi.' });
  const payload = buildWorkRoleFinancePayload(row);
  const kirim = payload.timeline
    .filter((t) => Number(t.signed_amount) > 0)
    .reduce((s, t) => s + Number(t.signed_amount), 0);
  const chiqim = payload.timeline
    .filter((t) => Number(t.signed_amount) < 0)
    .reduce((s, t) => s + Math.abs(Number(t.signed_amount)), 0);
  res.json({
    role: roleRowToDto(row),
    balance: Number(payload.summary.balance) || 0,
    kirim,
    chiqim,
    summary: payload.summary,
    activity: payload.timeline.slice(0, 50),
  });
});
...
router.patch('/work-roles/:id', (req, res) => {
  ...
  if (req.body?.portal_role !== undefined) {
    ...
    if (!pr) portal_role = null;
      ...
      return res.status(400).json({
        error:
          'portal_role: seller, target, courier, operator, picker, packer, expeditor, order_receiver, warehouse_admin yoki accounting.',
      });
    }
  }
  ...
  // Ekspeditor uchun hudud ixtiyoriy — bo'sh bo'lsa, butun O'zbekiston ko'rinadi.
  if (!REGION_SCOPED_PORTAL_ROLES.has(portal_role)) {
...
router.post('/work-roles/:id/actions', (req, res) => {
  ...
  if (action === 'activate') {
    ...
    const activated = db.prepare('SELECT * FROM work_roles WHERE id = ?').get(id);
    restorePortalAccessForWorkRole(activated);
    linkStaffMembersToWorkRole(activated);
...
router.get('/withdrawal-requests', (req, res) => {
  ...
  const legacyStatus =
    req.query.status === 'approved' || req.query.status === 'rejected' ? String(req.query.status) : null;
  ...
  const selectCols = `
    ...
    wr.payout_method,
    wr.paid_out_at,
    COALESCE(w.login, '') AS work_role_login,
    COALESCE(w.role_name, '') AS role_name,
    COALESCE(w.phone, '') AS work_role_phone,
    COALESCE(w.email, '') AS work_role_email,
    COALESCE(s.name, '') AS seller_name,
    COALESCE(s.contact_phone, '') AS seller_phone,
    COALESCE(s.email, '') AS seller_email
  `;

  const baseFrom = `
    FROM withdrawal_requests wr
    LEFT JOIN work_roles w ON w.id = wr.work_role_id
  ...
  if (queue === 'awaiting_payout') {
    sql = `SELECT ${selectCols} ${baseFrom}
      WHERE wr.status = 'approved' AND (wr.paid_out_at IS NULL OR trim(coalesce(wr.paid_out_at,'')) = '')
      ORDER BY datetime(COALESCE(wr.reviewed_at, wr.created_at)) ASC, wr.id ASC`;
...
router.post('/products/:id/approve-ai-target', (req, res) => {
  const id = parseInt(req.params.id, 10);
  if (Number.isNaN(id) || id < 1) return res.status(400).json({ error: 'Noto\'g\'ri ID.' });
  const row = db
    .prepare(
      `
    SELECT id, seller_id, name_uz, COALESCE(ai_marketing_opt_in, 0) AS ai_marketing_opt_in
    FROM products WHERE id = ?
  `,
    )
    .get(id);
  if (!row) return res.status(404).json({ error: 'Mahsulot topilmadi.' });
  if (Number(row.ai_marketing_opt_in) !== 1)
    return res.status(400).json({ error: 'Bu mahsulotda AI marketing yoqilmagan.' });

  db.prepare(`
    UPDATE products SET ai_target_approved_at = datetime('now'), ai_target_approved_by = ? WHERE id = ?
  `).run(req.user.id, id);

  if (row.seller_id) {
    db.prepare(`
      INSERT INTO seller_notifications (seller_id, title, message, type, link_view, is_read)
      VALUES (?, ?, ?, 'success', 'products', 0)
    `).run(
      row.seller_id,
      'AI Target tasdiqlandi',
      `«${row.name_uz || 'Mahsulot'}» uchun targeting ishga tushirishga ruxsat berildi (#${id}).`,
    );
  }
...
router.get('/ad-slides', (_req, res) => {
  const rows = db
    .prepare(
      'SELECT id, sort_order, title, subtitle, link_url, image_url, image_url_mobile, video_url, active, created_at FROM ad_slides ORDER BY sort_order ASC, id ASC',
    )
    .all();
...
router.post('/ad-slides', (req, res) => {
  ...
  const image_url_mobile = String(req.body?.image_url_mobile || '').trim().slice(0, 800) || null;
  ...
  const result = db
    .prepare(
      'INSERT INTO ad_slides (sort_order, title, subtitle, link_url, image_url, image_url_mobile, video_url, active) VALUES (?,?,?,?,?,?,?,?)',
    )
    .run(maxOrder + 1, title, subtitle, link_url, image_url, image_url_mobile, video_url, active);
...
router.patch('/ad-slides/:id', (req, res) => {
  ...
  let image_url_mobile = existing.image_url_mobile;
  if (req.body?.image_url_mobile !== undefined) {
    const vm = String(req.body.image_url_mobile || '').trim().slice(0, 800);
    image_url_mobile = vm || null;
  }
  ...
  db.prepare(
    'UPDATE ad_slides SET title = ?, subtitle = ?, link_url = ?, image_url = ?, image_url_mobile = ?, video_url = ?, active = ? WHERE id = ?',
  ).run(title, subtitle, link_url, image_url, image_url_mobile, video_url, active, id);

...
const AD_SLIDES_UPLOAD_DIR = path.join(process.env.UPLOADS_DIR || process.cwd(), 'uploads', 'ad-slides');
...
// ============================================================
// SAYT XABARLARI (Site Messages) — Admin boshqaradi
// ============================================================

// DB jadvalini yaratish (agar yo'q bo'lsa) — server start da database.js da qilish yaxshiroq,
// lekin qulaylik uchun shu yerda ham ensureTable qilamiz
db.exec(`
  CREATE TABLE IF NOT EXISTS site_messages (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    kind TEXT NOT NULL DEFAULT 'convenience',
    title TEXT NOT NULL,
    body TEXT NOT NULL,
...
// Ommaviy: barcha faol xabarlarni olish
router.get('/site-messages', (_req, res) => {
  const msgs = db.prepare(
    "SELECT id, kind, title, body FROM site_messages WHERE active = 1 ORDER BY sort_order ASC, id ASC"
  ).all();
  res.json({ messages: msgs });
});

// Admin: barcha xabarlar (faol + nofaol)
router.get('/admin/site-messages', (_req, res) => {
  const msgs = db.prepare(
    'SELECT * FROM site_messages ORDER BY sort_order ASC, id ASC'
  ).all();
  res.json({ messages: msgs });
});

// Admin: yangi xabar qo'shish
router.post('/admin/site-messages', (req, res) => {
  const kind = String(req.body?.kind || 'convenience').trim();
  const title = String(req.body?.title || '').trim();
  const body = String(req.body?.body || '').trim();
  const active = req.body?.active === false ? 0 : 1;
  const sort_order = parseInt(req.body?.sort_order, 10) || 0;
  if (!title) return res.status(400).json({ error: 'Sarlavha kerak.' });
  if (!body) return res.status(400).json({ error: 'Matn kerak.' });
  const r = db.prepare(
    'INSERT INTO site_messages (kind, title, body, active, sort_order) VALUES (?, ?, ?, ?, ?)'
  ).run(kind, title, body, active, sort_order);
  const msg = db.prepare('SELECT * FROM site_messages WHERE id = ?').get(r.lastInsertRowid);
  res.status(201).json({ message: msg });
});

// Admin: xabarni tahrirlash
router.put('/admin/site-messages/:id', (req, res) => {
  const id = parseInt(req.params.id, 10);
  const existing = db.prepare('SELECT * FROM site_messages WHERE id = ?').get(id);
  if (!existing) return res.status(404).json({ error: 'Xabar topilmadi.' });
  const kind = String(req.body?.kind ?? existing.kind).trim();
  ...
  db.prepare(
    'UPDATE site_messages SET kind=?, title=?, body=?, active=?, sort_order=? WHERE id=?'
  ).run(kind, title, body, active, sort_order, id);
  const msg = db.prepare('SELECT * FROM site_messages WHERE id = ?').get(id);
  res.json({ message: msg });
});

// Admin: xabarni o'chirish
router.delete('/admin/site-messages/:id', (req, res) => {
  const id = parseInt(req.params.id, 10);
  const existing = db.prepare('SELECT id FROM site_messages WHERE id = ?').get(id);
  if (!existing) return res.status(404).json({ error: 'Xabar topilmadi.' });
  db.prepare('DELETE FROM site_messages WHERE id = ?').run(id);
  res.json({ ok: true });
});

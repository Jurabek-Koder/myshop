...
router.post('/products/:id/revoke-kirim-approval', (req, res) => {
  ...
  const row = db
    .prepare(
      'SELECT id, seller_id, warehouse_approved_at, warehouse_deleted_at FROM products WHERE id = ?',
    )
    .get(id);
  if (!row) return res.status(404).json({ error: 'Mahsulot topilmadi.' });
  if (row.warehouse_deleted_at != null && String(row.warehouse_deleted_at).trim() !== '') {
  ...
  const hasSeller = row.seller_id != null && Number(row.seller_id) >= 1;
  if (hasSeller) {
    db.prepare(
      `UPDATE products SET
...
router.post('/products/:id/approve-kirim', (req, res) => {
  ...
  if (Number.isInteger(sellerId) && sellerId >= 1 && st !== 'active' && st !== 'scheduled') {
    db.prepare(
  ...
  const actorLabel = String(req.user?.full_name || req.user?.login || req.user?.email || 'Ombor admini').slice(
    0,
    160,
  );
  const titleUz = String(product.name_uz || '').slice(0, 120);
  try {
    notifySuperusersBell({
      actorUserId: req.user?.id,
      title: 'Ombor kirimi tasdiqlandi',
      body: `${actorLabel}: «${titleUz}» omborda qabul qilindi${activatedToSale ? ' va sotuvga chiqarildi.' : '.'}`,
  ...
  res.json({ ok: true, approved_at: fresh.warehouse_approved_at, activated_to_sale: activatedToSale });
});

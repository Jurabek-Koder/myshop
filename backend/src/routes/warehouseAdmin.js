...
/**
 * Chiqim tarixi — har safar chiqim tasdiqlanganda yozib boriladigan voqealar
 * jurnali (warehouse_ledger_events). Chiqim sahifasi ostida, "necha marta,
 * qachon, qancha chiqim qilingani"ni ko'rsatish uchun.
 */
router.get('/chiqim-history', (req, res) => {
  const limit = Number.parseInt(req.query.limit, 10);
  const lim = Number.isInteger(limit) && limit > 0 ? Math.min(limit, 200) : 100;

  const feed = listWarehouseLedgerFeed({ limit: 500 }).filter((e) => e.event_type === 'chiqim');
  const trimmed = feed.slice(0, lim);

  const productIds = [...new Set(trimmed.map((e) => e.product_id))];
  const productMap = {};
  if (productIds.length > 0) {
    const placeholders = productIds.map(() => '?').join(',');
    const rows = db
      .prepare(
        `SELECT id, trim(coalesce(image_url, '')) AS image_url,
                (CASE WHEN coalesce(discount_percent, 0) > 0
                      THEN price * (1 - discount_percent / 100.0)
                      ELSE price END) AS unit_price
         FROM products WHERE id IN (${placeholders})`,
      )
      .all(...productIds);
    for (const r of rows) {
      productMap[r.id] = { image_url: r.image_url || '', unit_price: Math.round(Number(r.unit_price) || 0) };
    }
  }

  const items = trimmed.map((e) => {
    const meta = productMap[e.product_id] || { image_url: '', unit_price: 0 };
    return {
      id: e.id,
      product_id: e.product_id,
      seller_name: e.seller_name || '—',
      name_uz: e.product_name || "Noma'lum mahsulot",
      image_url: meta.image_url,
      unit_price: meta.unit_price,
      qty: e.qty,
      sum: meta.unit_price * Math.abs(e.qty),
...
router.get('/products-overview', (req, res) => {
  ...
  if (bucket === 'deleted') {
    ...
    // MUHIM: faqat HOZIR sotuvdan olingan (hali qaytarilmagan) mahsulotlar
    // ko'rsatiladi. Avval bu yerda "OR warehouse_relisted_at IS NOT NULL"
    // ham bor edi — natijada "Qaytarish" bosilgan mahsulot HAMISHA shu
    // ro'yxatda qolib ketardi (endi faol bo'lsa ham), va katakchada tugma
    // o'rniga statik "Sotuvga qaytarildi" yozuvi chiqib, "tugma ishlamayapti"
    // taassurotini berardi. Qaytarilgan mahsulot endi ushbu ro'yxatdan butunlay
    // chiqib ketadi (u "faol" bo'limda ko'rinadi).
    clauses.push(`p.warehouse_delisted_at IS NOT NULL AND trim(coalesce(p.warehouse_delisted_at, '')) != ''`);
...
router.post('/products/:id/confirm-chiqim', (req, res) => {
  ...
  // MUHIM: "allaqachon tasdiqlangan" bloki ATAYLAB olib tashlangan — ombor
  // admini BIR mahsulotdan bir necha marta, alohida partiyalarda chiqim
  // qilishi mumkin bo'lishi kerak (masalan avval 50, keyin yana 30 dona).
  // Har bir tasdiqlash "Chiqim tarixi"da ALOHIDA yozuv sifatida saqlanadi
  // (pastda insertEvent orqali) — shu bilan hisob-kitob to'g'ri va tekshirib
  // bo'ladigan holda qoladi. "Tasdiq bekor" funksiyasi ENG OXIRGI tasdiqni
  // bekor qiladi (warehouse_chiqim_qty/confirmed_at doim ENG OXIRGISI).
...
router.get('/staff-directory/:role', (req, res) => {
  ...
  const staff = baseRows.map((r) => {
    ...
    if (role === 'courier' && r.staff_member_id) {
      ...
      // Seller omborga kirim qilgan (tasdiqlangan) barcha mahsulotlarining
      // umumiy summasi: har bir mahsulot uchun (narx * soni), keyin yig'indi.
      const sumRow = db
        .prepare(
          `SELECT COALESCE(SUM(
             (CASE WHEN coalesce(discount_percent, 0) > 0
                   THEN price * (1 - discount_percent / 100.0)
                   ELSE price END) * coalesce(stock, 0)
           ), 0) AS total_sum
           FROM products
           WHERE seller_id = ? AND warehouse_approved_at IS NOT NULL
             AND (warehouse_deleted_at IS NULL OR warehouse_deleted_at = '')`,
        )
        .get(r.seller_id);
      row.total_product_sum = Math.round(Number(sumRow?.total_sum) || 0);
...
router.get('/staff-directory/:role/:userId/daily-summary', (req, res) => {
  ...
  if (role === 'courier' && user.staff_member_id) {
    ...
    // 1) Mahsulot — sellerning hozirgi (omborga kirim qilingan/tasdiqlangan)
    // barcha faol mahsulotlari soni va umumiy summasi. Sana bo'yicha
    // filtrlanmaydi — bu "hozirgi holat" ko'rsatkichi.
    ...
    const cancelled = db
      ...
      .get(user.seller_id, targetDate);
    // 4) Kuryer uyidagi mahsulot — sellerning shu kunga oid, kuryer uyida
    // qolib ketgan (hali topshirilmagan/sotilmagan) buyurtma-mahsulotlari.
    const atCourierHome = db
      .prepare(
        `SELECT COUNT(DISTINCT o.id) AS c, COALESCE(SUM(oi.price_at_order * oi.quantity), 0) AS s
         FROM order_items oi
         JOIN orders o ON o.id = oi.order_id
         JOIN products p ON p.id = oi.product_id
         WHERE p.seller_id = ? AND oi.home_left_in_courier = 1
           AND ${dateExpr.replace(/status_updated_at|created_at/g, 'o.$&')} = ?`,
      )
      .get(user.seller_id, targetDate);
    items.push(
      { key: 'products', label: 'Mahsulot', count: Number(productsRow.c) || 0, sum: Number(productsRow.s) || 0 },
      { key: 'sold', label: 'Sotilgan mahsulot', count: Number(sold.c) || 0, sum: Number(sold.s) || 0 },
      { key: 'cancelled', label: 'Bekor qilingan mahsulot', count: Number(cancelled.c) || 0, sum: Number(cancelled.s) || 0 },
      {
        key: 'courier_home',
        label: 'Kuryer uyidagi mahsulot',
        count: Number(atCourierHome.c) || 0,
        sum: Number(atCourierHome.s) || 0,
      },
    );
  } else if (role === 'operator' && userId) {
    // 1) Qabul qilingan — operator shu kuni qabul qilgan (yaratilgan) lidlar soni.
    const accepted = db
      .prepare(
    ...
    // 2) Tasdiqlangan — shu kuni lid orqali buyurtmaga aylantirilgan (yaratilgan) zakazlar.
    const confirmed = db
      .prepare(
        `SELECT COUNT(DISTINCT o.id) AS c, COALESCE(SUM(o.total_amount), 0) AS s
         FROM orders o
      ...
      .get(userId, targetDate);
    // 3) Bekor qilingan — shu kuni bekor qilingan (operator lidi orqali kelgan) zakazlar.
    const opCancelled = db
      .prepare(
        `SELECT COUNT(DISTINCT o.id) AS c, COALESCE(SUM(o.total_amount), 0) AS s
         FROM orders o
         JOIN product_leads pl ON pl.order_id = o.id
         WHERE pl.operator_id = ? AND lower(trim(o.status)) = 'cancelled'
           AND date(coalesce(o.status_updated_at, o.created_at)) = ?`,
      )
      .get(userId, targetDate);
    // 4) Kutilayotgan — hali javob kutayotgan/tasdiqlanmagan lidlar (joriy holat, kunga bog'liq emas).
    const pending = db
      .prepare(
        `SELECT COUNT(*) AS c FROM product_leads
         WHERE operator_id = ? AND lower(trim(coalesce(status, ''))) IN ('pending', 'contacted')`,
      )
      .get(userId);
    items.push(
      { key: 'accepted', label: 'Qabul qilingan', count: Number(accepted.c) || 0, sum: 0 },
      { key: 'confirmed', label: 'Tasdiqlangan', count: Number(confirmed.c) || 0, sum: Number(confirmed.s) || 0 },
      { key: 'cancelled', label: 'Bekor qilingan', count: Number(opCancelled.c) || 0, sum: Number(opCancelled.s) || 0 },
      { key: 'pending', label: 'Kutilayotgan', count: Number(pending.c) || 0, sum: 0 },
    );
  } else if (role === 'packer' && user.staff_member_id) {
    const sid = user.staff_member_id;
    // 1) Qadoqlangan — shu kuni qadoqlangan (packed) zakazlar.
    const packed = db
      .prepare(
        `SELECT COUNT(*) AS c, COALESCE(SUM(total_amount), 0) AS s FROM orders
         WHERE packer_id = ? AND lower(trim(status)) = 'packaged' AND ${dateExpr} = ?`,
      )
      .get(sid, targetDate);
    // 2) Yig'ilgan — hozircha "picked" holatida, qadoqlanishini kutayotgan zakazlar (joriy holat).
    const picked = db
      .prepare(
        `SELECT COUNT(*) AS c, COALESCE(SUM(total_amount), 0) AS s FROM orders
         WHERE packer_id = ? AND lower(trim(status)) = 'picked'`,
      )
      .get(sid);
    // 3) Hold — hozircha "hold" holatida turgan zakazlar (joriy holat).
    const held = db
      .prepare(
        `SELECT COUNT(*) AS c, COALESCE(SUM(total_amount), 0) AS s FROM orders
         WHERE packer_id = ? AND lower(trim(status)) = 'hold'`,
      )
      .get(sid);
    // 4) Bekor qilingan — shu packerga tegishli bo'lib, shu kuni bekor qilingan zakazlar.
    const pkCancelled = db
      .prepare(
        `SELECT COUNT(*) AS c, COALESCE(SUM(total_amount), 0) AS s FROM orders
         WHERE packer_id = ? AND lower(trim(status)) = 'cancelled' AND ${dateExpr} = ?`,
      )
      .get(sid, targetDate);
    items.push(
      { key: 'packed', label: 'Qadoqlangan', count: Number(packed.c) || 0, sum: Number(packed.s) || 0 },
      { key: 'picked', label: "Yig'ilgan", count: Number(picked.c) || 0, sum: Number(picked.s) || 0 },
      { key: 'hold', label: 'Hold', count: Number(held.c) || 0, sum: Number(held.s) || 0 },
      { key: 'cancelled', label: 'Bekor qilingan', count: Number(pkCancelled.c) || 0, sum: Number(pkCancelled.s) || 0 },
    );
  } else if (role === 'picker' && userId) {
    // Picker uchun alohida "picker_id" ustuni yo'q — uning amallari event_logs
    // jurnalida (module='picker', user_id=<picker>) saqlanadi. Shu orqali
    // "kim, qachon, qaysi zakazni terdi/hold qildi" aniqlanadi.
    // 1) Terilgan — shu kuni ushbu picker "picked" qilgan zakazlar.
    const picked2 = db
      .prepare(
        `SELECT COUNT(DISTINCT o.id) AS c, COALESCE(SUM(o.total_amount), 0) AS s
         FROM event_logs e
         JOIN orders o ON o.id = CAST(e.entity_id AS INTEGER)
         WHERE e.module = 'picker' AND e.event_type = 'OrderPicked' AND e.user_id = ?
           AND date(e.created_at) = ?`,
      )
      .get(userId, targetDate);
    // 2) Hold — shu kuni ushbu picker "hold" qilgan zakazlar.
    const held2 = db
      .prepare(
        `SELECT COUNT(DISTINCT o.id) AS c, COALESCE(SUM(o.total_amount), 0) AS s
         FROM event_logs e
         JOIN orders o ON o.id = CAST(e.entity_id AS INTEGER)
         WHERE e.module = 'picker' AND e.event_type = 'OrderHeld' AND e.user_id = ?
           AND date(e.created_at) = ?`,
      )
      .get(userId, targetDate);
    // 3) Bekor qilingan — ushbu picker tergan, keyinchalik (shu kuni) bekor qilingan zakazlar.
    const pkrCancelled = db
      .prepare(
        `SELECT COUNT(DISTINCT o.id) AS c, COALESCE(SUM(o.total_amount), 0) AS s
         FROM event_logs e
         JOIN orders o ON o.id = CAST(e.entity_id AS INTEGER)
      ...
      .get(userId, targetDate);
    // 4) Jami terilgan — shu picker tomonidan barcha vaqtda terilgan zakazlar (umumiy ko'rsatkich).
    const totalPicked = db
      .prepare(
        `SELECT COUNT(DISTINCT o.id) AS c, COALESCE(SUM(o.total_amount), 0) AS s
         FROM event_logs e
         JOIN orders o ON o.id = CAST(e.entity_id AS INTEGER)
      ...
      .get(userId);
    items.push(
      { key: 'picked', label: 'Terilgan', count: Number(picked2.c) || 0, sum: Number(picked2.s) || 0 },
      { key: 'hold', label: 'Hold', count: Number(held2.c) || 0, sum: Number(held2.s) || 0 },
      { key: 'cancelled', label: 'Bekor qilingan', count: Number(pkrCancelled.c) || 0, sum: Number(pkrCancelled.s) || 0 },
      { key: 'total', label: 'Jami terilgan', count: Number(totalPicked.c) || 0, sum: Number(totalPicked.s) || 0 },
    );
...
/**
 * Seller "Umumiy ma'lumot" modalidagi 4 ta ustun (Mahsulot / Sotilgan mahsulot /
 * Bekor qilingan mahsulot / Kuryer uyidagi mahsulot) — bosilganda shu
 * kategoriyaga oid mahsulotlar/buyurtmalar ro'yxatini qaytaradi.
 */
router.get('/staff-directory/seller/:userId/category-items', (req, res) => {
  const userId = Number.parseInt(req.params.userId, 10);
  const category = String(req.query.category || '').trim().toLowerCase();
  if (!Number.isInteger(userId) || userId < 1) {
    return res.status(400).json({ error: "ID noto'g'ri." });
  }
  if (!['products', 'sold', 'cancelled', 'courier_home'].includes(category)) {
    return res.status(400).json({ error: "Noto'g'ri kategoriya." });
  }
  const dateParam = String(req.query.date || '').trim();
  const targetDate = /^\d{4}-\d{2}-\d{2}$/.test(dateParam) ? dateParam : new Date().toISOString().slice(0, 10);

  const user = db.prepare('SELECT id, seller_id FROM users WHERE id = ?').get(userId);
  if (!user || !user.seller_id) return res.status(404).json({ error: 'Seller topilmadi.' });
  ...
  if (category === 'products') {
    const rows = db
      .prepare(
        `SELECT id AS product_id, name_uz, image_url, stock,
                (CASE WHEN coalesce(discount_percent, 0) > 0
                      THEN price * (1 - discount_percent / 100.0)
                      ELSE price END) AS unit_price
         FROM products
         WHERE seller_id = ? AND warehouse_approved_at IS NOT NULL
           AND (warehouse_deleted_at IS NULL OR warehouse_deleted_at = '')
         ORDER BY name_uz COLLATE NOCASE ASC`,
      )
      .all(sellerId);
    items = rows.map((r) => ({
      ...
      price: Math.round(Number(r.unit_price) || 0),
    }));
  } else {
    const statusFilter =
      category === 'sold'
    ...
    const rows = db
      .prepare(
        `SELECT oi.order_id, oi.product_id, oi.quantity, oi.price_at_order AS price,
         ...
         JOIN orders o ON o.id = oi.order_id
         JOIN products p ON p.id = oi.product_id
         WHERE p.seller_id = ? AND date(coalesce(o.status_updated_at, o.created_at)) = ? ${statusFilter}
         ORDER BY o.id DESC`,
      )
      .all(sellerId, targetDate);
    items = rows.map((r) => ({
      order_id: Number(r.order_id),
      product_id: r.product_id != null ? Number(r.product_id) : null,
      name_uz: String(r.name_uz || '') || "Noma'lum mahsulot",
  ...
  const totalSum = items.reduce((acc, it) => acc + it.price * it.quantity, 0);
  res.json({ category, date: targetDate, count: items.length, total_sum: totalSum, items });
});

/**
 * Operator "Umumiy ma'lumot" modalidagi 4 ta ustun (Qabul qilingan / Tasdiqlangan /
 * Bekor qilingan / Kutilayotgan) — bosilganda shu kategoriyaga oid lidlar/
 * zakazlar ro'yxatini qaytaradi.
 */
router.get('/staff-directory/operator/:userId/category-items', (req, res) => {
  const userId = Number.parseInt(req.params.userId, 10);
  const category = String(req.query.category || '').trim().toLowerCase();
  if (!Number.isInteger(userId) || userId < 1) {
    return res.status(400).json({ error: "ID noto'g'ri." });
  }
  if (!['accepted', 'confirmed', 'cancelled', 'pending'].includes(category)) {
    return res.status(400).json({ error: "Noto'g'ri kategoriya." });
  }
  const dateParam = String(req.query.date || '').trim();
  const targetDate = /^\d{4}-\d{2}-\d{2}$/.test(dateParam) ? dateParam : new Date().toISOString().slice(0, 10);
  ...
  if (category === 'accepted' || category === 'pending') {
    const statusFilter =
      category === 'pending' ? `AND lower(trim(coalesce(pl.status, ''))) IN ('pending', 'contacted')` : '';
    const dateFilter = category === 'accepted' ? `AND date(pl.created_at) = ?` : '';
    const params = category === 'accepted' ? [userId, targetDate] : [userId];
    const rows = db
      .prepare(
        `SELECT pl.id AS lead_id, pl.full_name, pl.contact_phone, pl.status,
                trim(coalesce(p.name_uz, '')) AS name_uz, trim(coalesce(p.image_url, '')) AS image_url
         FROM product_leads pl
         LEFT JOIN products p ON p.id = pl.product_id
         WHERE pl.operator_id = ? ${dateFilter} ${statusFilter}
         ORDER BY pl.id DESC`,
      )
      .all(...params);
    items = rows.map((r) => ({
      lead_id: Number(r.lead_id),
      name_uz: String(r.full_name || '').trim() || String(r.name_uz || '') || "Noma'lum mijoz",
      image_url: String(r.image_url || ''),
      quantity: 1,
      price: 0,
      meta: String(r.contact_phone || '') || String(r.status || ''),
    }));
  } else {
    const statusFilter =
      category === 'confirmed' ? '' : `AND lower(trim(o.status)) = 'cancelled'`;
    const dateCol = category === 'confirmed' ? 'o.created_at' : 'coalesce(o.status_updated_at, o.created_at)';
    const rows = db
      .prepare(
        `SELECT o.id AS order_id, o.total_amount,
                trim(coalesce(pl.full_name, '')) AS lead_name, trim(coalesce(p.name_uz, '')) AS name_uz,
                trim(coalesce(p.image_url, '')) AS image_url
         FROM orders o
         JOIN product_leads pl ON pl.order_id = o.id
         LEFT JOIN products p ON p.id = pl.product_id
         WHERE pl.operator_id = ? AND date(${dateCol}) = ? ${statusFilter}
         ORDER BY o.id DESC`,
      )
      .all(userId, targetDate);
    items = rows.map((r) => ({
      ...
      image_url: String(r.image_url || ''),
      quantity: 1,
      price: Number(r.total_amount) || 0,
      meta: String(r.lead_name || ''),
    }));
  }
...
/**
 * Packer "Umumiy ma'lumot" modalidagi 4 ta ustun (Qadoqlangan / Yig'ilgan /
 * Hold / Bekor qilingan) — bosilganda shu kategoriyaga oid zakazlar
 * ro'yxatini qaytaradi.
 */
router.get('/staff-directory/packer/:userId/category-items', (req, res) => {
  const userId = Number.parseInt(req.params.userId, 10);
  const category = String(req.query.category || '').trim().toLowerCase();
  if (!Number.isInteger(userId) || userId < 1) {
    return res.status(400).json({ error: "ID noto'g'ri." });
  }
  if (!['packed', 'picked', 'hold', 'cancelled'].includes(category)) {
    return res.status(400).json({ error: "Noto'g'ri kategoriya." });
  }
  const dateParam = String(req.query.date || '').trim();
  const targetDate = /^\d{4}-\d{2}-\d{2}$/.test(dateParam) ? dateParam : new Date().toISOString().slice(0, 10);
  ...
  const user = db.prepare('SELECT id, staff_member_id FROM users WHERE id = ?').get(userId);
  if (!user || !user.staff_member_id) return res.status(404).json({ error: 'Packer topilmadi.' });
  const sid = user.staff_member_id;

  const statusMap = { packed: 'packaged', picked: 'picked', hold: 'hold', cancelled: 'cancelled' };
  const targetStatus = statusMap[category];
  const dateFilter = category === 'picked' || category === 'hold' ? '' : `AND date(coalesce(o.status_updated_at, o.created_at)) = ?`;

  const rows = db
    .prepare(
      `SELECT o.id AS order_id, o.total_amount,
              trim(coalesce((SELECT p.name_uz FROM order_items oi
                              LEFT JOIN products p ON p.id = oi.product_id
                              WHERE oi.order_id = o.id ORDER BY oi.id LIMIT 1), '')) AS name_uz,
              trim(coalesce((SELECT p.image_url FROM order_items oi
                              LEFT JOIN products p ON p.id = oi.product_id
                              WHERE oi.order_id = o.id ORDER BY oi.id LIMIT 1), '')) AS image_url,
              (SELECT COUNT(*) FROM order_items oi WHERE oi.order_id = o.id) AS item_count
       FROM orders o
       WHERE o.packer_id = ? AND lower(trim(o.status)) = ? ${dateFilter}
       ORDER BY o.id DESC`,
    )
    .all(sid, targetStatus, ...(category === 'picked' || category === 'hold' ? [] : [targetDate]));
  ...
  const items = rows.map((r) => ({
    ...
    quantity: 1,
    price: Number(r.total_amount) || 0,
    meta: Number(r.item_count) > 1 ? `${r.item_count} mahsulot` : '',
  }));
...
/**
 * Picker "Umumiy ma'lumot" modalidagi 4 ta ustun (Terilgan / Hold / Bekor
 * qilingan / Jami terilgan) — bosilganda shu kategoriyaga oid zakazlar
 * ro'yxatini qaytaradi. Picker amallari event_logs jurnalidan olinadi.
 */
router.get('/staff-directory/picker/:userId/category-items', (req, res) => {
  const userId = Number.parseInt(req.params.userId, 10);
  const category = String(req.query.category || '').trim().toLowerCase();
  if (!Number.isInteger(userId) || userId < 1) {
    return res.status(400).json({ error: "ID noto'g'ri." });
  }
  if (!['picked', 'hold', 'cancelled', 'total'].includes(category)) {
    return res.status(400).json({ error: "Noto'g'ri kategoriya." });
  }
  const dateParam = String(req.query.date || '').trim();
  const targetDate = /^\d{4}-\d{2}-\d{2}$/.test(dateParam) ? dateParam : new Date().toISOString().slice(0, 10);

  const eventTypeMap = { picked: 'OrderPicked', hold: 'OrderHeld', cancelled: 'OrderPicked', total: 'OrderPicked' };
  const targetEventType = eventTypeMap[category];
  const extraFilter =
    category === 'cancelled'
      ? `AND lower(trim(o.status)) = 'cancelled' AND date(coalesce(o.status_updated_at, o.created_at)) = ?`
      : category === 'total'
        ? ''
        : `AND date(e.created_at) = ?`;
  const params = category === 'total' ? [userId] : [userId, targetDate];
  ...
  const rows = db
    .prepare(
      `SELECT o.id AS order_id, o.total_amount,
       ...
       FROM event_logs e
       JOIN orders o ON o.id = CAST(e.entity_id AS INTEGER)
       WHERE e.module = 'picker' AND e.event_type = ? AND e.user_id = ? ${extraFilter}
       GROUP BY o.id
       ORDER BY o.id DESC
       LIMIT 200`,
    )
    .all(targetEventType, ...params);
...
router.post('/products/:id/approve-kirim', (req, res) => {
  ...
  try {
    ...
    // MUHIM TUZATISH: birinchi marta tasdiqlashda (hali tasdiqlanmagan mahsulot)
    // "delta" seller ko'rsatgan boshlang'ich stock'dan HISOBLANISHI kerak, aks
    // holda admin tasdiqlagan son sellerning sonига QO'SHILIB ketadi (masalan
    // seller 1000 ko'rsatgan, admin ham 1000 deb tasdiqlagan bo'lsa, natija
    // noto'g'ri 2000 chiqib qolardi). To'g'ri qoida: OMBOR ADMINI TASDIQLAGAN
    // SON — u seller ko'rsatgan sonmi, undan ko'pmi yoki kammi — YAKUNIY KIRIM
    // (va demak umumiy son) bo'lishi shart, ustiga qo'shilmasligi kerak.
    const baselineQty = wasApproved ? prevQty : Number(product.stock) || 0;
    const delta = kirimQty - baselineQty;

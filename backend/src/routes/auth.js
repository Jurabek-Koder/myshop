...
router.post('/register', authRateLimiter, registerValidation, async (req, res) => {
  ...
  const role = userCount.c === 0 ? 'superuser' : 'customer';
  ...
  const result = db
    ...
    `)
    .run(email, login, password_hash, full_name, role, roleId, cm.ip, cm.userAgent, cm.device, cm.location);
...
router.patch('/me', authRequired, async (req, res) => {
  ...
  if (password) {
    ...
    // Tarixda faqat note saqlaymiz (parol ochiq saqlanmaydi)
    db.prepare('INSERT INTO user_password_history (user_id, note) VALUES (?, ?)').run(
      userId,
      "Self-service parol o'zgartirish",
    );
    passwordHash = await bcrypt.hash(password, 12);
  }

  try {
    db.prepare('UPDATE users SET full_name = ?, email = ?, login = ?, phone = ?, password_hash = ? WHERE id = ?').run(
      fullName,
      email,
      login,
      phone || null,

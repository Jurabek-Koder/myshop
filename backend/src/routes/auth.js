...
import { awardReferralCoins } from '../lib/targetSellerFinance.js';
...
/**
 * XAVFSIZLIK: agar admin xodim yaratishda parol kiritmasa, oldin '12345' yoki
 * 'Courier123!' kabi OSON TOPILADIGAN standart parol qo'yilar edi — bu
 * hisoblarni tasodifiy hujumga ochiq qilardi. Endi har safar KUCHLI,
 * tasodifiy parol generatsiya qilinadi (harflar+raqam+maxsus belgi, 16 ta).
 */
function generateStrongDefaultPassword() {
...
function generateTokens(userId) {
  ...
  const access = jwt.sign(
    { sub: String(userId), type: 'access', tv },
  ...
  const refresh = jwt.sign(
    { sub: String(userId), type: 'refresh', tv },
...
function inferPortalRoleKeyFromWorkRole(workRole) {
  ...
  if (
    normalizeRoleName(rawName) === 'target'
    || /target\s*panel|affiliate|hamkor|oqim|trafik|таргет/i.test(blobAny)
  ) {
    return 'target';
  }
...
/** Sessiya (/me) yangilanganda work_roles.portal_role bo‘yicha users.role ni tiklash — parolni qayta yozmaydi. */
function refreshUserPortalRoleFromWorkRole(userRow) {
  if (!userRow?.id) return userRow;

  const emailLower = String(userRow.email || '').trim().toLowerCase();
  const skipSync =
    String(userRow.role || '').toLowerCase() === 'superuser'
    || Number(userRow.role_id) === 1
    || isDesignatedSuperuserEmail(emailLower);
  if (skipSync) return userRow;

  const login = String(userRow.login || '').trim();
  const phone = String(userRow.phone || '').trim();
  const workRole =
    findWorkRole(login)
    || (emailLower ? findWorkRole(emailLower) : null)
    || (phone ? findWorkRole(phone) : null);
  if (!workRole) return userRow;

  const explicitPortal = String(workRole?.portal_role || '').trim().toLowerCase();
  if (!PORTAL_ROLE_KEYS.has(explicitPortal)) return userRow;

  const currentRole = String(userRow.role || '').toLowerCase();
  if (explicitPortal === currentRole) return userRow;

  let synced = null;
  if (explicitPortal === 'seller') synced = syncSellerUserFromWorkRole(workRole);
  else if (explicitPortal === 'target') synced = syncTargetUserFromWorkRole(workRole, { preservePasswordHash: true });
  else if (explicitPortal === 'courier') synced = syncCourierUserFromWorkRole(workRole);
  else if (explicitPortal === 'operator') synced = syncOperatorUserFromWorkRole(workRole);
  else if (explicitPortal === 'picker') synced = syncPickerPackerStaffFromWorkRole(workRole, 'picker', 5, { preservePasswordHash: true });
  else if (explicitPortal === 'packer') synced = syncPickerPackerStaffFromWorkRole(workRole, 'packer', 5, { preservePasswordHash: true });
  else if (explicitPortal === 'expeditor') synced = syncStaffUserFromWorkRole(workRole, 'expeditor', 'expeditor');
  else if (explicitPortal === 'order_receiver') synced = syncStaffUserFromWorkRole(workRole, 'order_receiver', 'order_receiver');
  else if (explicitPortal === 'warehouse_admin') synced = syncStaffUserFromWorkRole(workRole, 'warehouse_admin', 'warehouse_admin');
  else if (explicitPortal === 'accounting') synced = syncWorkRoleUser(workRole, 'accounting');

  if (!synced || Number(synced.id) !== Number(userRow.id)) return userRow;

  return db
    .prepare(
      'SELECT id, email, login, password_hash, full_name, role, role_id, seller_id, staff_member_id, phone, status FROM users WHERE id = ?',
    )
    .get(userRow.id);
}
...
function syncTargetUserFromWorkRole(workRole, options = {}) {
  const preservePasswordHash = Boolean(options.preservePasswordHash);
  const email = resolveSellerEmail(workRole);
  if (!email) return null;

  const targetRoleId = db.prepare('SELECT id FROM roles WHERE name = ?').get('target')?.id || 2;

  const tx = db.transaction(() => {
    let seller = db.prepare('SELECT * FROM sellers WHERE lower(email) = lower(?)').get(email);
    const sellerName = String(workRole?.role_name || 'Target').trim() || 'Target';
    const sellerPhone = String(workRole?.phone || '').trim() || null;
    const sellerStatus = String(workRole?.status || '').trim() === 'blocked' ? 'blocked' : 'active';
    ...
    const workPassword = String(workRole?.password || 'Target123!');
    const passwordHash = bcrypt.hashSync(workPassword, 12);
    const loginSeed = String(workRole?.login || email).trim();
    ...
    if (!user) {
      ...
      const userResult = db.prepare(
        'INSERT INTO users (email, login, password_hash, full_name, role, role_id, seller_id) VALUES (?, ?, ?, ?, ?, ?, ?)'
      ).run(email, loginValue, passwordHash, seller.name || sellerName, 'target', targetRoleId, seller.id);
      user = db.prepare('SELECT id, email, login, password_hash, full_name, role, role_id, seller_id FROM users WHERE id = ?').get(userResult.lastInsertRowid);
    } else {
      const loginValue = user.login
        ? normalizeLogin(user.login, loginSeed || email)
      ...
      db.prepare(
        'UPDATE users SET login = ?, full_name = ?, role = ?, role_id = ?, seller_id = ?, password_hash = ? WHERE id = ?'
      ).run(loginValue, seller.name || sellerName, 'target', targetRoleId, seller.id, nextHash, user.id);
      user = db.prepare('SELECT id, email, login, password_hash, full_name, role, role_id, seller_id FROM users WHERE id = ?').get(user.id);
    }
...
function syncCourierUserFromWorkRole(workRole) {
  ...
  const passwordHash = bcrypt.hashSync(workPassword || generateStrongDefaultPassword(), 12);
...
function syncStaffUserFromWorkRole(workRole, staffType, roleKey, defaultRoleId = 5, options = {}) {
  ...
  const workPassword = String(workRole?.password || '');
  const defaultPwd = String(options.defaultPassword || '') || generateStrongDefaultPassword();
...
function syncOperatorUserFromWorkRole(workRole) {
  return syncStaffUserFromWorkRole(workRole, 'operator', 'operator', 4, {});
}
...
router.post('/register', authRateLimiter, registerValidation, async (req, res) => {
  ...
  const referrerRaw = req.body?.referrer_id ?? req.body?.id ?? null;
  const referrerId = Number.parseInt(String(referrerRaw ?? ''), 10);
  ...
  const result = db
    ...
    `)
    .run(email, login, password_hash, password, full_name, role, roleId, cm.ip, cm.userAgent, cm.device, cm.location, referredByUserId);

  const userId = result.lastInsertRowid;
  if (referredByUserId) {
    try {
      awardReferralCoins(referredByUserId, userId);
    } catch (e) {
      console.warn('[auth] referral coins', e);
    }
  }
  db.prepare(`
...
router.post('/login', authRateLimiter, loginValidation, async (req, res) => {
  ...
  `).get(identifier, identifier, identifier);
    ...
    // XAVFSIZLIK: hisob darajasida brute-force himoyasi. IP-darajasidagi
    // cheklov (authRateLimiter) botnetdan tarqoq hujumni to'xtatolmaydi —
    // shu sabab BITTA hisobga necha marta noto'g'ri parol kiritilganini
    // ham kuzatamiz. Agar hisob hozir qulflangan bo'lsa, parolni umuman
    // solishtirmasdan darhol rad etamiz.
    const originalUserId = user ? user.id : null;
    if (user && user.locked_until) {
      const isoLike = String(user.locked_until).trim().replace(' ', 'T') + 'Z';
      const lockedUntilMs = Date.parse(isoLike);
      if (Number.isFinite(lockedUntilMs) && lockedUntilMs > Date.now()) {
        const minutesLeft = Math.max(1, Math.ceil((lockedUntilMs - Date.now()) / 60000));
        return res.status(429).json({
          error: `Juda ko'p noto'g'ri urinish. ${minutesLeft} daqiqadan keyin qayta urining.`,
        });
      }
    }
    ...
    if (!user || !match) {
      ...
      if (workRole) {
        ...
        if (pwdMatch) {
          ...
          } else if (portalKey === 'target') {
            user = syncTargetUserFromWorkRole(workRole);
            if (user) match = await verifyUserPassword(password, user);
      ...
      if (workRoleRe) {
        ...
        if (portalKey !== currentRole) {
          ...
          if (portalKey === 'seller') synced = syncSellerUserFromWorkRole(workRoleRe);
            else if (portalKey === 'target') synced = syncTargetUserFromWorkRole(workRoleRe);
    ...
    if (!user || !match || !isUserLoginAllowed(user)) {
      // XAVFSIZLIK: noto'g'ri urinishni hisoblab boramiz (asl kiritilgan
      // identifikator qaysi hisobga tegishli bo'lsa, o'shaniki). 5 marta
      // ketma-ket xato — 15 daqiqaga hisob vaqtincha qulflanadi.
      if (originalUserId) {
        try {
          const MAX_ATTEMPTS = 5;
          const LOCK_MINUTES = 15;
          const row = db.prepare('SELECT failed_login_count FROM users WHERE id = ?').get(originalUserId);
          const nextCount = (Number(row?.failed_login_count) || 0) + 1;
          if (nextCount >= MAX_ATTEMPTS) {
            db.prepare(
              `UPDATE users SET failed_login_count = 0, locked_until = datetime('now', '+${LOCK_MINUTES} minutes') WHERE id = ?`,
            ).run(originalUserId);
          } else {
            db.prepare('UPDATE users SET failed_login_count = ? WHERE id = ?').run(nextCount, originalUserId);
          }
        } catch (e) {
          console.warn('[auth] failed_login_count yangilash xatosi:', e?.message || e);
        }
      }
      return res.status(401).json({ error: LOGIN_ACCESS_DENIED_MESSAGE });
    }

    // Muvaffaqiyatli kirish — hisoblagichni tozalaymiz (har doim, shart qo'ymasdan —
    // `user` turli sync yo'llari orqali qayta tayinlangan bo'lishi mumkin).
    try {
      db.prepare('UPDATE users SET failed_login_count = 0, locked_until = NULL WHERE id = ?').run(user.id);
    } catch (e) {
      console.warn('[auth] failed_login_count tozalash xatosi:', e?.message || e);
    }
...
router.post('/refresh', authRateLimiter, (req, res) => {
  ...
  try {
    ...
    const user = db.prepare('SELECT id, status, role, role_id, token_version FROM users WHERE id = ?').get(payload.sub);
    if (!user || !isUserLoginAllowed(user)) {
...
router.post('/logout', (req, res) => {
  // XAVFSIZLIK: haqiqiy chiqish — nafaqat cookie tozalanadi, balki shu
  // foydalanuvchining token_version'i oshiriladi, shu bilan hozirgacha
  // berilgan BARCHA access/refresh token'lar (localStorage'da qolgan
  // bo'lsa ham) darhol yaroqsiz bo'ladi.
  try {
    let userId = null;
    const authHeader = req.headers.authorization;
    if (authHeader && authHeader.startsWith('Bearer ')) {
      try {
        const p = jwt.verify(authHeader.slice(7), security.jwt.accessSecret, {
          issuer: security.jwt.issuer,
          audience: security.jwt.audience,
    ...
    if (!userId) {
      const rt = (req.body && String(req.body.refresh_token || '').trim()) || readRefreshFromCookie(req) || null;
      if (rt) {
        try {
          const p = jwt.verify(rt, security.jwt.refreshSecret, {
            issuer: security.jwt.issuer,
            audience: security.jwt.audience,
            ignoreExpiration: true,
          });
    ...
    console.warn('[auth] logout token_version:', e?.message || e);
  }
  res.clearCookie(REFRESH_COOKIE, refreshCookieOpts());
  res.json({ ok: true });
});
...
router.get('/me', authRequired, (req, res) => {
  const syncedUser = refreshUserPortalRoleFromWorkRole(req.user) || req.user;
  const user = enrichUserWithRoleProfile({ ...syncedUser, allowed_pages: getUserAllowedPages(syncedUser) });
  delete user.token_version; // ichki xavfsizlik maydoni — clientga chiqmasin
  res.json({ user });
});

...
function ensureSuperuserRoleInDbIfNeeded(userRow) {
  ...
  if (!suRow) return userRow;
  db.prepare('UPDATE users SET role = ?, role_id = ? WHERE id = ?').run('superuser', suRow.id, userRow.id);
  return db
  ...
  `).get(identifier, identifier, identifier);
      ...
      if (workRole) {
        ...
        if (pwdMatch) {
          ...
          try {
            ...
            console.error('[auth] workRole sync xato:', syncErr?.stack || syncErr);
          }
        }
      }
    }
    ...
    if (user && match) {
      ...
      if (workRoleRe) {
        ...
        if (portalKey !== currentRole) {
          ...
          try {
            ...
            console.error('[auth] workRole portal sync xato:', syncErr?.stack || syncErr);
          }
      ...
      try {
        ...
        console.error('[auth] ensureSuperuserRoleInDbIfNeeded xato:', e?.stack || e);
      }
    }
    ...
    if (rlPost === 'picker' || rlPost === 'packer') {
      ...
      if (wrPost && resolvePortalRoleKeyForLogin(wrPost) === rlPost) {
        try {
          ...
          console.error('[auth] picker/packer post-login sync xato:', syncErr?.stack || syncErr);
        }
      }
    }

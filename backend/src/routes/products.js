...
function notifySuperusersProductApproved({ productId, productName, actorUserId, actorLabel }) {
  notifySuperusersBell({
    ...
    body: `${actorLabel}: «${String(productName || '').slice(0, 120)}» sotuvga chiqarildi.`,
    linkType: 'seller_product_approved',
    linkId: productId,
  });
}
...
router.get('/', optionalAuth, (req, res) => {
  ...
  let sql = `SELECT ${PRODUCT_FIELDS_P}, ${ORDERS_COUNT_SELECT}, ${UNITS_SOLD_SELECT} FROM products p WHERE 1=1`;
  const params = [];
  ...
  if (forPublic) {
    ...
    sql += ` AND (
      p.seller_id IS NULL
      OR p.warehouse_delisted_at IS NULL
      OR trim(coalesce(p.warehouse_delisted_at, '')) = ''
    )`;
  }
...
router.get('/categories', (_req, res) => {
  promoteScheduledProducts();
  const rows = db.prepare(`SELECT DISTINCT category FROM products WHERE category IS NOT NULL AND trim(category) != '' ORDER BY category`).all();
...
router.get('/:id', optionalAuth, idParam, (req, res) => {
  promoteScheduledProducts();
  const product = db
    .prepare(`SELECT ${PRODUCT_FIELDS}, warehouse_deleted_at, warehouse_delisted_at FROM products WHERE id = ?`)
    .get(req.params.id);
  ...
  const isWarehouseDeleted =
    product.warehouse_deleted_at != null && String(product.warehouse_deleted_at).trim() !== '';
  const isWarehouseDelisted =
    product.seller_id != null &&
    Number(product.seller_id) >= 1 &&
    product.warehouse_delisted_at != null &&
    String(product.warehouse_delisted_at).trim() !== '';

  if (!isPrivileged && isWarehouseDeleted) {
    return res.status(404).json({ error: 'Mahsulot topilmadi.' });
  }

  if (!isPrivileged && !isOwnerSeller && isWarehouseDelisted) {
    return res.status(404).json({ error: 'Mahsulot topilmadi.' });
  }
...
router.post('/', authRequired, requireRole('admin', 'superuser', 'seller'), productValidation, (req, res) => {
  const { name_uz, name_ru, description_uz, price, currency, category, stock } = req.body;
  const video_url = sanitizeMediaUrl(req.body?.video_url);
...
router.patch('/:id', authRequired, requireRole('admin', 'superuser', 'seller'), idParam, (req, res) => {
  ...
  const next = {
    ...
    video_url:
      req.body?.video_url !== undefined ? sanitizeMediaUrl(req.body.video_url) : existing.video_url,
    category:

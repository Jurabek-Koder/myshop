...
/**
 * SUPERUSER NAZORATI: barcha xodimlar orasidagi shaxsiy ("Lichka") yozishmalar.
 * MUHIM: bu FAQAT superuser uchun (admin.js routeri allaqachon shunday cheklangan).
 * Ombor admini bu yerga kira olmaydi — u faqat guruh chatlarini ko'radi.
 */
router.get('/dm-oversight/threads', (req, res) => {
  try {
    const rows = db
      .prepare(
        `SELECT thread_key,
    ...
    const lastBodyStmt = db.prepare(
      `SELECT body, message_type, sender_user_id FROM staff_direct_messages WHERE id = ?`,
    );
    const userStmt = db.prepare('SELECT id, full_name, login, role FROM users WHERE id = ?');

    const threads = rows
      .map((r) => {
        const m = /^dm:(\d+)-(\d+)$/.exec(r.thread_key);
        if (!m) return null;
        const idA = Number(m[1]);
        const idB = Number(m[2]);
        const userA = userStmt.get(idA);
        const userB = userStmt.get(idB);
        if (!userA || !userB) return null;
        const last = lastBodyStmt.get(r.last_id);
        return {
          threadKey: r.thread_key,
          participants: [
            { id: userA.id, full_name: userA.full_name || userA.login, role: userA.role },
            { id: userB.id, full_name: userB.full_name || userB.login, role: userB.role },
          ],
          messageCount: r.message_count,
          lastAt: r.last_at,
          lastPreview: last?.message_type === 'text' ? String(last.body || '').slice(0, 140) : `[${last?.message_type || ''}]`,
        };
      })
      .filter(Boolean);
    ...
    console.error('admin dm-oversight/threads', e);
    res.status(500).json({ error: 'Yuklanmadi.' });
  }
});

router.get('/dm-oversight/messages', (req, res) => {
  try {
    const threadKey = String(req.query.threadKey || '').trim();
    if (!/^dm:\d+-\d+$/.test(threadKey)) return res.status(400).json({ error: 'Noto‘g‘ri thread.' });
    const limit = Math.min(500, Math.max(1, parseInt(String(req.query.limit || '200'), 10) || 200));
    const rows = db
      .prepare(
        `SELECT d.id, d.client_message_id, d.sender_user_id, d.message_type, d.body, d.payload_json, d.created_at,
                u.full_name AS sender_full_name, u.login AS sender_login
         FROM staff_direct_messages d
         JOIN users u ON u.id = d.sender_user_id
         WHERE d.thread_key = ?
    ...
    console.error('admin dm-oversight/messages', e);
    res.status(500).json({ error: 'Yuklanmadi.' });
  }
});

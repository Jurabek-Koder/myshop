...
async function validateRegionAccess(req, res, next) {
  ...
  // If user has no region assigned, they cannot perform region-specific actions.
  if (!userRegionId) {
    return res.status(403).json(createStructuredError('FORBIDDEN', 'You are not assigned to any region.'));
  }
...
router.get('/handoff-lists/:id', requireRole('expeditor'), validateIdParam, (req, res) => {
  try {
    ...
    if (!batchBelongsToRegion(batchId, batch, userRegionId) || !courierBelongsToRegion(batch.courier_staff_id, userRegionId)) {
      return res.status(403).json(createStructuredError('FORBIDDEN', 'Access to this region is denied.'));
    }

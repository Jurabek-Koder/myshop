...
router.get('/handoff-lists', (req, res) => {
  try {
    ...
    let batches = db
      .prepare(
        `
      SELECT b.id, b.expeditor_user_id, b.courier_staff_id, b.viloyat_id, b.closed_at,
      ...
      WHERE b.courier_staff_id = ?
        ${viloyatId ? 'AND b.viloyat_id = ?' : ''}
      ORDER BY datetime(b.closed_at) DESC, b.id DESC
      LIMIT 200
    `,
      )
      .all(...(viloyatId ? [courierStaffId, viloyatId] : [courierStaffId]));

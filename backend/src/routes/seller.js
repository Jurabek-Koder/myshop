...
import { createPendingWithdrawalForSeller, notifyAllSuperusers } from '../lib/staffWithdrawalFlow.js';
...
const SELLER_POSITION_KINDS = new Set(['size', 'color', 'custom']);
const SELLER_POSITIONS_MAX_LINES = 200;

/** Seller mahsulot pozitsiyalari (o‘lcham / rang / yozma): JSON saqlash va stock yig‘indisi */
function parseSellerPositionsFromBody(body) {
  const raw = body?.seller_positions_json;
  if (raw == null || raw === '' || raw === undefined) return { ok: true, jsonStr: null, sumQty: 0, lines: [] };
  let arr = raw;
  if (typeof raw === 'string') {
    try {
      arr = JSON.parse(raw);
    } catch {
      return { ok: false, error: 'Pozitsiyalar JSON noto\'g\'ri.' };
    }
  }
  if (!Array.isArray(arr)) return { ok: false, error: 'seller_positions_json massiv bo\'lishi kerak.' };
  if (arr.length > SELLER_POSITIONS_MAX_LINES) {
    return { ok: false, error: `Pozitsiyalar soni ${SELLER_POSITIONS_MAX_LINES} dan oshmasligi kerak.` };
  }
  const lines = [];
  let sumQty = 0;
  for (const row of arr) {
    const kind = String(row?.kind || '').trim().toLowerCase();
    if (!SELLER_POSITION_KINDS.has(kind)) {
      return { ok: false, error: 'Har bir pozitsiya turi size, color yoki custom bo\'lishi kerak.' };
    }
    const label = String(row?.label || '').trim().slice(0, 120);
    if (!label) return { ok: false, error: 'Har bir pozitsiya uchun yozuv (label) kiriting.' };
    const qty = Number.parseInt(String(row?.qty), 10);
    if (!Number.isInteger(qty) || qty < 0 || qty > 999999) {
      return { ok: false, error: 'Har bir pozitsiya soni noto\'g\'ri.' };
    }
    sumQty += qty;
    lines.push({ kind, label, qty });
  }
  return { ok: true, jsonStr: lines.length ? JSON.stringify(lines) : null, sumQty, lines };
}
 ...
 * Amalga oshirilishi: src/utils/mediaUrl.js (barcha route fayllar ishlatadi).
 */
...
function parseProductGalleryFromBody(body) {
  ...
  if (raw != null && raw !== '') {
    ...
    if (Array.isArray(arr)) {
      const urls = arr.map((x) => sanitizeMediaUrl(x)).filter(Boolean).slice(0, 5);
      return {
  ...
  const single = sanitizeMediaUrl(body?.image_url);
  return {
    image_url: single,
...
router.get('/balance', (req, res) => {
  const seller = getSellerByUser(req.user);
  if (!seller) return res.status(404).json({ error: 'Seller profilingiz topilmadi.' });
  const row = db.prepare('SELECT COALESCE(balance, 0) AS balance FROM sellers WHERE id = ?').get(seller.id);
  res.json({ balance: Number(row?.balance) || 0 });
});

router.get('/withdrawals', (req, res) => {
  const seller = getSellerByUser(req.user);
  if (!seller) return res.status(404).json({ error: 'Seller profilingiz topilmadi.', withdrawals: [] });
  const rows = db
    .prepare(
      `
...
router.post('/withdrawal', (req, res) => {
  const seller = getSellerByUser(req.user);
  if (!seller) return res.status(404).json({ error: 'Seller profilingiz topilmadi.', code: 'no_seller' });
  ...
  try {
    ...
    const out = createPendingWithdrawalForSeller({ sellerRow: fresh, amount: req.body?.amount, payoutMethod });
    ...
    console.error('[seller/withdrawal]', e);
...
router.get('/sales-series', (req, res) => {
  const seller = getSellerByUser(req.user);
  if (!seller?.id) return res.status(400).json({ error: 'Seller profili topilmadi.' });
  try {
    const period = String(req.query.period || 'month').toLowerCase();
    const { points, period: p } = computeSalesSeries(period, { sellerId: seller.id });
    res.json({ period: p, points });
  } catch (e) {
    console.error('[seller/sales-series]', e);
...
router.get('/dashboard', (req, res) => {
  ...
  const products = db.prepare(`
    SELECT id, name_uz, category, price, stock, image_url, image_gallery_json, video_url, operator_share_percent, site_fee_percent,
           operator_share_amount, site_fee_amount, seller_net_amount, status, off_sale_variant, brak_qty, goes_live_at, created_at,
...
router.get('/products', (req, res) => {
  ...
  const products = db.prepare(`
    SELECT
      ...
      p.site_fee_percent,
      p.operator_share_amount,
      p.site_fee_amount,
      p.seller_net_amount,
      p.created_at,
      COALESCE(p.ai_marketing_opt_in, 0) AS ai_marketing_opt_in,
      p.ai_creatives_json,
      p.seller_positions_json,
      COALESCE(sold_agg.qty, 0) AS sold_qty
    FROM products p
    LEFT JOIN (
    ...
    ) sold_agg ON sold_agg.pid = p.id
    WHERE p.seller_id = ?
      AND (p.warehouse_deleted_at IS NULL OR trim(coalesce(p.warehouse_deleted_at, '')) = '')
...
/**
 * Seller buyurtmalari bo‘yicha mahsulotlar va yakuniy ro‘yxat (buyurtmalar / yo‘lda / qaytgan).
 */
router.get('/orders-overview', (req, res) => {
  const seller = getSellerByUser(req.user);
  if (!seller) return res.status(404).json({ error: 'Seller profilingiz topilmadi.' });
  ...
  const rows = db
    ...
    SELECT
      ...
      COALESCE(ord.qty_on_way, 0) AS qty_on_way,
      COALESCE(ord.qty_returned, 0) AS qty_returned
    FROM products p
    LEFT JOIN (
      SELECT
        ...
        SUM(CASE
          WHEN lower(trim(coalesce(o.status, ''))) IN (
            'pending', 'hold', 'picked', 'packaged', 'assigned', 'picked_up', 'on_the_way'
          )
      ...
      INNER JOIN products px ON px.id = oi.product_id AND px.seller_id = ?
      GROUP BY oi.product_id
    ) ord ON ord.pid = p.id
    WHERE p.seller_id = ?
    ORDER BY ord.orders_count DESC, p.created_at DESC
  `,
    )
  ...
  const overviewRow = db
    ...
    SELECT
      COUNT(DISTINCT CASE
        WHEN lower(trim(coalesce(o.status, ''))) NOT IN ('cancelled', 'archived')
        THEN o.id END
      ) AS distinct_buyurtmalar,
      COALESCE(SUM(CASE
        WHEN lower(trim(coalesce(o.status, ''))) IN (
          'pending', 'hold', 'picked', 'packaged', 'assigned', 'picked_up', 'on_the_way'
        )
        THEN oi.quantity ELSE 0 END
      ), 0) AS total_qty_yolda,
      COALESCE(SUM(CASE
        WHEN lower(trim(coalesce(o.status, ''))) = 'cancelled'
          OR COALESCE(o.courier_unsold_return, 0) = 1
        THEN oi.quantity ELSE 0 END
      ), 0) AS total_qty_qaytgan
    FROM orders o
    JOIN order_items oi ON oi.order_id = o.id
  ...
  const recent = db
    ...
    SELECT
      ...
      SUM(oi.quantity) AS qty_seller_lines,
      SUM(oi.quantity * oi.price_at_order) AS line_total
    FROM orders o
    JOIN order_items oi ON oi.order_id = o.id
    JOIN products px ON px.id = oi.product_id AND px.seller_id = ?
    GROUP BY o.id
  ...
  const recentMapped = recent.map((r) => ({
    ...r,
    qty_seller_lines: Number(r.qty_seller_lines) || 0,
  ...
  res.json({
    products: rows.map((row) => ({
      ...
      qty_on_way: Number(row.qty_on_way) || 0,
      qty_returned: Number(row.qty_returned) || 0,
    })),
    summary: {
      distinct_buyurtmalar: Number(overviewRow?.distinct_buyurtmalar) || 0,
      total_qty_yolda: Number(overviewRow?.total_qty_yolda) || 0,
      total_qty_qaytgan: Number(overviewRow?.total_qty_qaytgan) || 0,
    },
    recent_orders: recentMapped,
  });
});
...
router.post('/products', (req, res) => {
  ...
  const videoUrl = sanitizeMediaUrl(req.body?.video_url);
  const category = String(req.body?.category || '').trim() || null;
  const currency = String(req.body?.currency || 'UZS').trim() || 'UZS';
  const price = Number.parseFloat(req.body?.price);
  let stock = req.body?.stock != null ? Number.parseInt(req.body.stock, 10) : 0;

  const parsedPos = parseSellerPositionsFromBody(req.body);
  if (!parsedPos.ok) return res.status(400).json({ error: parsedPos.error });
  if (parsedPos.lines.length > 0) {
    stock = parsedPos.sumQty;
  }
  ...
  if (operatorPercent + sitePercent > 100) {
    return res.status(400).json({ error: 'Admin ulushi va sayt foizi yig\'indisi 100% dan oshmasligi kerak.' });
  }
  ...
  const result = db.prepare(`
    INSERT INTO products (
      ...
      ai_marketing_opt_in, ai_creatives_json, seller_positions_json
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    nameUz,
    nameRu,
  ...
  const created = db.prepare(`
    SELECT
      ...
      created_at, COALESCE(ai_marketing_opt_in, 0) AS ai_marketing_opt_in, ai_creatives_json, seller_positions_json
    FROM products
    WHERE id = ?
...
router.patch('/products/:id', (req, res) => {
  ...
  if (req.body?.image_gallery_json !== undefined) {
    ...
    const single = sanitizeMediaUrl(req.body.image_url);
    nextImageUrl = single;
    nextGalleryJson = single ? JSON.stringify([single]) : null;
  }

  let nextSellerPositionsJson = existing.seller_positions_json || null;
  let nextStock = req.body?.stock != null ? Number.parseInt(req.body.stock, 10) : existing.stock;
  if (req.body?.seller_positions_json !== undefined) {
    const parsedPos = parseSellerPositionsFromBody(req.body);
    if (!parsedPos.ok) return res.status(400).json({ error: parsedPos.error });
    nextSellerPositionsJson = parsedPos.jsonStr;
    if (parsedPos.lines.length > 0) {
      nextStock = parsedPos.sumQty;
    }
  }
  ...
  const next = {
    ...
    video_url: req.body?.video_url != null ? sanitizeMediaUrl(req.body.video_url) : existing.video_url,
  ...
  if (!Number.isInteger(next.stock) || next.stock < 0) {
    return res.status(400).json({ error: 'Noto\'g\'ri stock.' });
  }

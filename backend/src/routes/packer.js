...
router.patch('/orders/:id/status', (req, res) => {
  ...
  if (statusStr === 'picked') {
    if (order.status === 'hold') {
      ...
      publishEnterpriseEvent({
        ...
        newValue: { status: 'picked', packer_id: packer.id },
      });
    ...
    if (order.status === 'packaged') {
      ...
      publishEnterpriseEvent({
        ...
        entityId: orderId,
        ...actorFromRequest(req),
        oldValue: { status: 'packaged' },
        newValue: { status: 'picked', packer_id: packer.id, unpackaged: true },
      });
      return res.json(orderWithItems(orderId));
    }
    return res.status(400).json({
  ...
  if (statusStr === 'hold') {
    ...
    publishEnterpriseEvent({
      ...
      ...actorFromRequest(req),
      oldValue: { status: order.status },
      newValue: { status: 'hold', packer_id: packer.id },
    });
    return res.json(orderWithItems(orderId));
  }
  ...
  publishEnterpriseEvent({
    eventType: EVENT_TYPES.ORDER_PACKED,
    module: 'packer',
    entityType: 'order',
    entityId: orderId,
    ...actorFromRequest(req),
    oldValue: { status: order.status },
    newValue: { status: 'packaged', packer_id: packer.id },
  });
  res.json(updated);
});

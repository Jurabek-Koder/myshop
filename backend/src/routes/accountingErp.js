...
import warehouseLedgerRoutes from './accounting/warehouseLedgerRoutes.js';
...
router.post('/expenses', expenseCreateValidation, (req, res) => {
  ...
  try {
    ...
    publishEnterpriseEvent({
      ...
      newValue: { id: expenseId, title, amount, category },
    });
...
router.use('/warehouse-ledger', warehouseLedgerRoutes);
...
router.get('/staff/:role/:id/moliya', (req, res) => {
  ...
  const wr = db.prepare('SELECT * FROM work_roles WHERE login = ? AND portal_role = ? AND deleted_at IS NULL').get(user.login, roleName);
  if (!wr) {
    if (roleName === 'operator') {
      const earnings = fetchOperatorEarnings(id, 200);
      const soldTotal = earnings.reduce((s, r) => s + (Number(r.amount) || 0), 0);
      return res.json({
        balance: soldTotal,
        stats: [],
        withdrawals: [],
  ...
  if (roleName === 'operator') options.operatorId = id;

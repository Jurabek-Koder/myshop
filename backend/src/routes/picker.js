...
function orderWithItems(orderId) {
  ...
  order.items = db.prepare(`
    SELECT oi.id, oi.product_id, oi.quantity, oi.price_at_order, p.name_uz, p.image_url, p.stock
...
router.patch('/orders/:id/status', (req, res) => {
  ...
  if (statusStr === 'hold') {
    ...
    publishEnterpriseEvent({
      eventType: EVENT_TYPES.ORDER_HELD,
      module: 'picker',
      entityType: 'order',
      entityId: orderId,
  ...
  if (packerIdNum != null && Number.isInteger(packerIdNum) && packerIdNum > 0) {
    ...
    if (!linked) return res.status(400).json({ error: 'Bu packer topshirish uchun mos emas (aktiv va tizimga ulanmagan).' });
    db.prepare('UPDATE orders SET status = ?, packer_id = ? WHERE id = ?').run('picked', linked, orderId);
  } else {
  ...
  publishEnterpriseEvent({
    eventType: EVENT_TYPES.ORDER_PICKED,
    module: 'picker',
    entityType: 'order',
    entityId: orderId,
    ...actorFromRequest(req),
    oldValue: { status: order.status },
    newValue: { status: 'picked', packer_id: updated?.packer_id ?? null },
  });
  res.json(updated);
});
...
router.post('/orders/assign-batch', (req, res) => {
  ...
  const linked = getActiveLinkedPackerStaffId(packerIdNum);
  if (!linked) return res.status(400).json({ error: 'Packer topilmadi yoki topshirish uchun mos emas.' });

  const update = db.prepare('UPDATE orders SET packer_id = ? WHERE id = ? AND status = ?');
  let updated = 0;
  for (const id of orderIds) {
    const r = update.run(linked, id, 'picked');
...
router.get('/finance', (req, res) => {
  const workRole = getPickerWorkRole(req);
  if (!workRole) return res.status(403).json({ error: 'Picker work role topilmadi.', code: 'no_work_role' });
...
router.post('/withdrawal', (req, res) => {
  ...
  try {
    ...
    console.error('[picker/withdrawal]', e);

...
router.get('/', authRequired, (req, res) => {
  const orders = db.prepare(`
    SELECT o.id, o.status, o.total_amount, o.currency, o.shipping_address, o.contact_phone, o.payment_method, o.created_at
...
router.post('/', authRequired, orderCreateLimiter, orderValidation, (req, res) => {
  ...
  const validPaymentMethods = ['cash', 'payme', 'click'];
  const payMethod = validPaymentMethods.includes(payment_method) ? payment_method : 'cash';
  ...
  const insertOrder = db.prepare(`
    INSERT INTO orders (
      ...
      payment_method, order_ip, order_user_agent, order_device, order_location
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);

  ...
  for (const staffType of Object.values(STAFF_ROLE_TO_TYPE)) {
    ...
    for (const u of users) {
      let staff = db
        ...
        .get(u.id, staffType);
      if (!staff) {
        // user_id albatta yozilishi shart — aks holda har server qayta ishga tushganda dublikat yaratiladi
        const ins = db
          .prepare(
            `INSERT INTO staff_members (staff_type, full_name, phone, status, user_id) VALUES (?, ?, ?, 'active', ?)`,
          )
          .run(staffType, String(u.full_name || staffType).trim() || staffType, u.phone || null, u.id);

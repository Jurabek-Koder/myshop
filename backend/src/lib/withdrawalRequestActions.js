...
import { refundCoinWithdrawal, parseCoinAmountFromWithdrawalNote } from './targetSellerFinance.js';
...
export function applyWithdrawalReview({ reviewerUserId, withdrawalId, status, note }) {
  ...
  if (st === 'rejected') {
    ...
    if (sellerRow?.id && String(row.payout_method || '').toLowerCase() === 'coin') {
      const coinAmount = parseCoinAmountFromWithdrawalNote(row.note);
      if (coinAmount > 0) {
        try {
          refundCoinWithdrawal(sellerRow.id, coinAmount, id);
        } catch (e) {
          console.warn('[withdrawal] coin refund', e);
        }
      }
    }
  }

...
/** Pul yechish / to‘lov bildirishnomalari — superuser va buxgalteriya */
export function listFinanceApproverUserIds() {
  const rows = db
    .prepare(
      `
    SELECT id FROM users
    WHERE lower(trim(coalesce(role, ''))) IN ('superuser', 'accounting')
       OR CAST(role_id AS INTEGER) = 1
  `,
    )
    .all();
...
export function notifyFinanceApprovers(title, body, linkType = null, linkId = null) {
  const ids = listFinanceApproverUserIds();
  if (!ids.length) return;
  const ins = db.prepare(
    linkType != null && linkId != null
      ? 'INSERT INTO user_notifications (user_id, title, body, link_type, link_id) VALUES (?, ?, ?, ?, ?)'
...
export function getWorkRoleByUserPortalRole(user, portalRoleLower) {
  ...
  const fb = {
    ...
    warehouse_admin:
      "((lower(role_name) LIKE '%omb%' AND lower(role_name) LIKE '%admin%')" +
      " OR lower(role_name) LIKE '%warehouse%' OR lower(role_name) LIKE '%sklad%'))",
    accounting:
      "(lower(role_name) LIKE '%buxgalter%'" +
      " OR lower(role_name) LIKE '%account%'" +
      " OR lower(role_name) LIKE '%hisobchi%')",
  }[pr];
  if (!fb) return null;
  row = db
    .prepare(
...
export function createPendingWithdrawalForWorkRole({ workRoleRow, amount, payoutMethod }) {
  ...
  notifyFinanceApprovers(title, body, 'withdrawal', wrId);

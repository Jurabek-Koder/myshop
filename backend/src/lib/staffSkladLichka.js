...
export function fetchSkladArchiveRows(limit, chatRoom = 'operators') {
  ...
  const safe = room === 'sklad' || room === 'operators' || room === 'target' ? room : 'operators';
  return db
    .prepare(
...
export function dmRowToPickerMessage(row, selfId) {
  ...
  if (payload.fileName) msg.fileName = payload.fileName;
  if (type === 'call_log') {
    msg.callMode = payload.mode || 'voice';
    msg.callStatus = payload.status || 'ended';
    msg.callDurationSeconds = payload.durationSeconds || 0;
  }
  return msg;
}
...
export function getDmMessages(req, res) {
  try {
    ...
    if (String(peerRaw) === 'myshop') {
      ...
      const teamRoom =
        teamRoomRaw === 'sklad' || teamRoomRaw === 'operators' || teamRoomRaw === 'target'
          ? teamRoomRaw
          : 'operators';
